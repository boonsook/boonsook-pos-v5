...
### S3 — auto-คลัง: ขายได้จริงแต่ติดธง "[สต็อกไม่ครบ]" ผิด (undersell + false flag) — 🔧 FIX บน branch `claude/phase-480-precheck-single-warehouse` (build 480, รอ owner smoke + Codex review · ยังไม่ merge)

> **แก้ (รอ review):** เลือก "precheck สะท้อนคลังเดียว" (ไม่ใช่ spill — คง concept 1-บิล-1-คลัง). +modules/warehouse_pick.js (pure `pickAutoWarehouseStock`) = single source ของ auto-pick (บ้านก่อน/max stock); deduct (main.js) + precheck (pos.js) ใช้ helper เดียวกัน → คลังตรงกัน 100% กัน false flag. legacy ไม่มี warehouse row → null → precheck fallback `products.stock` (= ค่าที่ deduct legacy branch ตัดจริง main.js:3211 → consistent; deviate จาก "block" ใน prompt เพราะ block จะขวาง legacy ที่ขายได้). block message ระบุ "คลัง {ชื่อ} มี {N}" + "เลือกคลังอื่น". +guard precheck_single_warehouse (8) + ปรับ pos_warehouse_deduct/checkout_precheck (intent เดิม). unit 1869 · e2e 14. ดู HANDOFF build 480.

...
**Phase 602 / build 601 (S4.1a — ฟอร์มรับงานสร้างงาน "เกิดมาปิดแล้ว" (born-done) ไม่ได้อีก · prevention-only · branch `claude/phase-602-service-create-prevention` รอ review):** **ปัญหา (S4.0 audit, PR #186):** July service NO_JV **6 งาน รวม 6,950 บาท** — งานที่ `status=done` แต่ `closed_at=null` + ไม่มี JV + ไม่ตัดสต็อก. **ต้นตอ (verify กับโค้ดจริงก่อนแก้):** closure side effects ถูกย้ายไป admin drawer ตั้งแต่ Phase 88.15/545 โดยตั้ง `COMPLETION_STATUSES = []` ในฟอร์มรับงานทั้งสาม (`service_form.js:602` · `ac_install.js:533` · `solar.js:740`) → `isClosure` เป็น false ตลอด → `closed_at: isClosure ? … : null` = null เสมอ + บล็อก `postJournalForServiceJob` (`service_form.js:695` · `ac_install.js:628` · `solar.js:902`) เป็น **dead code** — แต่ dropdown ยังมี `<option value="done">✅ เสร็จแล้ว` ค้างอยู่ (`service_form.js:255` · `ac_install.js:211` · `solar.js:571`) และ `normalizeServiceJobStatus("done")` คืน `"done"` (valid DB status) → **(a) admin เลือก done → INSERT ผ่าน = born-done (รายได้หายเงียบ)**; **(b) non-admin เลือก done → DB trigger `trg_service_jobs_insert_close_guard` (Phase 551, applied 2026-07-02) reject 42501 → บันทึกใบงานล้ม**. ⚠️ ไม่สรุปว่า 6 งานมาจาก admin เท่านั้น — อาจสร้างก่อน trigger 551 deploy (2026-07-02) หรือมาจาก trusted/service-role path. **verify เพิ่ม (ยืนยัน scope):** admin drawer (`main.js saveServiceJob`) **ไม่ใช่รูนี้** — สร้างงานใหม่เป็น done ที่ drawer จะ stamp `closed_at` (Phase 542 block) + โพสต์ JV ผ่าน `newJobAlreadyComplete` + ตัดสต็อก (deduct-on-close) ครบ → ฟอร์มรับงาน 3 ตัวคือรูเดียว. **แก้ (prevention-only, policy เดียวทุก role รวม admin — ไม่ใช่ role-conditional dropdown):** **(1) `modules/service_status.js`** +`SERVICE_INTAKE_CREATE_STATUSES = ["pending","in_progress","pending_review"]` + pure `normalizeServiceIntakeCreateStatus(status)`: trim+lowercase → allowed คงเดิม · `progress`→`in_progress` (draft เก่าเก็บค่า DB-safe) · `done`/`delivered`/`closed`→**`pending_review`** · null/empty/unknown/non-string→`pending`. **`normalizeServiceJobStatus` เดิมไม่แตะ contract** (admin drawer + writer อื่นยังต้องรับ completion statuses). **(2) ทั้ง 3 ฟอร์ม:** ลบ `<option value="done">` (เหลือ pending/in_progress/pending_review) + แทนคอมเมนต์ 88.15 เดิม (ที่สื่อว่า admin เลือก delivered/closed ได้) ด้วย help text **"ช่างเลือก 'รออนุมัติ' — แอดมินปิดงานจาก drawer ใบรับงาน"**; `const selectedStatus = normalizeServiceIntakeCreateStatus(container.querySelector("#xxStatusSel")?.value)` — **normalize ตั้งแต่อ่านค่าจาก DOM ครั้งเดียว** แล้วใช้ตัวเดียวกันต่อทั้ง `status: normalizeServiceJobStatus(selectedStatus)` และ `note: serviceJobNoteWithReviewMarker(…, selectedStatus)` → done ที่ inject ทาง DevTools/draft เก่า = **DB `pending` + note `[รออนุมัติแอดมิน]`** (โผล่ในคิว "รออนุมัติ" ให้แอดมินปิดต่อ); +normalize `draft.fields.status` **หลัง `loadServiceDraft` ก่อน `applyDraftFields`** (draft เก่า status=done → select ไม่กลายเป็นค่าว่าง). **(3) ลบ dead closure code ครบทั้ง 3 ไฟล์:** `COMPLETION_STATUSES` · `isClosure` · `closed_at` ternary ใน record · inline `postJournalForServiceJob` block (รวม `status: selectedStatus` ดิบที่ `ac_install.js:636` / `solar.js:910`) · import ที่ไม่ใช้ — **`service_form.js` คง `getServiceTransferCoa`** (ยังมี caller จริงที่ :324). **★ ไม่แตะ:** `main.js` (drawer close pipeline / Phase 542 closed_at stamp / Phase 545 UPDATE guard) · `modules/accounting/**` · stock deduct/restore/transfer internals · POS/checkout/refund/COGS · `insertServiceJobWithReplay`/client_uuid idempotency · payment_method/payment_slip_url/items_json/auto-transfer · LINE notify/ใบเสร็จ/slip verify · SQL/RLS/schema (**ไม่มี SQL ใหม่** — trigger 551 apply แล้ว) · **6 งานเดิมไม่ถูกซ่อม**. **Tests:** +`service_borndone_prevention_guard`(24 — source-regex เจาะบล็อก ไม่ grep ทั้งไฟล์: dropdown block ไม่มี done/delivered/closed แต่มี pending/in_progress/pending_review ครบ · `selectedStatus` ผ่าน `normalizeServiceIntakeCreateStatus` (และไม่อ่าน `.value` ดิบ) · record block มี `normalizeServiceJobStatus(selectedStatus)` + **ไม่มี `closed_at`** + ไม่มี raw status · note marker ใช้ selectedStatus ที่ normalize แล้ว · ไม่มี `COMPLETION_STATUSES`/`isClosure`/`postJournalForServiceJob` · draft normalize อยู่หลัง `loadServiceDraft` ก่อน `applyDraftFields` · help text ถูก · helper ไม่ผูก role); +8 intake test ใน `service_status_guard` (allowed/progress alias/completion→pending_review/trim+case/null-unknown→pending + **invariant: ทุก input → DB status ไม่เป็น completion เลย**); ปรับ `service_pos_jv_background_guard` (header comment stale + service assertions เดิม "background JV" → **intake forms ต้องไม่มี `postJournalForServiceJob`** + assert ว่า main.js drawer **ยังโพสต์ JV** เป็น single source; **POS awaited-JV assertions เดิมคงครบทุกข้อ ไม่อ่อนลง**); +1 assert ใน `service_mobile_draft_guard` (completion draft ถูก normalize ก่อน apply). companion `dashboard_readonly_guard` marker 601. **Verification:** lint:errors **0** · unit **2725/2725** (+33) · e2e **14/14** · markers 601 (`?v=601` ×4 + `data-app-build="601"` + `cache-v601` + `SW_BUILD 601` + version 5.69.69) · EOL LF ครบ. **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **⚠️ known residual:** งาน NO_JV **6 งาน / 6,950 บาท** ยังไม่ซ่อม (recovery = **S4.1b/c** แยกเฟส); `verify:reconcile` ยัง expected NO_JV 6 งาน. **⚠️ behavior change (owner รับทราบแล้ว):** ฟอร์มรับงานปิดงานไม่ได้อีกเลยทุก role — งานที่จบหน้างานให้เลือก "📨 รออนุมัติ" แล้วแอดมินปิดจาก drawer "ใบรับงาน" (ที่นั่น JV + ตัดสต็อก + วันปิดงานครบ). **🔜 STOP รอ owner/Codex audit — owner smoke (preview, ห้าม save งานจริงบน prod):** เปิดหน้าใบงานช่าง/ติดตั้งแอร์/โซล่า → dropdown ต้อง **ไม่มี "✅ เสร็จแล้ว"** เหลือ 3 ตัวเลือก + เห็นข้อความ "แอดมินปิดงานจาก drawer ใบรับงาน"; เครื่องที่มี draft ค้าง status=done → เปิดฟอร์มต้องเด้งเป็น "📨 รออนุมัติ" (ไม่ใช่ช่องว่าง); ช่าง (non-admin) เลือก "รออนุมัติ" → บันทึกได้ ไม่ติด 42501.**

**Phase 600 (เก็บเล็ก batch 7 nits · verify ทุก anchor บน build 599 · PR #184 `deddcdd` MERGED+DEPLOYED 2026-07-10 build 600):** **ทุกจุดเล็ก (1-10 บรรทัด) ไม่มี structural change — audit ทุก anchor กับโค้ดจริงก่อนแก้:** **(1) นาทีสายเพี้ยน (`time_clock.js` summarizePunctuality):** เดิม `acc.totalLateMinutes += Number(p.lateMinutes)||0` + `totalEarlyLeaveMinutes` บวก**ดิบทุก record** (รวม `missing_clock_out`/`on_time` ที่มี lateMinutes>0) → count "มาสาย" กับนาทีขัดกัน ("มาสาย 0 (รวม 750 นาที)"). แก้: บวก `totalLateMinutes` เฉพาะ `p.status==="late"||"late_and_early_leave"`; `totalEarlyLeaveMinutes` เฉพาะ `"early_leave"||"late_and_early_leave"`. **(2) sidebar logout ไม่มี confirm (`main.js:5066`):** `$("logoutBtn")?.addEventListener("click", logout)` → `() => { setTimeout(async ()=>{ if(await window.App.confirm("ออกจากระบบ?")) logout(); },0); }` (pattern 596 defer กัน tap-leak; profile-menu logout มี confirm อยู่แล้ว sidebar ยังตรง). **(3) "กำลังเข้าสู่ระบบ..." ค้างหลัง logout:** `auth_email.js:106` `setText("authStatus","กำลังเข้าสู่ระบบ...")`; `logout()` ล้าง otp/login status แต่ไม่ล้าง `authStatus`. แก้: +`if ($("authStatus")) $("authStatus").textContent=""` ในบล็อก reset UI. **(4) statusLabel ไม่ escape (`customer_dashboard.js:615`):** `${statusLabel}` → `${escHtml(statusLabel)}` (escHtml local :88; defense-in-depth — fallback `j.status` ดิบ กันอนาคต). **(5) backfill lock ค้าง (`accounting/backfill.js` `_onRun`):** `_running=true` (:288) แล้วล้างที่ early-exit (:315)+จบปกติ (:408) — throw กลางทางนอก catch → `_running` ค้าง true = ปุ่ม backfill ตายจน reload. แก้: ครอบ body หลัง `_running=true` ด้วย `try { … } finally { _running=false; restore btnRun }` (early-exit `return` เดิม → finally รันเสมอ; **ไม่แตะตรรกะ backfill/JV/re-indent body** — diff เล็กสุดตามสั่ง). **(6) ESC listener ค้าง (`product_detail_modal.js:203`):** `document.addEventListener("keydown", escClose)` remove เฉพาะปิดด้วย ESC → ปิดด้วยปุ่ม/backdrop = ค้างสะสม. แก้: `const close = () => { overlay.remove(); document.removeEventListener("keydown", escClose); }` + `function escClose(e){ if(ESC) close(); }` (named fn hoisted → ทุกทางปิดล้าง listener). **(7) label ข้อมูลไม่ครบ (5 หน้า, UI บรรทัดเดียว/หน้า):** `sales`/`quotations`/`delivery_invoices` (+`~50`) · `warranty_report` (+`~500`) · `serials` (+`~200`) — `<div class="muted" style="font-size:11px">แสดงจากรายการล่าสุด ~N รายการ — ค้นหา/รายงานดูข้อมูลเต็มระบบ</div>` ใต้หัวรายการ. **label เท่านั้น ไม่แตะ fetch/cap logic.** **★ ไม่แตะ:** checkout/money/JV logic · fetch caps (label เท่านั้น) · logout flow 595/596 · RLS/SQL. **Tests:** +`small_batch_600_guard`(7: source-regex ต่อจุด a-g). ปรับ `time_clock.test.js` summarizePunctuality: `totalLateMinutes` 95→**65** (25+40; `missing_clock_out` row มา 8:30 status≠late → ไม่นับนาทีสาย = พฤติกรรมใหม่ถูกต้อง; early 120 คงเดิม) — test เก่าเข้ารหัสพฤติกรรม buggy. ปรับ `sw_timeclock_precache_guard`(599) regex `SW_BUILD` เป็น anchor `^…/m` (กันจับเลขจากคอมเมนต์ header v599 ที่พูดถึง SW_BUILD). companion `dashboard_readonly_guard` marker 600. **Verification:** lint:errors 0 (135 warn baseline) · unit **2666/2666** (+7 −1 net) · e2e **14/14** (markers 600 เขียว; รอบนี้ smoke ผ่านครบ) · markers 600 (?v=600 ×4 + cache-v600 + SW_BUILD 600 + version 5.69.68). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#184 `deddcdd` — CI Tests run 29130922849 (verify: lint/unit 2666/playwright/E2E smoke success) + Deploy run 29130922902 (Cloudflare + Docker success) เขียวบน sha เดียวกัน). owner smoke: หน้าลงเวลา — "มาสาย X (รวม Y นาที)" สอดคล้อง (มาสาย 0 = ไม่มีนาทีรวมมั่ว); ปุ่ม logout ใน sidebar → มี confirm; หน้า login หลัง logout ไม่ค้าง "กำลังเข้าสู่ระบบ..."; หน้า sales/quotations/ฯลฯ เห็น label "แสดงจากรายการล่าสุด ~N".**

**Phase 599 (precache หน้าลงเวลาให้เปิด offline ได้เสมอ — ปิดช่อง 503 · follow-up 598 · PR #182 `c6df1f5` MERGED+DEPLOYED 2026-07-10 build 599):** **ปัญหา (owner ทดสอบจริง build 598):** airplane mode + เปิดหน้าลงเวลา → **HTTP 503 "โหลดหน้าลงเวลาไม่สำเร็จ"**. 598 แก้ให้ drain คิว sync ทำงาน แต่ตัว **หน้า/โมดูล** ยังเปิด offline ไม่ได้. **ต้นตอ (audit `sw.js` :402 modules fetch):** `modules/*` = network-first (`fetch(request,{cache:'reload'})`) + runtime-cache **เฉพาะที่เคยโหลด 200** + `install` ล้าง cache เก่าทุก build → หลัง build ใหม่ (ยังไม่เคยเปิดหน้านั้น online) โมดูลไม่อยู่ใน cache → offline = 503. **audit import chain (ยืนยันเอง):** `time_clock.js` import แค่ 3 relative: `ui_states.js` / `utils.js` / `_offline_queue.js` (`from` grep); ทั้ง 3 เป็น **leaf** (utils/​_offline_queue = 0 relative import; ui_states = 0 จริง — ที่ grep เจอเป็น JSDoc comment ตัวอย่าง). → precache 4 ไฟล์นี้ = ครอบ closure ทั้งหน้าลงเวลา. **แก้ (`sw.js` — 3 จุด):** (1) `+const SW_BUILD = '599'` ติดกับ `CACHE_NAME` (bump พร้อมกันทุก build); (2) เพิ่มเข้า `PRECACHE_URLS`: `'./modules/time_clock.js?v=' + SW_BUILD` + `ui_states` + `utils` + `_offline_queue` (`?v=SW_BUILD` ให้ตรง URL ที่ `_lazyImport`/`_bustedUrl` ยิง; `cache.add` เดิมใน `Promise.allSettled` = ไฟล์พลาดไม่ล้ม install); (3) เส้น modules `.catch(...)` (:414): `caches.match(request)` → ไม่เจอ → `caches.match(request, { ignoreSearch: true })` → ไม่เจอ → 503. **★ ทำไมต้อง ignoreSearch:** dependency (ui_states/utils/_offline_queue) ถูก import **ภายใน** time_clock.js แบบ static `from "./ui_states.js"` (ไม่มี `?v=`) → browser ยิง request ไม่มี query → exact `caches.match` กับ precache `?v=599` ไม่ match → `ignoreSearch:true` ถึงจับได้ (cache ล้างทุก build เหลือรุ่นเดียว = stale ต่ำ). **★ ไม่แตะ:** fetch strategy อื่น (network-first ของ modules คงเดิม — precache แค่ "เสริม" ให้มี fallback) · PRECACHE core เดิม (10 ไฟล์) · offline queue logic (598) · RLS/SQL. `CACHE_NAME` คงเป็น literal string (ไม่ประกอบจาก SW_BUILD) เพื่อไม่ให้ `dashboard_readonly_guard` regex พัง. **Tests:** +`sw_timeclock_precache_guard`(3: (a) `SW_BUILD` = `CACHE_NAME` number = `data-app-build` (index.html) — **ตรงกัน 3 จุด** กันลืม bump แล้ว precache ชี้ผิดเวอร์ชัน; (b) `PRECACHE_URLS` มี 4 โมดูล + `?v=' + SW_BUILD` ครบ; (c) เส้น modules fallback มี `caches.match(request,{ignoreSearch:true})` และอยู่ **ก่อน** fallback 503). `dashboard_readonly_guard` marker 599. **Verification:** lint:errors 0 (135 warn baseline) · unit **2659/2659** (+3) · e2e: smoke APP_BUILD flake เดิม container (markers 599 เขียว) · markers 599 (?v=599 ×4 + cache-v599 + SW_BUILD 599 + version 5.69.67). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#182 `c6df1f5` — CI Tests run 29099409034 (verify: lint/unit 2659/playwright/E2E smoke success) + Deploy run 29099408984 (Cloudflare + Docker success) เขียวบน sha เดียวกัน). owner smoke: force update เป็น 599 (ปุ่มอัปเดต/บังคับอัปเดต) → **รอ ~10 วิ ให้ sw install precache** → เปิด airplane mode → เข้าหน้าลงเวลา → **ต้องเปิดได้ (ไม่ 503)** → กดลงเวลา (ขึ้น queued) → ปิด airplane → toast "📥 Sync ลงเวลา". (regression 598: badge/online-drain ยังทำงาน).**

**Phase 598 (offline queue ลงเวลาไม่หายเงียบ [payroll] + PWA update UX · PR #181 `1cb3148` MERGED+DEPLOYED 2026-07-10 build 598):** **ปัญหา (A):** ลงเวลาตอน offline ถูกคิวใน IndexedDB (`OfflineQueue`) แต่ **ไม่เคย drain อัตโนมัติเมื่อ online กลับ** — audit ยืนยัน: ทั้ง repo ไม่มี `addEventListener("online")` เลย (grep ว่าง); drain เกิดเฉพาะตอนเปิดหน้า time clock (`time_clock.js` renderTimeClockPage sync-on-render); ตรงนั้นโชว์เฉพาะ `const {ok}=` (fail เงียบ); self view (non-admin) ไม่มี badge ของค้าง → ลงเวลา offline **หายจาก payroll เงียบ ๆ** ถ้าพนักงานไม่บังเอิญเปิดหน้า time clock ตอนมีเน็ต. **ปัญหา (B):** `boot.js` ปุ่ม "อัปเดตเลย" (swUpdateApply) `if (reg && reg.waiting) postMessage(SKIP_WAITING)` — **ไม่มี reg.waiting → no-op เงียบ**; ไม่มี fallback ถ้า controllerchange ไม่มา. **แก้ (A1 `main.js`):** +`async _drainTimeClockQueue(trigger)` [guard `window._tcSyncing` (ใช้ร่วม sync-on-render); `const m = await _lazyImport("./modules/time_clock.js")` (cache-bust helper เดิม — ไม่ eager import กัน bundle โต); `pending=offlinePendingCount()`; ถ้า>0 → `const {ok,fail}=await m.syncOfflineQueue()`; ok>0 → toast ✅, fail>0 → toast ⚠️ warn] + `window.addEventListener("online", ()=>_drainTimeClockQueue("online"))` (module-level, หลัง `showToast`) + เรียก `_drainTimeClockQueue("boot")` ใน background setTimeout ท้าย `afterLogin` (guard `navigator.onLine!==false`). **(A2 `time_clock.js` sync-on-render):** `const {ok}=` → `const {ok,fail}=await syncOfflineQueue()`; +`fail>0 → showToast?.(⚠️…,"warn")`. **(A3 `time_clock.js` self view `_renderSelfView`):** +`const pendingCount = await offlinePendingCount()`; ถ้า>0 → การ์ด badge "⏳ ลงเวลาค้าง sync N รายการ" + ปุ่ม `tcSyncSelfBtn` (แทรกหลัง profile card, ไม่รื้อ layout เดิม); extract `_bindOfflineSyncBtn(btnId, ctx)` (ยก handler admin เดิมมาไม่เปลี่ยน logic: sync→toast ✅/❌→re-render) → admin เรียก `_bindOfflineSyncBtn("tcSyncOfflineBtn",ctx)`, self เรียก `_bindOfflineSyncBtn("tcSyncSelfBtn",ctx)`. **แก้ (B `boot.js` — คง non-module style var/function):** extract `function forceReload()` (cache-bust `location.replace` เดิม) ใช้ร่วม; `controllerchange` → `forceReload`; `swUpdateApply` click → disable+`textContent='กำลังอัปเดต...'`, มี `reg.waiting`: `postMessage(SKIP_WAITING)` + `setTimeout(forceReload,4000)` (fallback controllerchange ไม่มา), ไม่มี `reg.waiting`: `forceReload()` ทันที (แทน no-op); `visibilitychange` +`if (reg.waiting && controller) maybeShowBanner(reg)` (เด้ง banner ซ้ำถ้า waiting ค้าง/isReallyNewBuild เคย fail; `maybeShowBanner` มี dup-guard; **ไม่ reload ใน startPeriodicUpdate** = boot guard :49 ผ่าน). **★ ไม่แตะ:** `syncOfflineQueue`/`OfflineQueue` internals (idempotency 409 ถูกแล้ว) · clock-in/out flow · payroll · `sw.js` cache strategy (แค่ bump) · RLS. **Tests:** +`offline_queue_drain_guard`(4: (a) main.js `addEventListener("online")→_drainTimeClockQueue` + `_lazyImport` time_clock + **ไม่ static-import** time_clock + เรียก `"boot"`; (b) `_drainTimeClockQueue` `{ok,fail}` + ok>0 toast + fail>0 toast warn + `_tcSyncing`; (c) time_clock sync-on-render `{ok,fail}`+warn + self badge `tcSyncSelfBtn` + `_bindOfflineSyncBtn` admin+self; (d) boot swUpdateApply มี `SKIP_WAITING`+`setTimeout(forceReload)`+else `forceReload()` + extract `forceReload` + controllerchange ใช้ร่วม + visibilitychange re-check waiting). `boot_periodic_sw_update`(6)/`lazy_import_cache_bust` เขียวครบ (visibilitychange +maybeShowBanner ไม่มี reload → ไม่ชน). companion `dashboard_readonly_guard` marker 598. **Verification:** lint:errors 0 (135 warn baseline) · unit **2656/2656** (+4) · e2e: smoke APP_BUILD flake เดิม container (markers 598 เขียว) · markers 598 (?v=598 ×4 + cache-v598 + version 5.69.66). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#181 `1cb3148` — CI Tests run 29095074128 + Deploy เขียว). owner smoke: (A) เปิด airplane mode → ลงเวลา (ขึ้น queued) → ปิด airplane → **ไม่ต้องเปิดหน้า time clock** → ควรมี toast "📥 Sync ลงเวลา ✅" เอง (ภายใน ~ตอน online event); เปิดหน้า time clock (self) ตอนยังมีค้าง → เห็น badge "⏳ ลงเวลาค้าง sync N". (B) ปุ่ม "อัปเดตเลย" บน banner → กดแล้ว **รีโหลดเสมอ** (ไม่เงียบ).**

**Phase 597 (profit_report รวมรายได้งานบริการ — นิยามเดียวกับ dashboard/income_overview · single source · PR #179 `190500f` MERGED+DEPLOYED 2026-07-10 build 597):** **ปัญหา:** หน้ารายงานกำไร (`modules/profit_report.js`) นับรายได้จากตาราง `sales` (POS) เท่านั้น (`totalRevenue` = sum `total_amount`) แต่รายจ่าย (`totalExpenses`) รวมของงานช่างด้วย (เช่นค่าแอร์ 49,200) → `netProfit = grossProfit − totalExpenses` **ติดลบเกินจริง**; และนิยาม "รายได้" ไม่ตรง income_overview/dashboard ที่รวมงานบริการแล้ว. **audit ก่อนทำ (ยืนยันกับโค้ดจริง):** `fetchServiceJobsSince` มีใน `range_fetch.js:60` (คืน `{ok,rows,error}` ผ่าน `_paginate`); `sumServiceJobIncome`(dashboard.js:36 = sum `total_cost` ของ job ที่ `isServiceIncomeJob` [delivered/done/closed + total_cost>0 + ไม่ลบ] และ `!isWebOrderServiceJob` และ dateMatch) + `serviceIncomeDate`(dashboard.js:34 = `closed_at||created_at`) export จริง; income_overview.js:16/78/106 ใช้ pattern นี้; `state.serviceJobs` มี (main.js:283). **แก้ (`profit_report.js`):** (1) import `fetchServiceJobsSince` เพิ่มใน range_fetch import + `import { sumServiceJobIncome, serviceIncomeDate } from "./dashboard.js"` (★ single source — ห้ามคิดสูตร service เอง กัน drift); (2) `Promise.all([fetchSalesSince, fetchExpensesSince, fetchServiceJobsSince])` (ดึงขนาน); `_fetchedServiceJobs = _svcRes.ok ? rows : (state.serviceJobs||[])` + `if(!_svcRes.ok) showToast(..."warn")` (fallback+เตือน ไม่เงียบ — pattern เดียวกับ expenses); (3) `const serviceIncome = sumServiceJobIncome(_fetchedServiceJobs, j => { const t=new Date(serviceIncomeDate(j)).getTime(); return t>=fromTs && t<=toTs; })` (ขอบเขต = ชุดเดียวกับ expensesInRange); (4) **export `computeNetProfit(grossProfit, serviceIncome, totalExpenses)` = gross+service−expenses (pure, guard NaN→0)**; `netProfit = computeNetProfit(...)`; (5) `renderSummaryCards(...serviceIncome)`: การ์ด "รายได้" = `money(totalRevenue+serviceIncome)` + muted "ขาย POS ฿X · งานบริการ ฿Y", label "ต้นทุนสินค้า **(POS)**"/"กำไรขั้นต้น **(POS)**" (COGS/กำไรขั้นต้น = เฉพาะ POS — ต้นทุนงานช่างอยู่ในรายจ่ายแล้ว), **+การ์ด "รายได้งานบริการ"** ระหว่างกำไรขั้นต้นกับค่าใช้จ่าย; (6) กราฟ 6 เดือน (`renderMonthlyTrend`) + ตาราง "กำไรจากแต่ละสินค้า" = **POS เดิม ไม่แตะ logic** — เปลี่ยนแค่ h2 กราฟเป็น "…(POS)" (ไม่ scope creep รวม service เข้ากราฟ). **★ ไม่แตะ:** `dashboard.js` (import single source อย่างเดียว) · `income_overview.js` · `range_fetch.js` logic · `renderProfitByProduct`/`renderExpenseBreakdown`/`renderMonthlyTrend` compute · expenses/JV/auto_post/RLS/SQL. **Tests:** +`profit_report_service_income_guard`(3: (a) **unit** `computeNetProfit(1000,49200,60000) = -9800` และ `> computeNetProfit(1000,0,60000)=-59000` (พิสูจน์ service ดันกำไรขึ้น) + guard NaN→0; (b) **source** import sumServiceJobIncome+serviceIncomeDate จาก dashboard.js + fetchServiceJobsSince ใน Promise.all + `serviceIncome=sumServiceJobIncome(` + `computeNetProfit(grossProfit,serviceIncome,totalExpenses)` + ไม่ reimplement `isServiceIncomeJob`; (c) **fallback** `_svcRes.ok?rows:(state.serviceJobs||[])` + `if(!_svcRes.ok) showToast(..."warn")`). ปรับ `profit_expense_fetch_guard` 2 assert (import braces + Promise.all destructure) ให้รับ `fetchServiceJobsSince`/`_svcRes` ที่เพิ่ม (คงเจตนา expenses ยังขนาน/hard-stop sales เดิม). companion `dashboard_readonly_guard` marker 597. **Verification:** lint:errors 0 (135 warn baseline) · unit **2652/2652** (+3 guard, +2 ปรับ) · e2e: smoke APP_BUILD flake เดิม container (build-sync markers 597 เขียว) · markers 597 (?v=597 ×4 + cache-v597 + version 5.69.65). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#179 `190500f` — CI Tests run 29092635834 (verify: lint/unit 2652/playwright/E2E smoke success) + Deploy run 29092635819 (Cloudflare deploy job + Docker success) เขียวบน sha เดียวกัน). owner smoke: เปิดรายงานกำไร ช่วง 01–10/07 → การ์ด "รายได้" ต้องรวมงานบริการ (เห็นหลักหมื่น ไม่ใช่ ~1,400) + บรรทัดย่อย "ขาย POS · งานบริการ" + การ์ด "รายได้งานบริการ" + กำไรสุทธิสมเหตุผลขึ้น (ไม่ติดลบเกินจริง); เทียบยอด "งานบริการ" ให้ตรงหน้าภาพรวมรายได้ (income_overview) ช่วงเดียวกัน.**

**Phase 596 (logout confirm มือถือ — logout ทันทีไม่ขึ้น dialog · hotfix ต่อ 595 · PR #178 `1ede894` MERGED+DEPLOYED 2026-07-10 build 596):** **อาการ (owner รายงาน + screenshot build 595):** กด 👤 profile menu → "ออกจากระบบ" บนมือถือ → **logout ทันที กลับหน้า login โดยไม่ขึ้น dialog confirm ให้กด "ยืนยัน"** (ปุ่มไม่ตายแล้ว = logout ทำงาน แต่ confirm ถูกข้าม). **ต้นเหตุ:** `App.confirm` = `confirmAsync` → `showConfirmModal` สร้าง `.confirm-overlay` (z-index 10000, full-screen) + `okBtn.focus()`; resolve(true) เฉพาะเมื่อคลิกปุ่ม "ยืนยัน". handler `profileMenu` click เรียก `closeProfileMenu()` (ซ่อน `<button>` ที่ถูกแตะ) **แล้วเปิด modal ใน gesture เดียวกัน** → บนมือถือ (viewport `user-scalable=no` + accessibility service active เห็นจาก nav bar) tap เดิม leak/replay ไปกดปุ่ม "ยืนยัน" ของ modal ที่เพิ่งสร้าง = confirm ผ่านทันทีโดย user ไม่เห็น. (ต่างจาก App.confirm ที่อื่น :1876/:2925 ที่ทำงานปกติ — จุดนั้นปุ่ม trigger ไม่ถูกซ่อนกลางคัน). **แก้ (`main.js` handler `profileMenu` — จุดเดียว):** เปลี่ยน listener จาก `async (e)=>` กลับเป็น sync `(e)=>`; branch logout ห่อ confirm ด้วย **`setTimeout(async () => { if (await window.App.confirm("ออกจากระบบ?")) logout(); }, 0)`** — เลื่อนสร้าง modal ไป macrotask ถัดไป (หลัง click/tap gesture ปัจจุบันจบ) → modal ไม่มีอยู่ตอน tap เดิมประมวลผล → ไม่มี event ทะลุ. `closeProfileMenu()` ยังทำงานทันที (ปิดเมนูตามเดิม). **★ ไม่แตะ:** `logout()` body (ล้าง PII `state.lastReceipt=null`+`removeItem` ของ 595 คงเดิม) · `logoutBtn`(:5042) · `showConfirmModal`/`confirmAsync`/`App.confirm` (shared — ไม่แก้เพื่อไม่กระทบ confirm อื่นทั้งแอป) · `auth.js` · checkout/money/RLS. **Tests:** ปรับ `logout_hygiene_guard` test(a) — คง assert (ไม่มี `__authLogout` + `App.confirm("ออกจากระบบ?")` + `logout()`) + **เพิ่ม assert `setTimeout(async…App.confirm` (lock การ defer)**; widen slice 700→900. `topbar_notif_profile` test(3) — เปลี่ยน regex เป็น robust (แยก assert `!/window\.__authLogout/` + `/App\.confirm\("ออกจากระบบ\?"\)\)\s*logout\(\)/` แทน ordered-match ที่โดนคำว่า App.confirm/logout() ในคอมเมนต์รบกวน); widen menu slice 600→950. guard เดิมเขียวครบ. companion `dashboard_readonly_guard` marker 596. **Verification:** lint:errors 0 (135 warn baseline) · unit **2649/2649** · e2e **14/14** (smoke APP_BUILD ผ่านครบ; build-sync markers 596 เขียว) · markers 596 (?v=596 ×4 + cache-v596 + version 5.69.64). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#178 `1ede894` — CI Tests + Deploy เขียว). owner smoke (build 596): กด 👤 → "ออกจากระบบ" บนมือถือ → **ต้องขึ้น dialog "ออกจากระบบ?" + ปุ่ม ยืนยัน/ยกเลิก ให้กดก่อน** → กดยืนยัน = ออก / กดยกเลิก = อยู่ต่อ. (regression) PII บิลล่าสุดยังถูกล้างตอน logout (จาก 595).**

**Phase 595 (logout hygiene — ปุ่ม logout topbar ตาย + PII บิลล่าสุดค้างข้าม login · PR #176 `90d184e` MERGED+DEPLOYED 2026-07-10 build 595):** **บั๊ก 2 จุด (audit ตรงกับโค้ดจริง):** (1) ปุ่ม "ออกจากระบบ" ใน profile menu บน topbar (`main.js:5065`) เรียก `window.__authLogout?.()` — ตัวนี้ assign ใน `initAuth()` ของ `modules/auth.js` แต่ **`initAuth()` ไม่เคยถูกเรียกทั้ง codebase** (grep ยืนยัน: มีแต่ definition ที่ auth.js:376; auth.js ถูก import แค่ใน staff.js เอา `isAdmin`/`showAccessDenied` — ไม่แตะ initAuth) → `__authLogout` = `undefined` เสมอ → **ปุ่มตาย กดแล้วเงียบ**. (`logoutBtn` :5042 = `logout` โดยตรง ใช้ได้อยู่แล้ว — คนละปุ่ม). (2) `logout()` (`main.js:1111`) ล้าง state เยอะ (currentUser/profile/cart/pos/customer_dashboard) แต่ **ไม่ล้าง `state.lastReceipt` + `localStorage "bsk_last_receipt"`** → PII ลูกค้า (ชื่อ/เบอร์ บนบิลล่าสุด) ค้างข้าม account (login คนใหม่เปิดบิลล่าสุดเห็นของเก่า). **แก้ (`main.js` 2 จุด):** (1) handler `profileMenu` click — เปลี่ยน listener เป็น `async (e)=>`, branch `act==="logout"` → `if (await window.App.confirm("ออกจากระบบ?")) logout();` (เลิกพึ่ง `__authLogout`; ใช้ `logout()` หลัก + confirm pattern เดิม `App.confirm` ที่มีใน main.js:1873/2922). (2) ใน `logout()` ก่อน `showToast("ออกจากระบบแล้ว")` เพิ่ม `state.lastReceipt = null;` + `try{ localStorage.removeItem("bsk_last_receipt") }catch(_){}`. **★ ไม่แตะ:** logout flow เดิมทั้งหมด (signOut/clear currentUser·profile·cart·pos·custDash/force UI reset/reset OTP·login form) · `logoutBtn`(:5042) · `modules/auth.js` (ไม่ลบไม่แก้เฟสนี้) · checkout/money/RLS. **main.js ไม่ import auth.js** (ยืนยัน guard — กันดึง dead module + `initAuth` side effect `showStaffLogin` modal). **Tests:** +`logout_hygiene_guard`(3: (a) extract profileMenu handler → ไม่มี `window.__authLogout` + มี `App.confirm("ออกจากระบบ?")` + เรียก `logout()`; (b) extract `logout()` body → มี `state.lastReceipt=null` + `removeItem("bsk_last_receipt")` ก่อน `showToast`; (c) main.js ไม่มี `import … from "./modules/auth.js"`). ปรับ `topbar_notif_profile.test.js` test(3) เดิมที่ **lock พฤติกรรมปุ่มตาย** (`assert.match(mainJs, /window\.__authLogout\?\.\(\)/)`) → อัปเป็น: ไม่มี `window.__authLogout` + `act==="logout"` → `App.confirm` → `logout()` (test เก่าเข้ารหัสสมมติฐานผิดว่า reuse `__authLogout` = ถูก; ความจริงมันตาย). guard เดิมเขียวครบ. companion `dashboard_readonly_guard` marker 595. **FUTURE CLEANUP (จดไว้):** `modules/auth.js` = staff-PIN auth system ทั้งชุด **dormant** — `initAuth()` (assign `__authLogout`/`showStaffLogin`) ไม่เคยถูกเรียก; ใช้จริงแค่ `isAdmin`/`showAccessDenied` (import ใน staff.js). พิจารณา **ลบทั้งไฟล์ หรือ รื้อฟื้น staff-login** ในอนาคต (นอก scope เฟสนี้). **Verification:** lint:errors 0 (135 warn baseline) · unit **2649/2649** (+3 logout_hygiene, −1 net จากปรับ topbar test = 2648+... รวม 2649) · e2e 14/14 (รอบนี้ smoke APP_BUILD ผ่านครบไม่ flake; build-sync markers 595 เขียว) · markers 595 (?v=595 ×4 + cache-v595 + version 5.69.63). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#176 `90d184e` — CI Tests run 29062184452 (verify: lint/unit 2649/playwright/E2E smoke ทั้งหมด success) + Deploy run 29062184464 (Cloudflare deploy job success + Docker success) เขียวบน sha เดียวกัน; Deploy success บน commit ฝัง data-app-build=595 = หลักฐาน). owner smoke (หลัง merge+deploy): (1) กด 👤 profile menu (topbar) → "ออกจากระบบ" → **มี confirm → กดตกลง → ออกจริง กลับหน้า login** (เดิมกดแล้วเงียบ). (2) login ใหม่ (account อื่น) → เปิดบิลล่าสุด → **ไม่เห็นบิล/ชื่อลูกค้าของ account เก่า**.**

**build 594 (PeriPage A9 Max — ปิดเคส พิมพ์ผ่านเว็บไม่ได้ + UX guard · PR #174 `57a4d20` MERGED+DEPLOYED 2026-07-09 build 594 · ✅ เคสปิด):** **สรุปเคส:** พิสูจน์ครบ 3 ทางแล้วว่า A9 Max พิมพ์ผ่านเว็บไม่ได้ — (1) **btsnoop log** แอปทางการพิมพ์ผ่าน Bluetooth Classic SPP (CID 0x42) ไบต์พิมพ์ผ่าน BLE=0; (2) **build 592** framing ตัวจริง (preamble/reset/feed) ผ่าน ff02 → owner ทดสอบ กระดาษไม่เลื่อน; (3) **build 593** notify handshake (startNotifications ff01/ff03 + settle) + ff02 → owner ทดสอบ (build 593 live, GATT ยืนยัน ISSC 8841 + ff00/ff01/ff02/ff03) ยังนิ่ง. **ข้อสรุปแน่นอน:** ช่อง BLE (ff02/8841) ของ A9 Max ไม่ได้ต่อเข้า print engine — เครื่องรับงานพิมพ์เฉพาะ Bluetooth Classic (RFCOMM/SPP) ที่ **Web Bluetooth เข้าไม่ถึงเลย** (กำแพง browser ระดับ platform ไม่ใช่บั๊ก). owner เลือก = **ทำ UX guard ให้ชัด (ปิดเคส)**. **แก้ (`receipt_bt.js` — ถอด experimental + guard):** ★ ลบ `A9MAX_PREAMBLE/A9MAX_FEED/A9MAX_FINALIZE` (592) · ลบ notify handshake ทั้งก้อน + state `_notifyChars`/`_notifyStarted` + resets (593) · ลบ ff02 MAX override ใน `connectSlipPrinter` → `const preferUuid = PERIPAGE_WRITE_CHAR` เสมอ (A9 Max connect = fec7→fallback firstWritable = 8841 เหมือน A9; ไม่มี MAX-special ใน connect แล้ว) · ลบ `if (isPeriPageMax())` branch ใน `buildPrintBytes` → กลับ 586 (isPeriPage → PeriPage framing; else generic). เพิ่ม export **`A9MAX_UNSUPPORTED_MSG`** ("รุ่น A9 Max พิมพ์ผ่านเว็บ (Bluetooth) ไม่ได้ … ใช้แอป PeriPage หรือเครื่อง A9"); `printReceipt` + `testPrint` หลัง `connectSlipPrinter()` → **`if (isPeriPageMax()) throw new Error(A9MAX_UNSUPPORTED_MSG)`** (บล็อกก่อน `renderTestCanvas`/`buildPrintBytes` → แจ้งชัดแทนกดแล้วเงียบ; ครอบทั้ง 3 จุดพิมพ์: settings test / main.js post-sale / receipts reprint). คง `isPeriPageMax()` (ใช้โดย guard) + `getPaperPx()` auto (ยังเช็ค /max/i ชื่อ — 832 สำหรับ MAX ถ้าอนาคตพิมพ์ได้). **`settings/pages.js`:** `btConnectBtn` — ถ้า `isPeriPageMax()` → สถานะ "🟠 เชื่อมแล้ว: <name> — ⚠️ พิมพ์ผ่านเว็บไม่ได้" + toast `A9MAX_UNSUPPORTED_MSG` (เห็นตั้งแต่เชื่อม); else เดิม (🟢 + [char]). `btTestPrintBtn` catch — ถ้า MAX → toast "⚠️ <msg>" (ไม่ใช่ "ทดสอบพิมพ์ไม่สำเร็จ:"). **★ A9 regression:** A9 (ชื่อไม่มี MAX) → `isPeriPageMax()=false` → ไม่โดน throw; connect `preferUuid=fec7`→ไม่เจอ→fallback 8841; `buildPrintBytes`=PeriPage framing (reset 12×00 + concentration 0002 + GS v 0 + ESC J 0x60) = **byte-identical 586**. **Tests:** ลบ `receipt_bt_a9max_mode_guard`(592)/`receipt_bt_notify_handshake_guard`(593)/`receipt_bt_max_guard`(590 — behavior ถอดออกหมดแล้ว); +`receipt_bt_a9max_unsupported_guard`(2: (1) A9 Max → `testPrint`+`printReceipt` reject ด้วย `A9MAX_UNSUPPORTED_MSG` (บล็อกก่อนวาด canvas = ทดสอบ Node ได้); (2) ★ A9 non-MAX → char 8841 + `buildPrintBytes` PeriPage framing 586 + `isPeriPageMax()=false` regression). guard 586 (peripage test(b) ชื่อ PeriPage_A9 / raster / feature_detect / paper_width) เขียวครบ. companion `dashboard_readonly_guard` marker 594. **Verification:** lint:errors 0 (135 warn baseline) · unit **2646/2646** (2650 −2−2−2 ลบ 3 ไฟล์ +2 ไฟล์ใหม่) · e2e: smoke APP_BUILD flake เดิม container (build-sync markers 594 เขียว) · markers 594 (?v=594 ×4 + cache-v594 + version 5.69.62 คง). grep ยืนยันไม่มี dangling ref (isMax/_notify*/A9MAX_*). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#174 `57a4d20` — CI Tests run 29023542760 (verify: lint/unit 2646/playwright/E2E smoke ทั้งหมด success — ไม่มี APP_BUILD flake) + Deploy run 29023542814 (Cloudflare deploy job success + Docker success) เขียวบน sha เดียวกัน; Deploy success บน commit ฝัง data-app-build=594 = หลักฐาน). ★ เคส A9 Max ปิดสมบูรณ์ — ทีม audit ที่ 2 ยืนยันตรงกัน (A9 Max = Bluetooth Classic SPP, Web Bluetooth เข้าไม่ถึง = กำแพง platform); ff02 ทดสอบครบแล้ว (590/592/593) ไม่มี option ค้าง. owner ยอมรับปิดเคส. owner test (Android, build 594): A9 Max → ตัดการเชื่อม → เชื่อมใหม่ → **ต้องเห็นสถานะ 🟠 + ⚠️ พิมพ์ผ่านเว็บไม่ได้** + toast แนะนำใช้แอป PeriPage (กดทดสอบพิมพ์ = ขึ้นข้อความชัด ไม่เงียบ). (regression) A9 → เชื่อม → ทดสอบพิมพ์ → **ต้องออกเหมือนเดิม**.**

**build 593 (PeriPage A9 Max — notify handshake · long-shot #2 ผ่าน BLE · PR #172 `cf210af` MERGED+DEPLOYED 2026-07-09 build 593):** **บริบท:** build 592 ยิง framing ตัวจริง (preamble/reset/feed `1B4A50`) ผ่าน BLE ff02 แล้ว owner ทดสอบ A9 Max = **กระดาษไม่เลื่อน (นิ่ง)** + รัน diagnostic ยืนยัน GATT: service `49535343` (ISSC — char `8841`[write], `1e4d`[notify]) + service `0000ff00` (char `ff01`[notify], `ff02`[write], `ff03`[notify]) — ตรงกับที่แกะจาก snoop log เป๊ะ. owner เลือกทิศทาง = **"ลองอีก 1 ครั้ง: notify handshake"**. **สมมติฐาน:** เครื่องช่อง ff00/ff02 บางรุ่นต้อง **enable CCCD ของ notify char (`ff01`/`ff03`) ก่อน** firmware ถึงจะประมวลผล write ที่ ff02 (pattern เดียวกับ ISSC UART ที่ 587 เคยลองบน `1e4d`/8841 — แต่ **ไม่เคยลองบนช่อง ff02**; ตั้งแต่ 590 A9 Max เลือก ff02 แต่ไม่ subscribe notify). **แก้ (`receipt_bt.js` — เฉพาะ A9 Max):** +state `_notifyChars=[]`/`_notifyStarted=false`; ใน `connectSlipPrinter` หลัง set `_char`/`_serviceUuid`/`_charUuid` → **`if (isMax && pickedSvc && !_notifyStarted)`**: `pickedSvc.getCharacteristics()` (= ff00) หา `c.properties?.notify && startNotifications` → `await c.startNotifications()` (ff01+ff03) + `addEventListener("characteristicvaluechanged", ()=>{})` (กัน CCCD หลุดบาง stack) + push `_notifyChars`; ถ้า subscribe ได้ → settle 120ms. reset ที่ `disconnectSlip` + `gattserverdisconnected`. **★ ล็อกเฉพาะ isMax:** A9 (ชื่อไม่มี MAX) ไม่เข้า block → **connect byte-identical 592/586** (ไม่ subscribe อะไร). **ไม่แตะ:** char selection · `buildPrintBytes` (A9MAX framing 592 คงเดิม) · `sendChunked` · raster · money flow · RLS/SQL. **Tests:** `receipt_bt_notify_handshake_guard`(2: (1) A9 Max → char ff02 + `startNotifications` บน **ff01+ff03** (closure counter) + **ไม่แตะ ISSC 1e4d**; (2) ★ A9 non-MAX → char 8841 + `startNotifications` **ไม่ถูกเรียกเลย** calls.length===0 regression). guard 592 เขียวครบ. companion `dashboard_readonly_guard` marker 593. **known-risk:** long-shot #2 — CCCD-gate เป็นการเดา (แอปทางการใช้ SPP ไม่มี BLE handshake ให้ก็อป); ถ้ายังนิ่ง = **ยืนยันปิดเคส A9 Max ผ่านเว็บ** (ใช้แอป PeriPage/SPP). A9 (regression) ต้องยังพิมพ์ได้. **Verification:** lint:errors 0 (135 warn baseline) · unit **2650/2650** (+2) · e2e: smoke APP_BUILD flake เดิม container (markers 593 เขียว) · markers 593 (?v=593 ×4 + cache-v593 + version 5.69.62 คง). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#172 `cf210af` — CI Tests run 29021610788 (verify: lint/unit 2650/playwright/E2E smoke ทั้งหมด success — ไม่มี APP_BUILD flake) + Deploy run 29021610675 (Cloudflare deploy job success + Docker success) เขียวบน sha เดียวกัน; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev CONNECT 403 — Deploy success บน commit ฝัง data-app-build=593 = หลักฐาน; owner hard-refresh ยืนยัน 593). owner test (Android, build 593): A9 Max → ตัดการเชื่อม → เชื่อมใหม่ (`[char …ff02…]`) → "ทดสอบพิมพ์" → **กระดาษเลื่อนไหม** (เลื่อน = notify handshake ปลุกช่องได้ → phase หน้าเขียน G4 encoder; นิ่งเหมือนเดิม = ปิดเคส A9 Max ผ่านเว็บ ใช้แอปทางการ). (regression) A9 → ต้องออกเหมือนเดิม.**

**build 592 (PeriPage A9 Max — framing ตัวจริงจากแอปทางการ · long-shot ผ่าน BLE ff02 · PR #170 `faf780f` MERGED+DEPLOYED 2026-07-09 build 592):** **บริบท:** A9 Max พิมพ์ไม่ออกมาตลอด (build 587–591 เดาช่อง/โปรโตคอลล้มเหลวทั้งหมด). owner ยืนยัน **แอป PeriPage ทางการพิมพ์ A9 Max ออกจริง** (ฮาร์ดแวร์โอเค) → เลิกเดา, capture ไบต์จริง. **การแกะ `btsnoop_hci.log` (Android HCI snoop log ของแอปทางการขณะพิมพ์ A9 Max — parser reassemble ACL→L2CAP→RFCOMM ใน scratchpad):** **★ ค้นพบต้นเหตุที่แท้จริง: แอปพิมพ์ผ่าน Bluetooth Classic (RFCOMM/SPP, L2CAP CID 0x42) ไม่ใช่ BLE** — data รูปทั้งหมด (3 sessions: 1364/3942/3821B) วิ่งผ่าน RFCOMM; ฝั่ง BLE GATT มี ATT write แค่ ~2KB = base64 EC-key + gzip (telemetry คนละอุปกรณ์) **ไม่มีไบต์รูปพิมพ์ผ่าน BLE เลย**. **Web Bluetooth (PWA เราใช้) เปิด socket Classic/SPP ไม่ได้** = ข้อจำกัด browser ระดับ platform = **ต้นเหตุที่ 587–591 ล้มเหลว** (A9 Max ไม่ได้ต่างเครื่องจาก A9 ใน GATT — ต่างที่ firmware เลือกพิมพ์ผ่านช่องที่เว็บแตะไม่ถึง). **โปรโตคอลจริง (ถอดไบต์):** preamble `10FF20F0`·`10FF70`·`10FF12 0014`+12×00 · density `10FF10 0004` (ไม่ใช่ 0002) · reset `10FFFE01 1FB210` (ไม่ใช่ 12×00) · image ผ่านคำสั่ง `10FF80 01` + header `1F 0000 67 HHHH 0000` (**width=0x67=103B=824dot**, height=16-bit BE) + **ภาพบีบอัด proprietary** (entropy~7.7, ratio 20–46×; ลอง gunzip/inflate/inflateRaw fail หมด — คล้าย CCITT G4; **reproduce ใน vanilla JS ไม่ได้ตอนนี้**) · feed `1B4A50` · finalize `10FFFE45`+12×00. **★ NO `GS v 0`** (1D7630 = 0 hits) — A9 Max ไม่ใช้ raster แบบ A9. **owner เลือกทิศทาง = "long-shot ผ่าน BLE".** **แก้ (`receipt_bt.js`):** เพิ่ม const `A9MAX_PREAMBLE`(36B ไบต์จริง)/`A9MAX_FEED`(`1B4A50`)/`A9MAX_FINALIZE`(`10FFFE45`+12×00); export **`isPeriPageMax()`** = `/max/i.test(_device?.name)`; **`buildPrintBytes` เช็ค `isPeriPageMax()` ก่อน `isPeriPage()`** → A9 Max = `concatBytes(A9MAX_PREAMBLE, canvasToRasterBody(canvas)[GS v 0 best-effort], A9MAX_FEED, A9MAX_FINALIZE)`; ยิงผ่าน char **ff02** (override 590 เดิม). **เหตุใช้ GS v 0 หุ้ม framing จริง:** ยัง reproduce การบีบอัด `10FF80` ไม่ได้ → **feed `1B4A50` เป็นตัวชี้วัด**: A9 Max **เลื่อนกระดาษ = ช่อง BLE ถึง print engine จริง** → คุ้มลงทุนถอด G4 encoder ต่อ (phase หน้า); **ไม่ขยับเลย = ช่อง BLE ไม่ใช่ช่องพิมพ์** → A9 Max พิมพ์จากเว็บไม่ได้ (ยืนยันปิดเคส). **★ ล็อกเฉพาะชื่อมี "MAX":** A9 (ชื่อไม่มี MAX) → `isPeriPageMax()=false` → `isPeriPage()` path เดิม = **`buildPrintBytes` byte-identical 586** (reset 12×00 + concentration 0002 + GS v 0 + ESC J 0x60). **ไม่แตะ:** connect char selection (fec7/ff02/MAX override 590) · `sendChunked` · `canvasToRasterBody`/raster · money flow · `window.print()` · `bt_printer.js` · RLS/SQL. **Tests:** `receipt_bt_a9max_mode_guard`(2: (a) A9 Max → buildPrintBytes ขึ้นต้น `10FF20F0` preamble + density `10FF10 0004` + reset `10FFFE01 1FB210` + GS v 0 + feed `1B4A50` + finalize `10FFFE45`+12×00 + **ไม่มี GS V cut**; (b) **★ A9 regression** = PeriPage framing 586 เป๊ะ: reset 12×00 + concentration **0002** + ESC J **0x60** + ไม่มี A9MAX preamble/finalize). ปรับ `receipt_bt_max_guard` test(2) A9 Max → คาด A9MAX framing (แทน PeriPage เดิม); ปรับ `receipt_bt_peripage_guard` test(b) ชื่อ "PeriPage_A9MAX"→"PeriPage_A9" (MAX มี path ของตัวเองแล้ว). companion `dashboard_readonly_guard` marker 592. **known-risk / LIMITATION:** long-shot 2 ตัวแปรไม่แน่นอน — (1) ช่อง BLE ff02 ของ A9 Max อาจไม่ใช่ช่องพิมพ์เลย; (2) `10FF80`+G4 บีบอัด reproduce ไม่ได้ → ใช้ GS v 0 หุ้ม (แอปจริงไม่ใช้ GS v 0). ค่าที่ได้ = **binary signal จาก feed** (กระดาษเลื่อน/ไม่เลื่อน). **Verification:** lint:errors 0 (135 warn baseline) · unit **2648/2648** (+2) · e2e: smoke APP_BUILD flake เดิม container (build-sync markers 592 เขียว = markers ถูก) · markers 592 (?v=592 ×4 + cache-v592 + version 5.69.62 — คง). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#170 `faf780f` — CI Tests run 29020172862 (verify: lint/unit 2648/playwright/E2E smoke success — รอบนี้ไม่มี APP_BUILD flake) + Deploy run 29020173190 (Cloudflare deploy job success) เขียวบน sha เดียวกัน; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev (CONNECT 403) — Deploy success บน commit ที่ฝัง data-app-build=592 = หลักฐาน; owner hard-refresh ยืนยัน 592). owner test (Android): A9 Max → ตัดการเชื่อม → เชื่อมใหม่ (สถานะ `[char …ff02…]`) → กด "ทดสอบพิมพ์" → **สังเกต: กระดาษเลื่อนไหม** (เลื่อน = ช่อง BLE ใช้ได้ → phase หน้าเขียน G4 encoder; ไม่เลื่อน/ไม่มีอะไรเกิด = ช่อง BLE ตาย → A9 Max พิมพ์จากเว็บไม่ได้ แนะนำใช้แอปทางการ). (regression) A9 → ทดสอบพิมพ์ → ต้องออกเหมือนเดิม.**

**Phase 591 (เลือกขนาดกระดาษสลิป 58/80/107mm + auto-default ตามรุ่น · PR #169 `67efd78` MERGED+DEPLOYED 2026-07-09 build 591):** **เป้า:** รองรับความกว้างกระดาษหลายขนาด (A9 Max=4"/832 dot, 80mm=576) — เดิม canvas คงที่ 384 (58mm). **แก้ (`receipt_bt.js`):** `PAPER_WIDTHS = {"58":384(=CANVAS_WIDTH), "80":576, "107":832}`; `PAPER_KEYS=["auto","58","80","107"]`; module state `let _paperKey="auto"` (init จาก `localStorage bsk_slip_paper` ตอนโหลด, guard `typeof localStorage!=="undefined"` + try). export **`getPaperPx()`** (auto → `/max/i.test(_device?.name) ? 832 : CANVAS_WIDTH(384)`; else `PAPER_WIDTHS[_paperKey]||384`) · **`getPaperWidthKey()`** (คืน `_paperKey` สำหรับ dropdown) · **`setPaperWidth(key)`** (validate ∈ PAPER_KEYS → set + persist localStorage; key ผิด → return false ไม่ทับ). `renderReceiptCanvas`/`renderTestCanvas`: เปลี่ยน `const W = CANVAS_WIDTH` → **`const W = getPaperPx()`** (layout center=W/2·right=W-PAD·item row ปรับตาม W อัตโนมัติ; `canvasToRasterBody` คิด `bytesPerRow=ceil(width/8)` จาก `canvas.width` = รองรับทันที ไม่ต้องแก้). **`settings/pages.js`:** การ์ดเครื่องพิมพ์ +`<select id="btPaperSize">` "📏 ขนาดกระดาษ" (อัตโนมัติ/58/80/107mm) ค่าเริ่ม `_btPaperKey=getPaperWidthKey()`; onChange → `setPaperWidth(value)` + toast. **★ ไม่แตะ (ยืนยัน `git diff 21904ef -- receipt_bt.js`):** char selection (`preferUuid`/`isMax`/fec7/ff02/MAX override ของ 590) · `buildPrintBytes`/framing · `sendChunked` · money flow · `window.print()` · `bt_printer.js` · RLS/SQL — diff = **เฉพาะ width (เพิ่ม PAPER const/state/getters + เปลี่ยน 2 จุด `W=CANVAS_WIDTH→getPaperPx()`)**. **★ A9 regression:** A9 (ชื่อไม่มี MAX, `_paperKey` default "auto") → `getPaperPx()=384=CANVAS_WIDTH` → renderReceiptCanvas W=384 = **byte-identical เดิม** (ยืนยัน node: auto no-device → 384). A9 Max auto → 832. **Tests:** `receipt_bt_paper_width_guard`(3): (a) auto — ชื่อ MAX→832 & ชื่อ A9→384 (regression, ผ่าน connect mock GATT จริง ISSC 8841+ff00/ff02); (b) `setPaperWidth` 58→384/80→576/107→832 + getPaperWidthKey + key ผิด reject; (c) `canvasToRasterBody(mockCanvas width)` → header GS v 0 xL=ceil(w/8) (832→104, 576→72, 384→48). *(renderReceiptCanvas `canvas.width` ต้อง DOM → ทดสอบ Node ตรงไม่ได้; cover ผ่าน getPaperPx + canvasToRasterBody).* guard 590 (max/peripage/raster/feature) เขียวครบ. companion `dashboard_readonly_guard` marker 591. **Verification:** lint:errors 0 (0 warn) · unit 2646/2646 (+3) · e2e: smoke APP_BUILD flake เดิม (build-sync markers 591 เขียว = markers ถูก) · markers 591 (?v=591 ×4 + cache-v591 + version 5.69.62). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#169 `67efd78` — CI Tests + Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev (CONNECT 403) — Deploy success บน commit ที่ฝัง data-app-build=591 = หลักฐาน; owner hard-refresh ยืนยัน 591). owner test (Android, build 591): (regression) A9 → เชื่อม (auto=58mm) → ทดสอบพิมพ์ → **ต้องออกเหมือนเดิม** · (A9 Max) เลือก dropdown 107mm หรือ auto → ทดสอบพิมพ์ → ลุ้นออกเต็มความกว้าง 4 นิ้ว · เลือกขนาดใน dropdown → toast + จำค่า (persist).**

**Phase 589-cont (กู้ A9 base 586 + A9 Max override ff02 — ล็อกเฉพาะชื่อ "MAX" · PR #168 `21904ef` MERGED+DEPLOYED 2026-07-09 build 590):** **บริบท:** revert 589 (`f23a731`) กู้ A9 กลับ 586 แล้ว (deployed). งานนี้ต่อยอด: A9 คงเดิม 586 + เพิ่มลอง A9 Max ใช้ช่อง `ff02`. **★ bump 590 (ไม่ใช่ 589):** main อยู่ 589 แล้ว → ต้อง bump CACHE_NAME เพื่อ bust SW cache ให้ `receipt_bt.js` ใหม่ถึงเครื่อง (receipt_bt.js import แบบ plain `./modules/receipt_bt.js` ไม่มี `?v=` → SW cache ตาม CACHE_NAME; คง 589 = เสิร์ฟไฟล์เก่า override ไม่ถึง). **แก้ (override แคบจุดเดียวใน `connectSlipPrinter`):** `const isMax = /max/i.test(_device?.name || ""); const preferUuid = isMax ? "ff02" : PERIPAGE_WRITE_CHAR;` แล้วเปลี่ยนเงื่อนไขเลือก char จาก `includes(PERIPAGE_WRITE_CHAR)` → `includes(preferUuid)`. **`git diff 561eceb -- receipt_bt.js` = เฉพาะ 5 บรรทัดนี้** (comment 2 + const 2 + เปลี่ยน includes 1). **ผล:** A9 Max (ชื่อมี "MAX") → `preferUuid="ff02"` → เลือก ff02 (thermal มาตรฐาน ใต้ ff00); **A9 (ชื่อไม่มี "MAX") → `preferUuid="fec7"` (= PERIPAGE_WRITE_CHAR) → ไม่เจอ → fallback firstWritable = 8841 = byte-identical 586** (A9 path ไม่เปลี่ยนเลย). **ไม่แตะ `buildPrintBytes`:** `isPeriPage()` = ชื่อ /peripage/ → A9 Max ชื่อ "PeriPage_A9MAX" → true → **PeriPage framing** (`10 FF FE 01` reset + concentration + GS v 0 + ESC J) → คอมโบที่ต้องการ = **char ff02 + PeriPage-classic framing**. **ไม่แตะ `sendChunked`** (586 direct: 200/20, `writeWithoutResponse` ถ้ามี — ff02 มี WoR). **ไม่แตะ `settings/pages.js`** (diagnostic 586). **Tests:** `receipt_bt_max_guard`(2): (1) A9 regression — name "PeriPage_A9_BLE", GATT จริง [ISSC 8841, ff00/ff02] → `getSlipTarget().char` = **8841** (ไม่ใช่ ff02) + buildPrintBytes ขึ้นต้น `10 FF FE 01` (PeriPage) = เหมือน 586; (2) A9 Max — name "PeriPage_A9MAX_BLE", GATT เดียวกัน → char = **ff02** + buildPrintBytes `10 FF FE 01` + GS v 0 + ESC J ท้าย (PeriPage framing, ไม่ใช่ ESC @/cut). ปรับ `receipt_bt_peripage_guard` test(e) ชื่อ "PeriPage_A9MAX" → "PeriPage_A9" (เพราะ MAX override เปลี่ยน preferUuid — test(e) ทดสอบ fec7-preference สำหรับเครื่อง non-MAX). raster guard (586) คงเดิม. companion `dashboard_readonly_guard` marker 590. **Verification:** `receipt_bt.js` diff 561eceb = เฉพาะ override MAX · lint:errors 0 (0 warn) · unit **2643/2643** (2641 + 2 max guard) · e2e 14/14 (build-sync markers 590 เขียว) · markers 590 (?v=590 ×4 + cache-v590 + version 5.69.61). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#168 `21904ef` — CI Tests + Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev (CONNECT 403) — Deploy success บน commit ที่ฝัง data-app-build=590 = หลักฐาน; owner hard-refresh ยืนยัน 590). owner test (Android, build 590): (regression) A9 → เชื่อม → ทดสอบพิมพ์ → **ต้องออกเหมือนเดิม** (เป้าหลัก) · (ลุ้น) A9 Max → ตัดการเชื่อม → เชื่อมใหม่ → สถานะควรโชว์ `[char 0000ff02]` → ทดสอบพิมพ์ → ลุ้นออก (ff02 + PeriPage framing). ยังไม่ออก → capture byte จริงจากแอปทางการ.**

**Phase 589 (REVERT ด่วน — คืนเครื่องพิมพ์กลับ build 586 · PR #167 `f23a731` MERGED+DEPLOYED 2026-07-09 build 589):** **อาการ:** build 588 ทำให้ **A9 (เครื่องที่เคยพิมพ์ได้)** พัง = **regression**. **สาเหตุ (บทเรียน):** 587/588 สร้างบนสมมติฐานผิดว่า "A9 ใช้ char `fec7`" (เชื่อจากคอมเมนต์ SLIP_PRINTER_SERVICES ที่เขียนเดาไว้ตั้งแต่ 585 — ไม่ได้ verify กับ GATT เครื่องจริง). **ความจริง (GATT เครื่องจริง):** A9 **ไม่มี fec7** — ใช้ ISSC service `49535343` (char `8841`) + service `ff00` (`ff02`) เหมือน A9 Max ทุกอย่าง. **A9 ที่พิมพ์ได้จริง (build 586, มีรูปยืนยันจาก owner)** ใช้: char `8841` (fallback "writable ตัวแรก" เพราะ 586 หา fec7 ไม่เจอ) + PeriPage framing (`isPeriPage()` ตัดสินจาก**ชื่อ** device = "PeriPage…" → true) + direct write (200/20/writeWithoutResponse). **588 พัง A9 เพราะ:** `PRINT_WRITE_CHARS=[fec7,ff02]` → A9 ไม่มี fec7 → เลือก **ff02** (แทน 8841); `usePeriPageProtocol()` (ช่อง-based /fee7//fec7/) → ff02/ff00 = **false** → **generic ESC/POS** (ESC @) แทน PeriPage reset → A9 (รับ PeriPage-proto) โดนสับทั้งช่องและโปรโตคอล → ไม่พิมพ์. **แก้ (revert เจาะจงไฟล์):** `git checkout 561eceb -- modules/receipt_bt.js` → **byte-identical build 586** (ยืนยัน `git diff 561eceb -- modules/receipt_bt.js` = ว่าง): connect = fec7-priority→fallback firstWritable (A9 ได้ 8841), `buildPrintBytes` = `isPeriPage()` ชื่อ-based (A9 ชื่อ PeriPage → PeriPage framing), `sendChunked(char,bytes)` เดียว (200/20/writeWithoutResponse, **ไม่มี** uart branch/opts), generic path มี `GS V` cut เดิม, **ไม่มี** `PRINT_WRITE_CHARS`/`usePeriPageProtocol`/`isUartTransport`/`_notifyChar`. คืน `receipt_bt_raster_guard`+`receipt_bt_peripage_guard` กลับ 586 (`git checkout 561eceb --`); **ลบ** `receipt_bt_a9max_guard.test.js` (test พฤติกรรม 587/588 ที่ revert ออกแล้ว). **ไม่แตะ `modules/settings/pages.js`** — ปุ่ม diagnostic "🔍 ตรวจเครื่องพิมพ์" + `getSlipTarget` status เป็นของ **586** อยู่แล้ว (ยืนยัน grep: ไม่ import `usePeriPageProtocol`/`isUartTransport` = ไม่มี import ค้างหลัง revert). **ยังไม่แก้ A9 Max** — จะทำแยกด้วยการ capture byte จริงจากแอปทางการ (nRF Connect / BLE sniff) แล้ว replay (เลิกเดา). **Verification:** ★ `receipt_bt.js` byte-identical 561eceb (diff ว่าง) · lint:errors 0 · unit **2641/2641** (= 586 count; 588 มี 2649 − 8 a9max = 2641) · e2e 14/14 (build-sync markers 589 เขียว) · markers 589 (?v=589 ×4 + cache-v589 + version 5.69.60). files: `receipt_bt.js`(revert) `raster/peripage guard`(revert) `a9max guard`(ลบ) `index.html`/`sw.js`/`dashboard_readonly_guard`(589) — **settings/pages.js ไม่แตะ**. **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ⏳ pushed → STOP รอ owner ยืนยัน A9 พิมพ์ได้อีกครั้ง (เป้าหลัก). A9 Max ยังไม่คาดหวังว่าออก (จะทำแยก).** *(บทเรียน: 585 เขียนคอมเมนต์ UUID เดาไว้ → 587/588 หลงเชื่อไม่ verify GATT จริง → regression. ต่อไปยึด diagnostic เครื่องจริงเท่านั้น.)*

**Phase 588 (A9 Max พิมพ์ผ่านช่อง ff02 / ESC/POS มาตรฐาน — เลือก char ตามช่อง; A9 คงเดิม · PR #166 `60e7ea8` ⚠️ REVERTED โดย build 589 (ทำให้ A9 พัง) · build 588):** **อาการ:** build 587 ยังพิมพ์ A9 Max ไม่ได้. **GATT diagnostic เครื่องจริงเผย A9 Max มี 2 service:** `49535343-…` (ISSC transparent UART, char `8841` [write] — โค้ด 585-587 เลือกตัวนี้ผ่าน "fallback writable ตัวแรก" = **ผิดช่อง**) + `0000ff00-…` (thermal มาตรฐาน, char `0000ff01`[notify], **`0000ff02`** [writeWithoutResponse,write], `0000ff03`[notify]). ช่อง ff00/ff02 = โปรไฟล์เครื่องพิมพ์ thermal มาตรฐาน (Xprinter/generic ESC/POS) — A9 Max เปิดช่องนี้ = พิมพ์ผ่าน ff02 ด้วย **ESC/POS มาตรฐาน** (ไม่ใช่ PeriPage-classic แบบ A9 fee7/fec7). **แก้ (`receipt_bt.js`):** **(1) เลือก characteristic ตามลำดับช่อง:** `PRINT_WRITE_CHARS = [PERIPAGE_WRITE_CHAR("fec7"), "ff02"]`; `connectSlipPrinter` เปลี่ยนจาก "หา fec7 ก่อน else fallback ตัวแรก" → **รวบรวม `allWritable` (writable char ทุก service) ก่อน** แล้ววน `PRINT_WRITE_CHARS`: `allWritable.find(uuid.includes(prefix))` — fec7 → ff02 → ไม่เจอ → `allWritable[0]`. ผล: **A9 → fec7 (เหมือนเดิม)**; **A9 Max → ff02** (แทน `49535343-8841`). เก็บ `_serviceUuid`/`_charUuid` เดิม. **(2) framing ตาม "ช่อง" ไม่ใช่ชื่อ:** เพิ่ม `usePeriPageProtocol()` = `/fee7/i _serviceUuid || /fec7/i _charUuid` (แทน `isPeriPage()` ชื่อ-based ใน buildPrintBytes; `isPeriPage` ยัง export เพื่อ backward-compat). `buildPrintBytes`: `usePeriPageProtocol()` (A9 fec7/fee7) → `concatBytes(PERIPAGE_RESET, PERIPAGE_CONCENTRATION, body, PERIPAGE_FEED)` (คงเดิมเป๊ะ); else (ff02/generic) → `concatBytes(ESC_INIT, body, ESC_FEED)` = **ESC @ + GS v 0 + ESC d feed**. **(3) ★ ตัด `ESC_CUT` (GS V 1) ออกทุก path** — `canvasToEscposRaster` wrapper + generic branch เหลือ feed-only (เครื่องพกพา thermal ไม่มีคัตเตอร์ → cut เสี่ยง jam). **A9 Max ผล:** เลือก ff02 → service ff00 → `isUartTransport()=false` → **direct write path** (writeWithoutResponse/200/20) + generic ESC/POS framing. 587 UART code (isUartTransport/_notifyChar/startNotifications/sendChunked uart branch) **คงไว้** แต่ไม่ทำงานกับ A9/A9 Max (ff00≠49535343) — เผื่อเครื่อง ISSC-only. **★ A9 (fee7/fec7) byte-identical 587:** connect เลือก fec7 (PRINT_WRITE_CHARS[0]) → `_serviceUuid=fee7`; `usePeriPageProtocol()=true` → PeriPage framing; `isUartTransport()=false` → sendChunked `{uart:false}` = chunk 200/delay 20/writeWithoutResponse. **ไม่แตะ:** `renderReceiptCanvas`/`canvasToRasterBody` (raster body ถูกแล้ว) · money flow · `window.print()` · `bt_printer.js` · RLS/SQL. **Tests (`receipt_bt_a9max_guard` +3):** (f) A9 (fec7): `usePeriPageProtocol` true + `isUartTransport` false + buildPrintBytes ขึ้นต้น `10 FF FE 01` + ESC J ท้าย (คงเดิม); (g) **A9 Max (ISSC 8841 + ff02): getSlipTarget char = ff02 (ไม่ใช่ 8841)** + `isUartTransport` false + `usePeriPageProtocol` false + buildPrintBytes ขึ้นต้น `1B 40` + มี `1D 76 30 00` + ท้าย feed `1B 64` + ไม่มี `1D 56` cut + ไม่มี `10 FF FE` reset; (h) fallback ไม่มี fec7/ff02 → writable ตัวแรก. a9max UART tests เดิม (a-e) คงผ่าน (ISSC-only mock ไม่มี ff02 → fallback 8841 → UART). ปรับ `receipt_bt_raster_guard` test(3) trailing 6→3 bytes + test(6) cut→feed; `receipt_bt_peripage_guard` test(c) generic cut→feed. companion `dashboard_readonly_guard` marker 588. **known-risk / LIMITATION:** attempt #3 (ช่อง ff02 + ESC/POS มาตรฐาน) — "A9 Max พิมพ์ออกจริง" ต้อง owner ทดสอบเครื่องจริง; ถ้ายังไม่ออก → ขั้นถัดไป capture byte จริงจากแอปทางการ (Wireshark BLE / nRF Connect) แล้ว replay. **A9 (regression) ต้องยังพิมพ์ได้เหมือนเดิม.** **Verification:** lint:errors 0 (0 warnings) · unit 2649/2649 (+3) · e2e: smoke APP_BUILD flake เดิม (build-sync markers 588 เขียว = markers ถูก) · markers 588 (?v=588 ×4 + cache-v588 + version 5.69.59). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#166 `60e7ea8` — CI Tests + Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev (CONNECT 403) — Deploy success บน commit ที่ฝัง data-app-build=588 = หลักฐาน; owner hard-refresh ยืนยัน 588). owner test (Android): (regression) เชื่อม A9 → ทดสอบพิมพ์ → **ต้องยังออก** · (ใหม่) ตัดการเชื่อม → เชื่อม A9 Max → สถานะควรโชว์ `[char …ff02…]` (ไม่ใช่ 49535343) → ทดสอบพิมพ์ → ออกไหม; ไม่ออก → 🔍 diagnostic + บอกอาการ (error/เงียบ) + แอปทางการพิมพ์ออกไหม.**

**Phase 587 (PeriPage A9 Max via ISSC transparent UART — hotfix ต่อ 586 · PR #165 `10a6153` MERGED+DEPLOYED 2026-07-09 build 587):** **อาการ:** build 586 พิมพ์ **A9** ได้แล้ว (fee7/fec7) แต่ **A9 Max** ยังไม่ออก. **จาก GATT diagnostic บนเครื่องจริง:** A9 Max ใช้ BLE stack คนละแบบ = **ISSC/Microchip transparent UART** — service `49535343-fe7d-4ae5-8fa9-9fafd205e455`, write char `49535343-8841-43f4-a8d4-ecbe34729bb3` [writeWithoutResponse,write], notify char `49535343-1e4d-4bd9-ba61-23c647249616` [notify]. (A9 Max=80mm firmware V1.22; A9=58mm fee7/fec7 V1.27 — คนละสถาปัตยกรรม). 586 เชื่อม A9 Max ได้ + เลือก char 8841 ถูก (fallback writable) แต่ transparent UART มักต้อง **"เปิด pipe"** ก่อน (startNotifications บน notify char activate UART bridge; write-with-response ไม่ใช่ WoR; chunk เล็ก+delay มาก). **★ ห้ามแตะ path A9 (fee7/fec7) ที่พิมพ์ได้แล้ว** — regression = A9 พัง ยอมรับไม่ได้. **แก้ (`receipt_bt.js` — เฉพาะ ISSC UART):** **(1)** `isUartTransport()` export = `String(_serviceUuid).toLowerCase().includes("49535343")` (`ISSC_SERVICE_PREFIX`). **(2) connect:** หลังเลือก `_char`/`_serviceUuid`/`_charUuid` — ถ้า `isUartTransport() && pickedSvc` → `pickedSvc.getCharacteristics()` หา `c.properties.notify` เก็บ `_notifyChar`; ถ้ามี `startNotifications` → `await _notifyChar.startNotifications()` ครั้งเดียว (`_notifyStarted`, try/catch — บาง stack ไม่ต้อง). reset `_notifyChar`/`_notifyStarted` ที่ disconnect + gattserverdisconnected. **(3) `sendChunked(char, bytes, opts={})`:** `uart=!!opts.uart`; `size = uart?128:200` (`CHUNK_UART`/`CHUNK`), `delay = uart?40:20`; **UART → `writeValue` (with-response) เสมอ**; **non-uart → เดิมเป๊ะ** (`writeWithoutResponse` ถ้ามี else `writeValue`). **(4)** `printReceipt`/`testPrint` ส่ง `sendChunked(char, bytes, { uart: isUartTransport() })`. **★ A9 path ไม่เปลี่ยน:** A9 (fee7) → `isUartTransport()=false` → connect ข้าม notify block ทั้งก้อน + sendChunked `uart:false` = chunk 200/delay 20/writeWithoutResponse (byte-identical 586). **protocol bytes + `buildPrintBytes` framing + `renderReceiptCanvas`/`canvasToRasterBody` คงเดิม** (แบรนด์เดียวกัน — เปลี่ยนแค่ "วิธีส่ง"; ถ้ารอบนี้ยังไม่ออกค่อยลอง generic ESC/POS รอบหน้า). **ไม่แตะ:** A9 path (chunk/WoR/delay) · framing · money flow · `window.print()` · `bt_printer.js` · RLS/SQL. **Tests:** `receipt_bt_a9max_guard`(5: (a) `isUartTransport` true ISSC / false A9 (ผ่าน connect mock) + getSlipTarget char=8841 · (b) sendChunked uart → `writeValue` (ไม่ใช่ WoR) + chunk ≤128 + ครบ byte · (c) **non-uart A9 → `writeValueWithoutResponse` + chunk ≤200 คงเดิม (regression guard)** · (d) connect ISSC → `startNotifications` ถูกเรียกบน notify char (1e4d) ครั้งเดียว · (e) A9 fee7 connect ไม่เรียก startNotifications). mock `navigator` `Object.defineProperty` (Node22) + `getPrimaryServices`/notify char. guard 586 (`receipt_bt_peripage`/`raster`/`feature_detect`) เขียวครบ — ไม่แก้ framing. companion `dashboard_readonly_guard` marker 587. **known-risk / LIMITATION:** best-effort attempt #1 — unit ครอบ transport-select/chunk/subscribe เท่านั้น; "A9 Max พิมพ์ออกจริง" ต้อง owner ทดสอบเครื่องจริง; สมมติฐานเรียงน้ำหนัก (startNotifications > write-with-response > chunk เล็ก) — ถ้ายังไม่ออกอาจต้อง generic ESC/POS หรือ protocol A9 Max เฉพาะ (ส่ง diagnostic + อาการ error toast มา). **A9 (regression) ต้องยังพิมพ์ได้เหมือนเดิม.** **Verification:** lint:errors 0 · unit 2646/2646 (+5) · e2e 14/14 (build-sync markers 587 เขียว) · markers 587 (?v=587 ×4 + cache-v587 + version 5.69.58). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#165 `10a6153` — CI Tests + Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev (CONNECT 403) — Deploy success บน commit ที่ฝัง data-app-build=587 = หลักฐาน; owner hard-refresh ยืนยัน 587). owner test (Android): (regression) เชื่อม A9 → ทดสอบพิมพ์ → **ต้องยังออกเหมือนเดิม** · (ใหม่) ตัดการเชื่อม → เชื่อม A9 Max → ทดสอบพิมพ์ → ออกไหม; ไม่ออก → 🔍 ตรวจเครื่องพิมพ์ + บอกอาการ (มี error toast ไหม).**

**Phase 586 (PeriPage protocol support + BLE GATT diagnostic — hotfix ต่อ 585 · PR #164 `561eceb` MERGED+DEPLOYED 2026-07-09 build 586):** **อาการ:** build 585 เชื่อม PeriPage A9MAX_BLE ได้ (owner ยืนยัน "🟢 เชื่อมแล้ว") แต่กดพิมพ์ไม่ออก. **ต้นตอ (ref bitrate16/peripage-python):** PeriPage ใช้ raster `GS v 0` (1D 76 30 00) เหมือน ESC/POS มาตรฐาน (`renderReceiptCanvas`/raster ของเราถูก) **แต่ต้องมีคำสั่งเฉพาะ PeriPage หุ้ม** ที่ 585 ไม่มี: (i) ไม่รู้จัก `ESC @` (1B 40) → ต้อง reset `10 FF FE 01` + zero (16 bytes); (ii) ต้องตั้ง concentration ก่อน ไม่งั้นจาง/ว่าง: `10 FF 10 00 02`; (iii) ท้าย `GS V 1` (1D 56 01 partial cut) → PeriPage ไม่มีคัตเตอร์ ทำเครื่องค้าง/ไม่พิมพ์ → ต้อง `ESC J` (1B 4A n) feed แทน; (iv) char ต้องเป็น `0000fec7` ใต้ service `0000fee7` — 585 เชื่อมได้อาจโดน writable char ผิดตัว. **แก้ (`modules/receipt_bt.js`):** **(1) service/char:** เพิ่ม `"0000fee7-..."` ต้น `SLIP_PRINTER_SERVICES` (คงตัวเดิม 5); `connectSlipPrinter` เปลี่ยนจาก "getPrimaryService ตัวแรกที่เจอ + find writable ตัวแรก" → **วน `getPrimaryServices()` ทุก service** (fallback วนทีละ uuid ถ้า getPrimaryServices ว่าง) → หา characteristic ที่ uuid `.includes(PERIPAGE_WRITE_CHAR="fec7")` **ก่อน**, ไม่เจอ → fallback ตัว writable ตัวแรกข้าม service; เก็บ `_serviceUuid`/`_charUuid` (reset ที่ disconnect + gattserverdisconnected); `getSlipTarget()` export. **(2) framing แยกตามเครื่อง:** `canvasToRasterBody(canvas)` ใหม่ = **raster `GS v 0` เท่านั้น** (ไม่มี init/feed/cut), แบ่ง **chunk ≤255 แถว** (RASTER_MAX_ROWS) — แต่ละ chunk header `1D 76 30 00 xL xH yL yH` (yL=จำนวนแถว chunk ≤255, yH=0). `canvasToEscposRaster(canvas)` (คงชื่อเดิม) = generic wrapper `concatBytes(ESC_INIT, body, ESC_FEED, ESC_CUT)`. **`buildPrintBytes(canvas)`** route ตาม `isPeriPage()`: PeriPage → `concatBytes(PERIPAGE_RESET[16], PERIPAGE_CONCENTRATION[10FF100002], body, PERIPAGE_FEED[1B4A60])`; generic → `ESC @ + body + ESC d 3 + GS V 1`. `isPeriPage()` = `/peripage/i` ชื่อ device หรือ `/fee7/i` service. **(3)** `printReceipt`/`testPrint` เปลี่ยนจาก `canvasToEscposRaster` → **`buildPrintBytes`**; `sendChunked` เดิม (200B/20ms). **(4) `diagnoseSlipPrinter()`** export ใหม่ — `requestDevice`(gesture) → `getPrimaryServices()` → per-service `getCharacteristics()` → array `{service,char,props}`; `props` จาก `CHAR_PROP_FLAGS.filter(k=>props[k])` (★ `BluetoothCharacteristicProperties` เป็น getter → `Object.entries`=[] → เช็คชื่อเอง). **UI (`settings/pages.js` About):** +ปุ่ม `btDiagnoseBtn` "🔍 ตรวจเครื่องพิมพ์ (debug)" → `diagnoseSlipPrinter()` แสดงใน `<pre id=btDiagOutput>` บนจอ (จัดกลุ่มตาม service, owner screenshot ได้) + สถานะ connect โชว์ `[char xxxxxxxx]` (getSlipTarget). **คง `renderReceiptCanvas` (เนื้อใบเสร็จ)/feature-detect เดิม** — แก้เฉพาะ "การหุ้มคำสั่ง" + characteristic + service. **ไม่แตะ:** money flow · `window.print()` · `bt_printer.js` (label) · `thermal_printer.js` · RLS/SQL. **Tests:** `receipt_bt_peripage_guard`(7: (a) fee7+`PERIPAGE_WRITE_CHAR="fec7"` source · (b) buildPrintBytes PeriPage = reset+concentration+GS v0+ESC J, ไม่มี cut/ESC @ · (c) generic = ESC @+body+GS V cut · (d) canvasToRasterBody 300แถว→≥2 chunk, chunk แรก yL=255, body ไม่มี init/cut · (e) connect เลือก fec7 ข้าม writable ตัวอื่น+ข้าม service + getSlipTarget · (f) diagnose คืน array {service,char,props} · (g) diagnose reject Android/Chrome). ปรับ `receipt_bt_raster_guard` test(3) เป็น chunk เดียว ≤255 (200แถว yL=200,yH=0); test อื่น (ESC @/GS v0/bit-pack/cut ท้าย) คงเดิม (generic wrapper ยังมี cut). mock `navigator` ผ่าน `Object.defineProperty` (Node22 configurable getter) + restore; mock `getPrimaryServices`. `receipt_bt_feature_detect_guard` เดิมเขียวครบ. companion `dashboard_readonly_guard` marker 586. **known-risk / LIMITATION:** unit ครอบ **byte-framing/select/detect เท่านั้น** — "PeriPage A9MAX พิมพ์ออกจริง (เข้ม/ไทยชัด)" ต้อง owner ทดสอบเครื่องจริง (ไม่มีฮาร์ดแวร์ CI); ถ้ายังไม่ออก → กด 🔍 diagnostic ส่ง screenshot (ยืนยัน fee7/fec7 จริง หรือ firmware ต่าง); concentration ตั้ง 2 (เข้มสุด) — ถ้าเข้มไป/จางไปปรับ byte ท้าย `PERIPAGE_CONCENTRATION`; chunk 255 แถว = หลาย GS v 0 บล็อก (เครื่องรับซ้อนแนวตั้งได้). **Verification:** lint:errors 0 · unit 2641/2641 (+7) · e2e: smoke 14/14 (build-sync markers 586 เขียว = markers ถูก; APP_BUILD flake เดิม container timing ไม่ใช่ regression) · markers 586 (?v=586 ×4 + cache-v586 + version 5.69.57). **ไม่มี SQL. ไม่เพิ่ม npm dependency.** **Status: ✅ MERGED+DEPLOYED (#164 `561eceb` — CI Tests + Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev (CONNECT 403) — Deploy success บน commit ที่ฝัง data-app-build=586 = หลักฐาน; owner hard-refresh ยืนยัน 586). owner test (Android): Settings→About→**ตัดการเชื่อมเก่าก่อน (เริ่มสด)**→เชื่อม PeriPage ใหม่ (สถานะโชว์ `[char 0000fec7]`)→ทดสอบพิมพ์ → ต้องออก (ไทยชัด เข้ม). ไม่ออก → 🔍 ตรวจเครื่องพิมพ์ → ส่ง screenshot service/char → รอบถัดไปปรับตามจริง.** *(หมายเหตุ: แทนที่ 586 เดิม (fee7/fec7+ตัด cut อย่างเดียว, ยังไม่ merge) ด้วยเวอร์ชัน protocol เต็มนี้ — reset branch จาก origin/main ตาม prompt)*

**Phase 585 (Bluetooth thermal slip printer — Android · PR #163 `e649ce4` MERGED+DEPLOYED 2026-07-09 build 585):** **เป้า:** เพิ่มพิมพ์ใบเสร็จเข้าเครื่องสลิป 58/80mm ที่เชื่อม BLE ตรง ผ่าน Web Bluetooth + ESC/POS. เดิมพิมพ์ได้แค่ `window.print()` (main.js:4602 `printLastReceipt`, receipts.js:1121) = ส่งเข้าเครื่องสลิป BLE ไม่ได้; `thermal_printer.js` (ESC/POS receipt) **dead** ไม่มีใคร import; ไม่มี UI เชื่อม Bluetooth → พิมพ์สลิปไม่ได้. **ขอบเขตอุปกรณ์: Android (Chrome/Edge) เท่านั้น** — iOS บล็อก Web Bluetooth ทุกเบราว์เซอร์ → feature-detect + แจ้งชัด ไม่ crash. **สถาปัตยกรรมหลัก (canvas→raster→BLE — กันไทยเพี้ยน + offline):** พิมพ์ใบเสร็จเป็น **"ภาพ" (raster) ไม่ใช่ text** — วาดลง `<canvas>` กว้างคงที่ 384px (58mm@203dpi) ด้วย `ctx.fillText` (เรนเดอร์ไทยเนทีฟจาก browser font) → แปลง ESC/POS raster (`GS v 0`) → ส่งเข้าเครื่อง. **เหตุผล:** เครื่องสลิปถูก ๆ ส่ง ESC/POS text ต้องเลือก codepage ไทย (TIS-620/CP874) ต่างกันทุกรุ่น = เพี้ยนง่าย; ส่งเป็นภาพ = เครื่องพิมพ์ pixel ตรง ๆ ไม่พึ่ง font/codepage. **ห้ามใช้ html2canvas/CDN** — canvas 2D API ตรง (offline-safe, ไม่มี runtime dep). **`modules/receipt_bt.js` (ใหม่ — โมดูลหลัก):** (1) `isSlipSupported()` = `typeof navigator!=="undefined" && !!navigator.bluetooth` (Node/iOS → false ไม่ throw). (2) `connectSlipPrinter()` (mirror `bt_printer.js connectPrinter` proven) — ไม่รองรับ → `throw Error("...Android + Chrome/Edge...")`; reuse ถ้า `_device.gatt.connected && _char`; `requestDevice({acceptAllDevices,optionalServices: SLIP_PRINTER_SERVICES})` (5 UUID generic thermal + 000018f0) → gatt.connect → วน getPrimaryService หา service → getCharacteristics หา `write||writeWithoutResponse`; singleton `_device`/`_char`; `gattserverdisconnected`→`_char=null`. (3) `isSlipConnected`/`getSlipDeviceName`/`disconnectSlip`. (4) `renderReceiptCanvas(receipt, store)` — normalizer รับได้ทั้ง POS `lastReceipt` (`order_no`/`total_amount`/items `product_name`/`line_total`) และ receipts record (`receipt_no`/`grand_total`/`_lineItems` `item_name`/`unit_price`); สองรอบ (วัดความสูง→วาด); font `'Sarabun','Noto Sans Thai',sans-serif`; ชื่อร้าน/ที่อยู่/โทร/ภาษี/เลขที่/วันที่ (`formatDateTime` Asia/Bangkok util)/รายการ/รวม-ส่วนลด-เครดิต-รับ-ทอน/"ขอบคุณ"; เงินผ่าน `money` util. (5) `canvasToEscposRaster(canvas)` — `getImageData` → luminance `<128`=ดำ(bit1, MSB=pixel ซ้ายสุด), `bytesPerRow=ceil(w/8)`; `ESC @` + `GS v 0 m(0) xL xH yL yH` (xL/xH=bytesPerRow LE, yL/yH=rows LE) + data + `ESC d 3` feed + `GS V 1` partial cut → Uint8Array. (6) `sendChunked(char, bytes)` (mirror bt_printer) — chunk 200 byte, prefer `writeValueWithoutResponse`, delay 20ms. (7) `printReceipt` (connect→`document.fonts.ready`→canvas→raster→send; **connect ก่อน await font** = requestDevice ใช้ user-gesture สด) / `testPrint`. **UI (settings About page `pages.js renderSettingsAbout`):** การ์ด "🖨️ เครื่องพิมพ์สลิป (Bluetooth)" — `_btSupported`→ปุ่ม [เชื่อม]/[ทดสอบพิมพ์]/[ตัดการเชื่อม] + สถานะ; `!_btSupported`→กล่องแดง "ใช้ได้เฉพาะ Android + Chrome/Edge (iOS ใช้ไม่ได้)" ไม่มีปุ่ม (feature-detect, ไม่ crash). connect จาก click gesture. **main.js (ใบเสร็จหลังขาย):** import `* as receiptBt` (:8); ปุ่ม `btSlipReceiptBtn` "🖨️ สลิป BT" ใน receiptDrawer (index.html:844) + `printSlipBt()` (feature-detect → `printReceipt(state.lastReceipt, state.storeInfo)` → toast; error → showToast+console.error). **receipts.js (reprint บิลเก่า):** import `* as receiptBt`; ปุ่ม `rcBtSlipBtn` ข้าง rcPrintBtn (:773) + handler (`{...r, items:_lineItems}` → `printReceipt`). **null-guard (bug เดิม):** `window.open` อาจคืน null (popup block) — main.js:4604 `printLastReceipt` + receipts.js:1121 `rcPrintBtn` เพิ่ม `if(!w) return showToast(...)`. **ไม่แตะ:** ตรรกะขาย/checkout/`saveReceipt`/JV/`auto_post`/money flow (ฟีเจอร์นี้ **อ่าน** ใบเสร็จไปพิมพ์เท่านั้น) · ปุ่ม `window.print()`/PDF/Share เดิม (คงพฤติกรรม + แค่เพิ่ม null-guard) · `bt_printer.js` (label TSPL — คนละเครื่อง/ภาษา) · `thermal_printer.js` (ปล่อย dead) · RLS/SQL/loadAllData cap. **Tests:** `receipt_bt_raster_guard`(6: mock canvas → header `GS v 0`+m=0 / xL·xH·yL·yH LE / มิติใหญ่ 384×300 + data len=bytesPerRow*rows / bit-pack ทึบ0xFF·สลับ0xAA·ว่าง0x00 / width ไม่หาร8 padding ceil / partial cut ท้าย) · `receipt_bt_feature_detect_guard`(6: `isSlipSupported` false ใน Node / `connectSlipPrinter` reject Error /Android|Chrome/ ไม่ throw undefined / sendChunked ≤200 ครบทุก byte / prefer writeWithoutResponse / source-regex null-guard main+receipts / import ได้ใน Node ไม่มี DOM top-level). companion `dashboard_readonly_guard` marker 585. guard เดิมเขียวครบ. **known-risk / LIMITATION:** unit ครอบ **encoding/detect/fallback เท่านั้น** — "พิมพ์ออกจริง + ไทยสวย + UUID เครื่องรุ่นนี้เชื่อมติด" **ไม่มีฮาร์ดแวร์ใน CI → owner ต้องทดสอบบน Android + เครื่องสลิปจริง**; SLIP_PRINTER_SERVICES เป็น UUID ทั่วไป — ถ้าเครื่องบางรุ่นใช้ service อื่นอาจต้องเพิ่ม; 384px คงที่ (80mm พิมพ์ครึ่งซ้าย); เครื่อง Bluetooth Classic (ไม่ใช่ BLE) จะเชื่อมไม่ได้ (ข้อจำกัด Web Bluetooth — แจ้ง error ชัด). **Verification:** lint:errors 0 · unit 2634/2634 (+12) · e2e: smoke APP_BUILD = env flake เดิม (isolation ผ่านครบ 12/12 + build-sync markers 585 เขียว = markers ถูก; CI จริงผ่าน) · markers 585 (?v=585 ×4 + cache-v585 + version 5.69.56). **ไม่เพิ่ม npm dependency** (Web API + canvas ล้วน). **Status: ✅ MERGED+DEPLOYED (#163 `e649ce4` — CI Tests #1082 + Deploy #1080 เขียวบน sha เดียวกัน; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev (CONNECT 403) — Deploy success บน commit ที่ฝัง data-app-build=585 = หลักฐาน; owner hard-refresh ยืนยัน 585 บนเว็บจริง). owner test (Android): Settings→About→เชื่อมเครื่องพิมพ์→เห็นชื่อเครื่อง→ทดสอบพิมพ์ออกกระดาษ→ขายจริง 1 บิล→กด "🖨️ สลิป BT"→ไทย/ตัวเลข/ยอดถูก · iPhone: เปิด Settings เห็นข้อความ "ใช้ไม่ได้บน iOS" ไม่ crash.**

**Phase 584 (POS view sticky + JV header persist + settings nav listener — re-render/listener กินสถานะ · PR #162 `d6800a2` MERGED+DEPLOYED 2026-07-09 build 584 · 3 บั๊กอิสระ):** **B (accounting correctness §4.3 — สำคัญสุด):** `modules/accounting/journal_form.js` header (วันที่/ประเภท/คำอธิบาย :104/108-110/115) อยู่ใน DOM เท่านั้น (ไม่มี module state) → กด "+ เพิ่มบรรทัด" (:207-210) / ลบบรรทัด (:243-247) เรียก `renderJournalFormPage` rebuild `innerHTML` = 3 ช่องกลับ today/JV/ว่าง **เงียบ**; `_save` (:260-262 เดิม) อ่าน DOM หลัง re-render → JV โพสต์ด้วย**วันนี้**แม้ตั้ง period ก่อน (`validateJournalDate` วันนี้ผ่านเสมอ = **โพสต์ผิด period เงียบ** + `doc_no` prefix yyyymm ผิดเดือน). **แก้ (mirror `_lines`):** `let _header = {doc_date:"",doc_type:"JV",desc:""}` (ใกล้ :51); init `if(!_header.doc_date) _header.doc_date = today` (หลัง :83 — เฉพาะครั้งแรก คงค่าตอน re-render); template: date `value="${escAttr(_header.doc_date)}"`, doc_type option `${k===_header.doc_type?" selected":""}`, desc `value="${escAttr(_header.desc)}"`; header inputs → `_header` (date `change`/type `change`/desc `input`); `_save` อ่าน `_header` (single source ไม่พึ่ง DOM timing — change/input ยิงก่อน click save); reset `_header` คู่ `_lines` ที่ cancel (:203) + post-save success (:372). ★ ห้าม reset `_header` แบบไม่มีเงื่อนไขใน render (มีแต่ init guard). **A (UX รายวัน):** `pos.js`/`main.js` — `renderPosPage` (:237) รีเซ็ต `posView="home"`/numpad/quickPay ทุกครั้ง; `renderAll` (:1363) หลัง background `loadAllData` (deferred 10s–2min) เรียก renderPosPage → เด้ง home/ดีดสกอลล์ระหว่าง numpad/payment/search. **แก้ (mirror `refreshProductsPage`):** `let _lastPosCtx=null` + `_lastPosCtx=ctx` ใน renderPosPage; `export function refreshPosPage()` — `!_lastPosCtx→false`; `posView!=="home"→return true` (mid-flow short-circuit ไม่แตะ DOM); home → `renderPosView(_lastPosCtx)` (refresh แบนเนอร์, **ไม่แตะ** posView/numpadValue/quickPayAmount/`_checkoutKey`/`_resetCreditState`); `main.js` import +`refreshPosPage` (:7) + renderAll `if(currentRoute==="pos"&&refreshPosPage())return` ต่อจาก products (:1367). navigate จริงผ่าน `showRoute→renderPosPage` เต็ม = เริ่ม home เดิม (เปลี่ยนเฉพาะ background renderAll). **C (companion):** `modules/settings/index.js` `setupEventListeners` (:42) เพิ่ม `addEventListener("navigate-settings")` ใหม่ทุกครั้ง (call :36 render + :105 renderCurrentView ทุก navigate/goBack) → listener ซ้อน → navigate ทวีคูณ. **แก้:** `let _navBound=false` + `if(_navBound)return; _navBound=true` ก่อน addEventListener (call site คงไว้ = no-op หลังครั้งแรก; handler อ้าง goBack/navigateToView module-scope stable). **ไม่แตะ:** checkout/multi-pay/void/`_checkoutKey` logic · stock CAS · `_save` POST entry→lines→rollback · `validateJournalDate`/`date_guard` · `doc_no` seq · `auto_post` · RLS/SQL · `loadAllData` cap · line handlers `_lines` (คง `renderJournalFormPage(ctx)` — header รอดเพราะมาจาก `_header`). **Tests:** `jv_form_header_persist_guard`(6: `_header` state / `_save` ไม่อ่าน DOM+อ่าน `_header` / date value ผูก `_header` ไม่ใช่ `${today}` / option selected / init เฉพาะ guard (`= today` จุดเดียว) / reset 2 จุด) · `pos_view_sticky_guard`(4: export+false-when-no-ctx behavioral / ไม่แตะ posView·numpad·quickPay·_checkoutKey·_resetCreditState / guard `posView!=="home"`+ไม่เรียก renderPosPage เต็ม / main.js short-circuit ถัดจาก products) · `settings_nav_listener_once_guard`(2: `_navBound` guard ก่อน addEventListener / handler เดิม). companion `dashboard_readonly_guard` marker 584. guard เดิมเขียวครบ. **known-risk:** `_header` module state คงข้าม session ฟอร์ม (reset ที่ cancel+post-save + init guard เมื่อว่าง — ออกฟอร์มโดยไม่ cancel/save เช่น back-nav → `_header` ค้าง แต่ init guard ไม่ overwrite → เปิดใหม่เห็นค่าเดิม; acceptable, mirror `_lines` เดิมที่ก็ค้างแบบเดียวกัน) ; refreshPosPage home-refresh เรียก `renderPosView` (banner) — ถ้า future เพิ่ม input ที่ home ต้องทบทวน (ตอนนี้ home ไม่มี input ค้าง). **Verification:** lint:errors 0 (warnings = pre-existing baseline ของ 4 ไฟล์) · unit 2622/2622 (+12) · e2e: guard+unit เขียว, smoke APP_BUILD = env flake เดิม (container fake-headless — marker guard+build-sync e2e ที่ผ่านยืนยัน markers ถูก; CI จริงผ่าน) · markers 584 (?v=584 ×4 + cache-v584 + version 5.69.55). **Status: ✅ MERGED+DEPLOYED (#162 `d6800a2` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev (CONNECT 403) — owner ยืนยัน data-app-build=584 บนเว็บจริง (hard refresh)). owner smoke: (B) JV set วันที่ period ก่อน + desc → "+ เพิ่มบรรทัด" → 3 ค่าคงอยู่ → save doc_no ตรงเดือน · (A) POS อยู่ numpad/payment ตอน background reload → ไม่เด้ง home · (C) settings เมนูย่อย navigate ปกติ (ไม่ทวีคูณ).**

**Phase 583 (receipts-range-full-fetch — ปิดกลุ่ม money-display · PR #161 `704ef0b` MERGED+DEPLOYED 2026-07-09 build 583 · = AUDIT B5 receipts cap 50 ✅ ติ๊กออกแล้ว):** **บั๊ก:** หน้าใบเสร็จ (`modules/receipts.js` · `renderReceiptsPage:187`) summary "ยอดรวม" (`_receiptFinancialTotal` :256) + ค้นหา + Excel (`rcExportBtn` :374) + tab counts (`_receiptStatusCounts`) ทำบน `ctx.state.receipts` **cap 50** (:199, `loadAllData`) → ใบเสร็จ >50 = ยอดรวมขาด/ค้นใบเก่าไม่เจอ/Excel ไม่ครบ (เอกสารการเงิน อ้างบัญชี/ภาษี). **แก้:** (1) `range_fetch.js` เพิ่ม `fetchReceiptsSince(cutoffKey)` mirror `fetchExpensesSince` — `receipts?select=*&created_at=gte.{bufferedSince}&order=created_at.desc`, paginated, `""`→ทั้งหมด, คืน `{ok,rows}`/`{ok:false,error}`. (2) `receipts.js` async-load (mirror loyalty Phase 581): module state `_rcAllRows`/`_rcState`(idle|loading|loaded|error)/`_rcSeq`/`_rcKey` + `_rcCutoffKey()` (`_rcDateRange`→cutoff, all→"") + `_loadReceiptsRange(ctx)` (guard `_rcState!=="idle" && _rcKey===rangeKey → return` = range เดิมไม่ refetch; seq-guard stale; re-render เมื่อเสร็จ; error→null cache). `renderReceiptsPage`: kick `_loadReceiptsRange` เมื่อ `_rcState==="idle" || _rcKey!==_rcDateRange` (range เปลี่ยน→refetch, search/tab→ไม่) + **`const receipts = _rcLoaded ? _rcAllRows : (ctx.state.receipts||[])`** (`_rcLoaded = loaded && _rcAllRows && _rcKey===_rcDateRange`) — เปลี่ยน source **จุดเดียว** (:199) → `scopedReceipts`/counts/summary/`filtered`/Excel อ่าน set เต็ม; client-side date filter (`dateBkk`, `_receiptMatchesSearchDate`) เดิมคง exact bound (buffer -2 วัน = superset); ป้ายใต้ header: loaded→เงียบ / loading→"⏳ ~50 ใบล่าสุด" / error→"⚠️ ~50 ใบล่าสุด" + ปุ่ม `rcRetryBtn` (reset idle+key→refetch). **★ verify Excel:** `rcExportBtn` (:374) map จาก `filtered` (closure) ← `scopedReceipts` ← `receipts` = `_rcAllRows` เมื่อ loaded → Excel ครบตาม range จริง (ไม่ใช่ cap 50). ยืนยันแล้ว. **ไม่แตะ:** mutation handlers (multi-pay save `_multiPayGuard`/void/repost — Phase 574/575) · `_repostReceiptJvIfChanged` · `logActivity` · `loadAllData` cap · receipt detail/print · RLS/SQL (20 refs `_appXhrPatch`/guard เดิมครบ). **Tests:** `tests/receipts_range_fetch_guard.test.js` ใหม่ (9: behavioral `fetchReceiptsSince` — paginate >1000/error ไม่เงียบ/`""`ไม่มี lower bound/bufferedSince; source — import+state, `receipts` loaded-else-state + summary/filter บน set เต็ม ไม่ aggregate จาก ctx.state.receipts, cache keyed `_rcDateRange`+idle/seq-guard, error→ป้าย+retry, mutation handlers ไม่แตะ). guard เดิมเขียวครบ (`multi_payment_guard`/`receipts_multipay_patch_guard`/`receipt_multipay_repost_guard`). companion `dashboard_readonly_guard` marker 583. **known-risk:** cache ไม่ auto-refresh หลัง mutation (multi-pay/void) บนหน้า — mutation handler เรียก `renderReceiptsPage` (re-render จาก cache) แต่ `_rcState` ยัง loaded = ไม่ refetch → ยอดรวมอาจไม่รวม mutation ล่าสุดจนกด range/retry (เท่าพฤติกรรมเดิม state.receipts จน loadAllData; acceptable — เอกสารเดิมก็ต้อง reload); range=all + ร้านใหญ่มาก = fetch หนักขึ้น (paginated, correctness > speed). **Verification:** lint:errors 0 (2 warnings `_rcState` = async-cache pattern เดียวกับ loyalty/dashboard) · unit 2610/2610 (+9) · e2e 14/14 · markers 583 (?v=583 ×4 + cache-v583 + version 5.69.54). **Status: ✅ MERGED+DEPLOYED (#161 `704ef0b` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev — owner ยืนยัน data-app-build=583). owner smoke: ร้าน >50 ใบเสร็จ → ยอดรวมครบ · ค้นใบเก่า (เกิน 50) เจอ · Excel ครบตาม range · สลับ range→โหลดใหม่ · สลับ tab/พิมพ์ค้นหา→ไม่กระพริบโหลด.**

**Phase 582 (sales-ranking-full-fetch — top_customers + profit_by_product · money-display · PR #160 `49304c8` MERGED+DEPLOYED 2026-07-08 build 582 · = AUDIT sales-ranking ✅ ติ๊กออกแล้ว):** **บั๊ก:** 2 หน้ารายงานจัดอันดับอ่านจาก `state.sales` **cap 50** → ranking/ยอดเพี้ยนเมื่อ >50 บิล/ช่วง. `top_customers.js:41` → อันดับลูกค้า + `totalRevenue`/top5; `profit_by_product.js:64` → กำไรรายสินค้า. **แก้ (mirror `profit_report.js:74`):** ทั้งสองหน้ามี period selector → `cutoffKey` (all→`""`) → `await fetchSalesSince(cutoffKey)` ดึงเต็มช่วง (paginated); `visibleSalesForRole(_ok ? _salesRes.rows : (state.sales||[]), ...)` role isolation ครอบ fetched-or-fallback; client-side date filter เดิมคง exact bound `.filter(s => !cutoffKey || slice(0,10) >= cutoffKey)` (helper buffer -2 วัน = superset); fetch ล้ม → fallback `state.sales` + `showToast(warn)` + ป้าย ⚠️ "จาก ~50 บิลล่าสุด" (ไม่ 0/เพี้ยนเงียบ). top_customers restructure โหลด sales ก่อน sale_items (`_tcPeriodSnap` guard คง); profit_by_product wrap async IIFE (sales ก่อน sale_items, error+retry branch เดิมคง) + `_ppRenderBody(...,salesFetchOk)`. **ไม่แตะ:** `fetchSaleItemsForSaleIds` · money mutation · `loadAllData` cap · `visibleSalesForRole` · RLS/SQL. **★ verify ข้อ 4 (follow-up ไม่แก้):** `customers.js:137` `getCustomerTier(c.id, state.sales)` (tier จาก cap 50) · `cash_recon` per-day (flagged Phase 578) · `customer_dashboard` mySales (per-customer cap 50, minor) · **receipts.js ยอดรวม/ค้นหา/Excel cap 50 = Phase 583 (known)**. ตรวจแล้วไม่ใช่ปัญหา: `credit_tracker` ใช้ `fetchCreditSales` แล้ว · `dead_stock` เลี่ยง cap แล้ว · `sales.js`/`serials.js` = list/server-search. **Tests:** `tests/sales_ranking_fetch_guard.test.js` ใหม่ (6 source-regex: import ทั้งคู่ · sales จาก fetch(cutoffKey)+fallback+ป้าย · visibleSalesForRole ครอบ fetched · date filter exact bound คง · ไม่แตะ fetchSaleItemsForSaleIds). guard เดิมเขียวครบ (`sales_fetch_guard`/`financial_cancelled_guard`). companion `dashboard_readonly_guard` marker 582. **known-risk:** fetch เต็มช่วงใหญ่ (year/all) ช้าลงเล็กน้อย (paginate) แต่ correctness > speed; ป้าย/fallback → worst case = พฤติกรรมเดิม (cap 50). **Verification:** lint:errors 0 · unit 2601/2601 (+6) · e2e: guard+unit เขียว, smoke APP_BUILD = env flake เดิม (container fake-headless — marker guard+build-sync e2e ที่ผ่านยืนยัน markers ถูก; CI ผ่าน) · markers 582 (?v=582 ×4 + cache-v582 + version 5.69.53). **Status: ✅ MERGED+DEPLOYED (#160 `49304c8` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev — owner ยืนยัน data-app-build=582). owner smoke: ร้าน >50 บิล/ช่วง → top_customers อันดับ/ยอดครบกว่าเดิม; profit_by_product กำไรรายสินค้าครบ (ไม่ตัดที่ 50).**

**Phase 581 (admin-loyalty-full-fetch — companion Phase 580, staff เห็นแต้มรวมครบ · PR #159 `760e7ae` MERGED+DEPLOYED 2026-07-08 build 581 · = ครึ่ง admin ที่เหลือของ AUDIT "Phase 578 loyalty" ✅ ติ๊กออกแล้ว):** **บั๊ก:** หน้า loyalty ฝั่งแอดมิน (`modules/loyalty.js` · `renderLoyaltyPage:378`) summary (`customersWithPoints`/`totalEarned`/`totalRedeemed`/`totalRemaining`/`totalValue` :389-409) + `renderSummaryTab` (:521) + `showPointHistory` per-customer อ่านจาก `state.loyaltyPoints` **cap 500** (`loadAllData`) → loyalty rows รวม >500 = ตัวเลขสรุปต่ำกว่าจริง + **"คงเหลือ" ติดลบได้** (earn เก่าหลุดหน้าต่างก่อน redeem). staff อ่าน `loyalty_points` ได้ (customer-deny Phase 505 เฉพาะ role customer). **แก้ (`loyalty.js` เท่านั้น):** module state `_loyAllRows`/`_loyState`(idle|loading|loaded|error)/`_loySeq` + `_loadAllLoyalty(ctx)` async (mirror dashboard Phase 562): `fetchAllRowsRaw((off,lim)=>loyalty_points?select=*&order=created_at.desc&limit&offset, headers)` (paginate, throw บน error) · idle-guard `if(_loyState!=="idle")return` (กัน refetch loop) · seq-guard `if(seq!==_loySeq)return` (กัน stale) · re-render เมื่อเสร็จ (เช็ค page-loyalty) · error → `_loyState="error"`+`_loyAllRows=null`. `renderLoyaltyPage`: kick `_loadAllLoyalty(ctx)` เมื่อ idle + **`const loyaltyPoints = (_loyState==="loaded"&&_loyAllRows) ? _loyAllRows : (state.loyaltyPoints||[])`** — เปลี่ยน source **จุดเดียว** (:389) → customersWithPoints/totalEarned/totalRedeemed + `renderSummaryTab(loyaltyPoints,...)` + `showPointHistory` อ่านชุดเต็มพร้อมกัน (fallback state ระหว่างโหลด/error); ป้ายใต้ h1: loaded→เงียบ / loading→"⏳ ~500 ล่าสุด" / error→"⚠️ ~500 ล่าสุด (อาจต่ำกว่าจริง/คงเหลือเพี้ยน)" + ปุ่ม `loyRetryBtn` (reset idle→refetch). **★ แต้มฝั่งลูกค้าแก้แล้ว proxy Phase 580 — เฟสนี้เฉพาะหน้าแอดมิน.** **ไม่แตะ:** `getCustomerPoints` (:34, redeem UX precheck — server-enforce Phase 540, prompt ระบุ out of scope; ยัง cap แต่ precheck เท่านั้น real enforce = atomic RPC) · `earnPoints`/`redeemPoints` · `reverseEarnedPointsForSale` (อ่าน DB ตรง `ref_id=eq` อยู่แล้ว) · `loyalty_settings` · `loadAllData` cap (หน้าอื่นพึ่ง) · RLS/SQL. **Tests:** `tests/admin_loyalty_fetch_guard.test.js` ใหม่ (5 source-regex: import+state · `loyaltyPoints` loaded-else-state + summary/tab อ่านจากตัวแปรนั้น ไม่ใช่ `state.loyaltyPoints.filter` ตรง · idle+seq guard+paginate urlFor · error→ป้าย+retry / loaded→เงียบ · ไม่แตะ getCustomerPoints/reverse). guard เดิมเขียวครบ (`loyalty_settings_admin_guard`/`loyalty_history_modal_listener`/`loyalty_admin_check`/`loyalty_redeem_atomic`/`loyalty_reverse_sale` = 53). companion `dashboard_readonly_guard` marker 581. **known-risk:** เอา `clearLoyaltyAdminCache` export ออก (loyalty admin = global data, ไม่มี cross-user leak แบบ customer points; ไม่ wire main.js = ไม่ dead code); cache ไม่ auto-refresh หลัง manual earn/redeem บนหน้า (เท่าพฤติกรรมเดิม state.loyaltyPoints — retry/reload ได้); `getCustomerPoints` precheck ยัง cap 500 (follow-up ได้ถ้าต้องการ — redeem จริงปลอดภัยแล้ว server RPC). **Verification:** lint:errors 0 (2 warnings = async-cache pattern เดียวกับ `_dashSalesState`) · unit 2595/2595 (+5) · e2e 14/14 · markers 581 (?v=581 ×4 + cache-v581 + version 5.69.52). **Status: ✅ MERGED+DEPLOYED (#159 `760e7ae` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev — owner ยืนยัน data-app-build=581). owner smoke: admin ร้าน >500 loyalty rows → เปิดหน้า loyalty → totalEarned/Remaining ครบ/ตรงกว่าเดิม (ไม่ตัดที่ 500) + ไม่ติดลบผิดปกติ.**

**Phase 580 (customer-loyalty-balance-proxy — ลูกค้าเห็นแต้มจริง · PR #158 `794963f` MERGED+DEPLOYED 2026-07-08 build 580 · = AUDIT customer loyalty balance ✅ ติ๊กออกแล้ว):** **บั๊ก:** ลูกค้าอ่าน `loyalty_points` ตรงไม่ได้ (Phase 505 RESTRICTIVE deny — table GROUP A) → `state.loyaltyPoints` ว่างสำหรับลูกค้า → `customer_dashboard` แต้มโชว์ **0 เสมอ** (บั๊กตั้งแต่ Phase 505; `:132-133` `state.loyaltyPoints.filter(...).reduce` = 0). **แก้ (Cloudflare Function proxy — pattern ning-memory Phase 569 / verify-slipok customer-JWT):** **(1) `functions/api/v1/loyalty-balance.js` ใหม่ (GET):** identity จาก `data.user.email` (middleware-verified JWT) → `phoneFromEmail` (`<phone>@phone.boonsook.local` → digit phone; staff/non-customer → `null` → 403) → service-role `customers?phone=eq.<phone>&select=id&order=id.asc&limit=1` (mirror customer_dashboard first-match) → `loyalty_points?customer_id=eq.<id>&select=type,points,ref_type,created_at,note` **paginate** (offset/limit loop, break < PAGE) → `balance = Σ(type=earn) − Σ(type=redeem)` (mirror `getCustomerPoints` loyalty.js:34) + `recent` 20 ล่าสุด; ลูกค้าใหม่ไม่มี customers record → `{balance:0}` (0 จริง ตรวจครบ). **★ security invariant:** identity derive จาก JWT ฝั่ง server **เท่านั้น** — ไม่มี `searchParams.get`/`request.json` (ไม่รับ phone/customer_id จาก request → กันลูกค้า A อ่านแต้ม B); `authMode==="ning_agent"` → 403; `SUPABASE_SERVICE_ROLE_KEY` required (`getServiceAuth({})` → 500, ไม่ fallback anon/publishable); error sanitize `logServerError`/`clientError` (ไม่ leak raw). **(2) `_middleware.js`:** +`/api/v1/loyalty-balance` ใน `REQUIRE_AUTH_ENDPOINTS` + `RATE_LIMITS` (60/60s) — **ไม่**อยู่ `STAFF_ONLY`/`NING_AGENT`/`REPORT_ONLY` (customer JWT verify แล้ว set `data.user={id,email,role:null}` → handler). **(3) `customer_dashboard.js`:** module state `_loyalty`/`_loyaltyState`/`_loyaltyKey` (keyed phone); render kick `_loadLoyaltyBalance(userPhone,ctx)` เมื่อ idle/สลับ account (async ไม่ await → re-render เมื่อเสร็จ, stale-guard `_loyaltyKey!==phone`); hero badge `:295` (`_pointsBadge` = ยอด/…/—) + points tab (`:712`) loaded→ยอดจริง+recent (isEarn=`type==="earn"`), loading→skeleton, error→ปุ่ม `custLoyaltyRetry` (ไม่โชว์ 0 หลอก); `clearCustomerDashboardState` (logout) เคลียร์ `_loyalty` cache (กัน A→B). **เลิกพึ่ง `state.loyaltyPoints`** ในจุดแต้ม (grep codeOnly = 0 ref). **ไม่แตะ:** redeem flow (`redeem_loyalty_points_atomic` server-enforce) · `loyalty.js` แอดมิน (cap 500 = เฟสแยก) · RLS/SQL/auth flow · customers/service_jobs read เดิม. **Tests:** `tests/loyalty_balance_proxy_guard.test.js` ใหม่ (9: **behavioral** `phoneFromEmail` (customer→phone / staff/empty/non-digit→null) + `getServiceAuth` (no key→500, มี key→service-role, ไม่ fallback anon); **source-regex** identity JWT-only (no searchParams/request.json) · balance Σearn−Σredeem+paginate · middleware REQUIRE_AUTH+RATE_LIMITS ไม่ STAFF/NING/REPORT · ning_agent→403 · dashboard เรียก proxy+JWT+error retry+cache keyed phone+logout clear · sanitize logServerError/clientError). guard เดิมเขียวครบ (`ning_memory_proxy_guard` / middleware guards). companion `dashboard_readonly_guard` marker 580. **known-risk:** edge หลาย customers record phone เดียว → ใช้ตัวแรก (order id.asc) mirror customer_dashboard (phone = identity เดียว — record ซ้ำควรไม่มี); ต้องมี `SUPABASE_SERVICE_ROLE_KEY` ใน Cloudflare env (มีแล้วจาก ning-memory Phase 569 — ไม่ต้องตั้งใหม่); ยังไม่แก้ hero badge ของ role อื่น (เฉพาะ customer path). **Verification:** lint:errors 0 · unit 2590/2590 (+9) · e2e: guard+unit เขียว, smoke APP_BUILD = env flake เดิม (intermittent container fake-headless — marker guard+build-sync e2e ที่ผ่านยืนยัน markers ถูก; CI จริงผ่าน) · markers 580 (?v=580 ×4 + cache-v580 + version 5.69.51). **Status: ✅ MERGED+DEPLOYED (#158 `794963f` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev — owner ยืนยัน data-app-build=580). owner smoke: ลูกค้าที่มีแต้มจริง login → แท็บแต้มสะสม → เห็นยอดจริง (ไม่ใช่ 0); สลับ account ลูกค้าคนละคน → เห็นแต้มตัวเองเท่านั้น (isolation). ★ ต้องมี SUPABASE_SERVICE_ROLE_KEY ใน Cloudflare env (มีแล้วจาก ning-memory Phase 569).**

**Phase 579 (period-close-readiness-fail-closed — accounting blocker · PR #157 `159cacd` MERGED+DEPLOYED 2026-07-08 build 579 · = AUDIT period-close readiness ✅ ติ๊กออกแล้ว):** **บั๊ก:** close-readiness ของ "ปิดงวดบัญชี" (`modules/accounting/periods.js` · `fetchCloseReadiness`, prompt เรียก computeReadiness) เดิม **2 จุด "fail-safe-green"** + cap 1000 → การ์ดโชว์ "ปิดงวดได้ ✅" ทั้งที่ตรวจไม่ครบ. **(a) completeness (:116-131):** fetch source ids ในเดือน**ไม่มี limit** (PostgREST cap 1000) + `id=in.(1000 ids)` URL ยาวเสี่ยง fail → `!vr.ok`/`!rr.ok continue` + `catch{ข้ามเงียบ}` → `orphanSrc` คง 0 = เขียว. **(b) orphan JV (:133-150):** `journal_entries` fetch ไม่มี limit (cap 1000) + existence check `er.ok ? new Set(...) : new Set(uniq)` (fail → **ถือว่ามีครบ**) + `catch{}` เงียบ → `orphanJV` คง 0 = เขียว. เทียบ **(c) service (:152-160)** ที่ทำ**ถูก**: fetch fail → `serviceUnknown=true` → ไม่เขียว. **แก้ (periods.js เท่านั้น):** `import { fetchAllRowsRaw } from "../fetch_paginated.js"` (paginated read-only, throw บน error — pattern เดียวกับ `je_fetch.js` โมดูลเดียวกัน). (a) paginate source-id fetch (เกิน 1000 ได้ครบ) + batch `id=in.(...)` chunk `ID_CHUNK=120` (กัน URL ยาว) → `catch → srcUnknown=true` (fail-CLOSED, เลิก `catch{ข้ามเงียบ}`; เลิก `!vr.ok/!rr.ok continue`); "ไม่มี source ในเดือน" (empty) = เขียวจริง (ไม่ใช่ fail). (b) paginate `journal_entries` + batch existence check chunk 120 → `catch → jvUnknown=true` (เลิก `er.ok? Set : new Set(uniq)` fail-safe-green). **return** += `srcUnknown,jvUnknown`; **ready** += `!srcUnknown && !jvUnknown`; **`_monthNeedsAttention`** (:33 = prompt เรียก "isReadinessBlocking") += `srcUnknown || jvUnknown`; **`_readinessLinesHtml`** เพิ่มบรรทัด ⚠️ unknown (สีเหลือง `#b45309` "ตรวจบิล/orphan JV ไม่ได้ (unknown) — ยังไม่ยืนยันว่าครบ") + เงื่อนไข "ครบ ✅" += `!srcUnknown && !jvUnknown`. **★ นอก scope ที่ prompt ระบุ แต่จำเป็น (Iron Rule #1 grep ทุก reader):** lock-confirm dialog `:409-412` (consumer ที่ 3 ของ rd flags — สร้าง issues[] + "✅ พร้อมปิด") ต้องเพิ่ม `srcUnknown/jvUnknown` ด้วย ไม่งั้น confirm dialog ยังบอก "✅ พร้อมปิด" หลอกตอน unknown → เพิ่ม 2 บรรทัด issues.push. **ไม่แตะ:** Dr=Cr balance check (:88 คนละเรื่อง) · period lock trigger · การโพสต์ JV · (c) service ที่ทำถูก · `_classifyOrphan` logic · RLS/SQL. **Tests:** `tests/period_readiness_failclosed_guard.test.js` ใหม่ (9: **behavioral** import `_monthNeedsAttention`/`_readinessLinesHtml` (pure, ไม่มี top-level window) — green→needsAttn false+"ครบ ✅" · srcUnknown/jvUnknown→needsAttn true+ไม่มี "ครบ ✅"+บรรทัด unknown; **source-regex** — (a) `catch→srcUnknown` ไม่เหลือ catch เงียบ/`!ok continue` · (b) `catch→jvUnknown` ไม่เหลือ `er.ok? Set : new Set(uniq)` · paginate `fetchAllRowsRaw`+`ID_CHUNK`+offset/order · return/ready/`_monthNeedsAttention`/confirm-issues รวม unknown · (c) service คงเดิม). guard เดิมเขียวครบ (`period_lock_delete_guard` / `je_fetch_guard` / `accounting_readiness_service_scope`). companion `dashboard_readonly_guard` marker 579. **known-risk:** pre-existing lint warning "unused eslint-disable :308" มีบน origin/main อยู่แล้ว (ไม่ใช่จาก diff นี้ — ไม่แตะ นอก scope); readiness fetch ตอนนี้ paginate = ช้าลงเล็กน้อยสำหรับงวดใหญ่ (แต่ correctness > speed สำหรับ gate ปิดงวด); การ์ดปกติ (ข้อมูลครบ) พฤติกรรมเท่าเดิม (เขียว). **Verification:** lint:errors 0 (warnings baseline) · unit 2581/2581 (+9) · e2e 14/14 (APP_BUILD flake ไม่ทริปรอบนี้) · markers 579 (?v=579 ×4 + cache-v579 + version 5.69.50). **Status: ✅ MERGED+DEPLOYED (#157 `159cacd` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev — owner ยืนยัน data-app-build=579). owner smoke: เปิดหน้าปิดงวด → งวดข้อมูลครบจริง = เขียว ✅ เหมือนเดิม; network สะดุด/ตรวจไม่ได้ = ⚠️ unknown "ยังไม่ยืนยันว่าครบ" (ไม่เขียวหลอก) + confirm dialog ขึ้น "ตรวจบิล/orphan JV ไม่ได้".**

**Phase 578 (profit-expense-full-fetch — follow-up Phase 577 · money-display · PR #156 `ac3d943` MERGED+DEPLOYED 2026-07-08 build 578):** **บั๊ก:** เสียบ helper `range_fetch` (Phase 577) 2 จุดที่เหลือ. (1) `profit_report.js` sales ดึงเต็มผ่าน `fetchSalesSince` (:74) แล้ว แต่ `expensesInRange` (:111) กรองจาก `state.expenses` cap 200 → **"กำไรสุทธิ" สูงเกินจริง** (revenue เต็ม − expense ขาด) เมื่อ >200 รายจ่าย/ช่วง. (2) `expense_overview.js` raw `fetch` (:60) ไม่ paginate → >1000 รายจ่าย/ช่วง = ยอดรวม/เฉลี่ย/Top ขาดเงียบ. **แก้:** (1) `profit_report.js`: `import { fetchExpensesSince } from "./range_fetch.js"`; `const [_salesRes, _expRes] = await Promise.all([fetchSalesSince(fromDate), fetchExpensesSince(fromDate)])` (ขนาน); sales fail = hard-stop เดิม (return), **expenses fail = `_fetchedExpenses = state.expenses` + `showToast(warn "กำไรสุทธิอาจสูงกว่าจริง")`** (ไม่ 0 เงียบ — mirror pattern sale_items warn :93); `expensesInRange` กรองจาก `_fetchedExpenses` (client-side `fromTs/toTs` เดิมคง exact boundary → helper buffer -2 วัน harmless). (2) `expense_overview.js`: `import fetchExpensesSince`; แทน raw fetch ด้วย `const _res = await fetchExpensesSince(startStr)`, `!_res.ok` → `renderError`+`eoRetryBtn` เดิม (auth 401/403 hint คงไว้ผ่าน regex `\b(401|403)\b` บน `_res.error`); **★ re-filter `_data = _res.rows.filter(e => slice(0,10) >= startStr)`** — expense_overview เดิม**ไม่มี** client-side date filter (sum ทุก row ที่ query คืน) → ถ้าเสียบตรง ๆ buffer -2 วันจะ **over-count 2 วัน** ก่อนช่วง → re-filter exact lower bound (ตาม contract helper: buffer = candidate superset เท่านั้น); ลบ `cfg/token/headers` ที่ไม่ใช้แล้ว (helper อ่าน SUPABASE_CONFIG เอง). **READ-ONLY** ไม่แตะ money mutation/`auto_post`/P&L·TB·BS/`loadAllData` caps/`range_fetch.js` (helper เสร็จแล้ว)/RLS. **★ verify ข้อ 3 (grep money-sum จาก state cap ที่เหลือ — flag follow-up ไม่แก้):** `top_customers.js` + `profit_by_product.js` (ไม่มี `fetchSalesSince` → sum จาก `state.sales` cap 50 → ranking/ยอดขาด) · `expenses.js:57,63,86` (month expense total จาก `state.expenses` cap 200 — income card ใช้ `fetchSalesSince` แล้ว แต่ expense total ยังไม่) · `cash_recon.js:49` (per-day cash-in/out จาก `state.sales`/`state.expenses` cap — เสี่ยงต่ำ per-day แต่ >50 บิล/วัน undercount; รับ `expensesInput` param = caller inject ได้). **Tests:** `tests/profit_expense_fetch_guard.test.js` ใหม่ (9 source-regex: profit_report import+Promise.all+sales hard-stop คงเดิม+expenses fallback+warn+`expensesInRange` จาก `_fetchedExpenses`+exact boundary คง · expense_overview ไม่เหลือ raw fetch+`fetchExpensesSince(startStr)`+`eoRetryBtn` คง+re-filter `>= startStr`+`monthsAgoStartBkk` คง+ลบ headers; behavioral ของ `fetchExpensesSince` cover แล้วใน `income_dashboard_fetch` Phase 577). guard เดิมเขียวครบ (`sales_fetch_guard` profit_report patterns / `financial_cancelled_guard` visibleSalesForRole / `month_bounds_bkk_guard` monthsAgoStartBkk / `finance_audit_92_63`). companion `dashboard_readonly_guard` marker 578. **known-risk:** expense_overview เดิมไม่มี date re-filter — เพิ่ม re-filter แล้ว (ตรวจ format `monthsAgoStartBkk`→`YYYY-MM-01` ISO string-comparable ✓); expenses fail ที่ profit_report = fallback cap 200 (undercount แต่มี warn — ยอมรับ, ดีกว่า hard-stop ทั้ง report). **Verification:** lint:errors 0 · unit 2572/2572 (+9) · e2e: guard+unit เขียว, smoke APP_BUILD = env flake เดิม (intermittent: 1 fail 1 pass เมื่อรันเดี่ยว = container fake-headless contention ไม่ deterministic กับโค้ด; marker guard+build-sync e2e ที่ผ่านยืนยัน markers ถูก; CI จริงผ่าน) · markers 578 (?v=578 ×4 + cache-v578 + version 5.69.49). **Status: ✅ MERGED+DEPLOYED (#156 `ac3d943` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev — owner ยืนยัน data-app-build=578). owner smoke: ร้าน >200 รายจ่าย/ช่วง → profit_report กำไรสุทธิต่ำลง/ตรงความจริงขึ้น (รายจ่ายครบ) · expense_overview ยอดรวมครบ.**

**Phase 577 (income-dashboard-full-fetch — money-display B1+B3 · undercount blocker · PR #155 `72aafb7` MERGED+DEPLOYED 2026-07-08 build 577 · = AUDIT "Phase 576 income_overview + dashboard" ✅ ติ๊กออกแล้ว):** **บั๊ก:** ตัวเลขเงินหน้า "ภาพรวมรายได้" (`income_overview.js` ทั้งหน้า: POS/web/service + กราฟรายเดือน) จาก `state.sales` cap 50 + `state.serviceJobs` cap 50; Dashboard รายจ่าย (`periodExpenseTotal :448` / `monthExpenseTotal :465` จาก `state.expenses` cap 200) + รายได้บริการ (`todayServiceIncome :458` / `monthServiceIncome :467`) + ออเดอร์เว็บ (`:435` จาก `state.serviceJobs` cap 50) → **กำไรสุทธิเดือน/ตัวเลขรายได้โชว์ต่ำกว่าจริงเงียบ ๆ** เมื่อร้านมี >200 รายจ่าย/>50 งาน (Phase 562 แก้เฉพาะ sales ผ่าน `_dashSalesRows`). **แก้ (ขยาย pattern Phase 562/492):** (1) `modules/range_fetch.js` **ใหม่** — paginated read-only (1000/page, mirror `sales_fetch.js`), `import { bufferedSince }` จาก sales_fetch (single-source date-math) + re-export; `fetchExpensesSince(cutoff)` (filter `expense_date.gte`) + `fetchServiceJobsSince(cutoff)` (**PostgREST OR `or=(closed_at.gte.SINCE,created_at.gte.SINCE)`** = superset ของ income-date `closed_at` (Phase 542 cash-basis ตรง JV `auto_post.js:839`) + web-order-date `created_at`; NULL closed_at ยัง match created_at); คืน `{ok,rows}`/`{ok:false,error}` ไม่เงียบ. (2) `dashboard.js`: บล็อก aux **แยกจาก sales** (`_dashExpenseRows`/`_dashServiceRows`/`_dashAuxState`/`_dashAuxSeq`) — **★ deviation จาก prompt "Promise.all กับ sales, seq เดียวกัน":** ถ้ารวม sales เข้า Promise.all จะลบ literal `await fetchSalesSince(_yearStart)` ที่ `dashboard_sales_fetch_guard:53` assert = guard แดง (test #4 ห้าม weaken) → ทำบล็อกแยก (fire-and-forget IIFE ขนานกันจริง behavior เท่ากัน); `_expensesForAgg`/`_serviceForAgg` = fetched rows เมื่อ `_dashAuxState==="loaded"` else `state` fallback; ป้าย trend `:790` + KPI "กำไรสุทธิเดือนนี้" badge ⚠️/⏳ ตาม aux state. (3) `income_overview.js` pure-state → fetch-backed: cache keyed user/role/ปี, idle→skeleton+โหลด `fetchSalesSince`+`fetchServiceJobsSince` (yearStart cutoff ครอบ today/month/year+กราฟ, filter ช่วง client-side เดิม), loaded→ยอดเต็ม, error→fallback state+banner ⚠️+ปุ่ม `ioRetryBtn`; แก้ header comment + footer `:146` (loaded=ใกล้ P&L). **READ-ONLY** — raw `fetch()` อยู่ใน `range_fetch.js` ที่เดียว → dashboard/income เรียกผ่าน helper (`fetchSalesSince(`/`fetchServiceJobsSince(` **ไม่ทริป** `\bfetch\s*\(` — พิสูจน์เชิงประจักษ์) = guard `income_overview_guard`/`dashboard_sales_fetch`/`dashboard_readonly` เขียวไม่ต้องแตะ. **★ verify ข้อ 4 (follow-up ไม่แก้ในเฟสนี้ — เกิน scope):** `profit_report.js:111` netProfit ใช้ `state.expenses` cap 200 (revenue เต็มจาก fetchSalesSince แล้ว แต่ expense ยังขาด → **net profit เกินจริง**) + `expense_overview.js:60` fetch แต่ไม่ paginate — **ทั้งคู่ใช้ `fetchExpensesSince` ได้** (แนะทำเฟสถัดไป). **ไม่แตะ:** money mutation/`auto_post`/P&L·TB·BS (paginate อยู่แล้ว)/`loadAllData` caps (หน้าอื่นพึ่ง)/RLS/operational job-lists (todayJobs/overdue/activeJobs/pendingReview — ตั้งใจ recent)/LINE daily-summary (:1407 คนละ path). **Tests:** `tests/income_dashboard_fetch_guard.test.js` ใหม่ (15: behavioral mock-fetch — paginate >1000/error ไม่เงียบ/bufferedSince/service OR filter closed_at+created_at/fail-closed · source dashboard — `_*ForAgg` loaded-else-state/aux idle-seq-guard/error nulls/sales block intact · source income_overview — helper import/skeleton/loaded-else-state/⚠️ banner+retry/serviceIncomeDate·created_at คงเดิม); **update** `dashboard_income_guard` source token `state.serviceJobs`→`_serviceForAgg` (คง `serviceIncomeDate` invariant — ไม่ weaken); companion `dashboard_readonly_guard` marker 577. **known-risk:** aux fetch แยก seq จาก sales → sales กับ aux อาจ loaded คนละจังหวะ (re-render 2 ครั้ง — ยอมรับ, ไม่กระพริบเพราะ fallback state ระหว่างรอ); service_jobs ไม่ role-filter (dashboard เดิมอ่าน state.serviceJobs ตรง ๆ — คงพฤติกรรม). **Verification:** lint:errors 0 (warnings = baseline async-cache pattern เดียวกับ `_dashSalesState`) · unit 2563/2563 (+15) · e2e: guard+unit เขียว, smoke APP_BUILD fail เฉพาะรันรวมใน container (fake headless-shell contention — ผ่าน 3/3 เมื่อรันเดี่ยว + baseline build 576 ก็ fail เท่ากัน = env flake ไม่ใช่ regression; CI จริงผ่าน) · markers 577 (?v=577 ×4 + cache-v577 + version 5.69.48). **Status: ✅ MERGED+DEPLOYED (#155 `72aafb7` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev — owner ยืนยัน data-app-build=577 บนเว็บจริง). 🔜 follow-up เฟสถัดไป (จาก verify ข้อ 4): `profit_report.js:111` + `expense_overview.js:60` ใช้ `fetchExpensesSince` (paginated) — ตัด undercount รายจ่ายในงบกำไร/หน้าภาพรวมรายจ่าย. owner smoke: ร้าน >50 บิล/>200 รายจ่าย/เดือน → ภาพรวมรายได้ "เดือนนี้/ปีนี้" สูงขึ้น/ครบกว่าเดิม + ใกล้ P&L · Dashboard กำไรสุทธิเดือน = รายได้เต็ม − รายจ่ายเต็ม.**

**Phase 576 (quotations-items-integrity — กันลบรายการทั้งใบแบบเงียบ · data-loss blocker · PR #154 `56a89e3` MERGED+DEPLOYED 2026-07-08 build 576 · = AUDIT "Phase 575 quotations items" ✅ ติ๊กออกแล้ว):** **บั๊ก (2 จุดเสริมกัน):** (1) โหลด `quotation_items` ใบเดิมล้ม → `catch { _lineItems = [] }` **เงียบ** 4 จุดใน `modules/quotations.js` (pending-preview ~:200 / `openEditForm` / `openPreview` / `convertToDeliveryInvoice`) — user ไม่รู้ว่าฟอร์ม/เอกสารที่เห็นว่างเพราะโหลดพัง (2) `saveQuotationFull` edit path: `xhrDelete("quotation_items")` fire-and-forget + loop `xhrPost` ไม่เช็คผล → toast สำเร็จเสมอ. **scenario ร้ายสุด:** เน็ตสะดุดตอนกดแก้ไข → ฟอร์มเปิดมาว่าง → user เพิ่มของ 1 ชิ้นแล้วกดบันทึก → DELETE รายการทั้งใบ + insert เฉพาะที่เห็น + toast สำเร็จ = **ใบเสนอราคาหายถาวรแบบเงียบ**. **แก้ (quotations.js เท่านั้น):** module flag `_lineItemsLoadFailed` — set true ในทุก catch โหลด-items ที่มี save/document ตาม (edit/preview/convert), reset false เมื่อโหลดสำเร็จ/เปิดใบใหม่ (qtAddBtn·qtEmptyAddBtn·air-draft)/ออกฟอร์มกลับ list/บันทึกสำเร็จ; ทุก catch มี toast "⚠️ โหลดรายการสินค้าไม่สำเร็จ..." (เลิกเงียบ — pending-preview :200 read-only จึงแค่ toast ตาม spec); `openPreview` โหลดล้ม → ยกเลิกการเปิด (กลับ list + return) / convert โหลดล้ม → ยกเลิกการแปลง (return ก่อน `xhrPost delivery_invoices`) — เอกสาร 0 รายการห้ามเกิด; `saveQuotationFull`: (a) guard `_editingId && _lineItemsLoadFailed` → toast "ห้ามบันทึกทับ" + return **ก่อนถึง xhrDelete** (b) `const delRes = await xhrDelete(...)` + `!delRes?.ok → throw` ก่อน insert loop (c) loop เก็บ `failedItems` แบบ Phase 412 — มี fail = toast บอกจำนวน + **ห้าม toast สำเร็จ** (สำเร็จ toast เฉพาะ else). **★ เพิ่ม catch ใน saveQuotationFull** (เดิม try/finally **ไม่มี catch** — throw จาก (b) จะเงียบ/unhandled rejection): catch → console.error + toast "❌ บันทึกไม่สำเร็จ: ..." — deviation เดียวจาก prompt (จำเป็นเพื่อให้ throw มีคนรับ ตาม pattern Phase 575), finally เดิม (ปลด `_qtSaveInflight` + เปิดปุ่ม) คงเดิม. **★ verify ข้อ 4 (report):** grep `_lineItems = []` ทุกจุด — 4 catch ข้างบนแก้ครบ, ที่เหลือเป็น reset ปกติ (list/ปุ่มใบใหม่/หลัง save); grep `quotation_items` ทั้ง repo (js+html, นอก tests) — **มีแค่ quotations.js ไฟล์เดียว** ไม่มีจุดโหลดเงียบไฟล์อื่น. **ไม่แตะ:** โครง `saveQuotationFull` ส่วนอื่น · PDF flow · convert logic อื่น (dup-check/failedItems เดิม Phase 412) · ไฟล์อื่น · **ไม่มี SQL**. **Tests:** `tests/quotations_items_integrity_guard.test.js` ใหม่ (13: flag module-level / ไม่เหลือ catch เงียบ (sweep ทุก catch ที่มี `_lineItems = []` ต้องมี toast) / edit·preview·convert set-true+reset-false / guard ก่อน xhrDelete (source order) + "ห้ามบันทึกทับ" / delRes throw ก่อน loop + ไม่มี xhrDelete เปลือย / failedItems + toast สำเร็จเฉพาะ else / catch รับ throw + finally inflight / preview·convert block+return / reset ≥6 จุด). guard เดิมเขียวครบ (`convert_inflight_guard` 9 · `quotation_save_inflight_guard` ฯลฯ). companion `dashboard_readonly_guard` marker 576. **known-risk:** โหลดล้มแบบ HTTP 4xx ที่ `resp.json()` คืน object error (ไม่ throw) → `.map` throw เข้าถึง catch เหมือนกัน = covered; ฟอร์มที่เปิดค้างอยู่ก่อน deploy build นี้ไม่มี flag (window เก่า) — เคสขอบ ยอมรับ. **Verification:** lint:errors 0 (warnings = baseline 3 เดิม) · unit 2548/2548 (+13) · e2e 14/14 · markers 576 (?v=576 ×4 + cache-v576 + version 5.69.47). **Status: ✅ MERGED+DEPLOYED (#154 `56a89e3` — CI Tests+Deploy เขียว; live marker เช็คตรงจาก session ไม่ได้เพราะ network policy บล็อก pages.dev — owner ยืนยัน data-app-build=576 บนเว็บจริง). คิว AUDIT ถัดไป: income_overview + dashboard caps (→ build 577).**

**Phase 575 (receipts-multipay-patch-check — ปิด multi-pay ใบเสร็จ "สำเร็จปลอม" · money/GL blocker · PR #153 `04512c1` MERGED+LIVE 2026-07-08 build 575 · = AUDIT "Phase 574 receipts multipay" ✅ ติ๊กออกแล้ว):** **บั๊ก:** handler บันทึก multi-payment (`modules/receipts.js` ใต้ `_multiPayGuard`, ~:1354) เดิม `await window._appXhrPatch?.("receipts", patchBody, "id", r.id)` **ไม่เช็คผล** — `xhrPatch` ไม่เคย throw (คืน `{ok:false}` ทั้ง RLS/network/timeout/`undefined`→await undefined) → โค้ดไหลต่อ: mutate `r.payments`+`r.payment_method` + `_repostReceiptJvIfChanged` (void+repost JV ตาม method ใหม่) + toast "บันทึกการชำระเงินแล้ว ✅" ทั้งที่ **DB ไม่ได้รับ** = **GL ไม่ตรง receipts** (สำเร็จปลอม). **แก้ (receipts.js multipay handler เท่านั้น):** (1) `const res = await window._appXhrPatch?.(...)` + `if (!res?.ok) throw new Error(res?.error?.message || "บันทึกการชำระเงินไม่สำเร็จ")` **ก่อน** ทุก optimistic — catch เดิม (rollback `saveBtn.disabled=false` + textContent) รอรับ throw พอดี; (2) error toast → `"❌ บันทึกไม่สำเร็จ: " + e?.message` (เดิม hardcode "รัน supabase-phase69-multi-payment.sql" = mislead เมื่อพังจาก RLS/network — ย้าย SQL hint ไป console). **★ verify ข้อ 3 (report):** สแกน `_appXhrPatch` **ทุกจุดใน receipts.js** — เช็ค `res.ok` ครบแล้วทุกตัว: cancel `:466` (gate if ok), status `:594` (gate if ok), paid `:890` (throw), created_at `:1012` (throw), payment_method single-edit `:1034` (throw), edit `:1210` (throw), invoice-revert `:475/615/649/932` (เช็ค invRes.ok). **multipay `:1354` เป็นจุดเดียวที่หลุด** = fix ครบ ไม่มี money-path PATCH อื่นค้าง. **ไม่แตะ:** `_repostReceiptJvIfChanged` logic · `_multiPayGuard` · handler อื่น · ไฟล์อื่น · **ไม่มี SQL**. **Tests:** `tests/receipts_multipay_patch_guard.test.js` ใหม่ (5: const res+throw / order res-check-ก่อน-mutate·repost·toast / doesNotMatch await เปลือย / catch rollback+toast สาเหตุจริง / ไม่แตะ _multiPayGuard·_repostReceiptJvIfChanged) + `multi_payment_guard` เดิม 6/6 ไม่แตะ. companion `dashboard_readonly_guard` marker 575. **known-risk:** เคสพังจริง (DB reject) จำลองยากบน prod → พิสูจน์ด้วย guard test (source-structure) + smoke happy-path. **Verification:** lint:errors 0 · unit 2535/0 · e2e 14/14 · markers 575 (?v=575 ×4 + cache-v575 + version 5.69.46) · CI Tests+Deploy เขียว · live `data-app-build=575` (v5.69.46). **Status: ✅ MERGED+LIVE (#153 `04512c1`).**

**Phase 574 (ac-catalog-live-sync — แคตตาล็อกแอร์ sync ทุกเครื่อง "รวมหน้าลูกค้า" · customer-visible + ตารางใหม่/RLS · PR #152 `54e7bfe` MERGED+LIVE 2026-07-08 build 574 · คิว 4 จาก AUDIT_ROUND2_UX.md ✅ ติ๊กออกแล้ว · SQL applied+verified 5/5 + owner smoke cross-device ผ่าน):** **บั๊ก:** (1) แคตตาล็อกแอร์ = localStorage ต่อเครื่อง → desktop/มือถือ staff ไม่ตรงกัน; (2) หน้าร้านลูกค้า (`customer_dashboard.js:164`) อ่าน `bsk_ac_catalog` ที่มาจากไฟล์ static `data/ac_catalog.json` → ราคาที่ owner แก้ **ลูกค้าไม่เคยเห็น** (บั๊ก product ตั้งแต่ Phase 347). **แก้ด้วยตารางใหม่ `ac_catalog_doc`** (single-row jsonb id=1): SELECT ทุก authenticated (ลูกค้าอ่านได้ = ราคาโชว์โดยดีไซน์), INSERT/UPDATE เฉพาะ `NOT is_customer_role()` (Phase 505 helper), **ไม่มี DELETE policy** (ล้าง=UPDATE []), +trigger touch updated_at. **main.js:** `loadAcCatalogCloud()` (เรียกใน afterLogin ถัดจาก loadAppSettings ก่อน loadAllData — ทุก role): cloud value array>0 → `setItem bsk_ac_catalog` + `bsk_ac_catalog_from_cloud="1"`; **ไม่มีแถว (!data)** + local `user_edited` + `currentRole()!=="customer"` → seed (push local ขึ้น = เครื่องมือถือ owner ที่มีข้อมูลจริง seed เอง); value=[] (มีแถวแต่ว่าง) → ไม่ทับ local; fetch fail → เงียบ (offline-friendly). `saveAcCatalogCloud(list)` upsert id=1, gate `currentRole()==="customer"`→`{ok:false,denied:true}` no-op (RLS กันอีกชั้น), expose `window._appSaveAcCatalog`. `needRefresh` (Phase 570) เพิ่ม `&& !fromCloud` ใน lowSpec branch (cloud=source of truth ห้าม static JSON ทับ specs-coverage; `!cacheUsable` recovery ยังคงไว้). **ac-catalog.js:** `_writeCatalog` เขียน local **ก่อน** → fire-and-forget cloud sync (fail ที่ไม่ใช่ denied → toast "บันทึกในเครื่องแล้ว แต่ sync ข้ามเครื่องไม่สำเร็จ"); ปุ่ม "โหลดใหม่จาก JSON" push data ขึ้น cloud + set from_cloud; ปุ่ม "ล้างทั้งหมด" push `[]` ขึ้น cloud. **★ verify ข้อ 6 (mutation paths):** 5 ทางผ่าน `_writeCatalog` (add/edit/set-stock/import Excel/edit-spec) = sync อัตโนมัติ ✅; **2 ทางที่ไม่ผ่าน _writeCatalog = reload-JSON (:553 direct setItem) + clear-all (:566 direct removeItem)** → เพิ่ม push cloud ตรงทั้งคู่ (ปิดช่องหลุด). **★ ห้ามใช้ app_settings** (GROUP A Phase 505 customer deny — verify แล้ว). **ไม่แตะ:** `customer_dashboard.js` (อ่าน localStorage cache เหมือนเดิม) · `app_settings` · `data/ac_catalog.json` · RLS ตารางอื่น. **Tests:** `ac_catalog_cloud_sync_guard.test.js` ใหม่ (9: load from_cloud/seed 3 เงื่อนไข/save gate customer/expose+afterLogin order/ห้าม app_settings/needRefresh !fromCloud/_writeCatalog local-ก่อน-cloud/reload+clear push/SQL RLS 3 policy no-DELETE) + update `ac_catalog_no_clobber` (needRefresh copy → 3-arg +fromCloud + source-regex บรรทัดใหม่; **ไม่ weaken** — assert เดิมทั้งหมดผ่าน). companion `dashboard_readonly_guard` marker 574. **★ known-limitation (จด):** (1) **last-write-wins ทั้งก้อน** — 2 คนแก้พร้อมกัน คนหลังทับ (ร้านแก้คนเดียว—รับได้); (2) **clear/empty ไม่ propagate เต็ม** — cloud value=[] ไม่ทับ local เครื่องอื่น (offline-safe) + auto-refresh recovery (`!cacheUsable`→fetch static JSON) ยัง re-populate local หลัง clear ในโหลดถัดไป (พฤติกรรมเดิม Phase 87.5) → clear ถาวรจริงต้องลบ ac_catalog.json ด้วย (นอก scope). **🔴 deploy order:** owner รัน `supabase-phase574-ac-catalog-doc.sql` (มี verify queries ท้ายไฟล์: helper/RLS/policy 3 ตัว/trigger) → merge → smoke 3 เครื่อง/บทบาท (มือถือแก้ราคา → desktop F5 เห็นตาม → **ลูกค้า OTP เห็นราคาใหม่** = จุดพิสูจน์). **Verification:** lint:errors 0 · unit 2530/0 · e2e 14/14 · markers 574 (?v=574 ×4 + cache-v574 + version 5.69.45) · CI Tests+Deploy เขียว · live `data-app-build=574` (v5.69.45) · **SQL `supabase-phase574-ac-catalog-doc.sql` applied+verified 5/5** (owner รัน SQL Editor — helper/RLS/policy 3 ตัว no-DELETE/trigger; log ใน DB_MIGRATIONS_APPLIED.md) · **owner smoke cross-device ผ่าน** (มือถือแก้ราคา → desktop เห็นตาม → ลูกค้า OTP เห็นราคาใหม่ = จุดพิสูจน์). **Status: ✅ MERGED+LIVE (#152 `54e7bfe`).**

**Phase 573 (boot-safe-parse — กันแอปขาวถาวรจาก localStorage เสีย · boot-safety · PR #151 `1c6654e` MERGED+LIVE 2026-07-08 build 573 · คิว 3 จาก AUDIT_ROUND2_UX.md ✅ ติ๊กออกแล้ว):** **บั๊ก:** `JSON.parse(localStorage.getItem(key))` ตอน **module evaluation** (ไม่ใช่ใน function) ไม่มี try/catch → key เดียวเก็บ JSON เสีย = throw ตอน `import` → (1) `main.js` state init 4 keys (`bsk_cart_v2`/`bsk_last_receipt`/`bsk_store_info`/`bsk_payment_info`) throw = main.js ทั้งไฟล์ไม่โหลด = **แอปขาวถาวร**; (2) `customer_dashboard.js:15` top-level (`bsk_cust_cart`) throw = lazy import reject = **ลูกค้าติดหน้าเปล่าถาวร**. `selfheal.js` ล้างแค่ SW/Cache **ไม่แตะ localStorage** → refresh กี่รอบก็ไม่หาย (boot-loop). **corruption vector จริง:** หน้ากู้ backup (`settings/pages.js` restore loop) เขียน `bsk_*` ลง localStorage โดยเช็คแค่ `typeof v === "string"` ไม่ validate JSON → ไฟล์ backup เสีย/แก้มือ = เขียน JSON พังลงไป. **แก้:** helper ใหม่ `modules/_safe_parse.js` (pure, inject storage param default `globalThis.localStorage` เพื่อ unit-test) — `safeParse(key, fallback, storage)`: parse ผ่าน→ค่า · null/undefined/""→fallback · catch→`console.warn`+`removeItem` (★ self-heal: boot ถัดไปสะอาด ปิด boot-loop ที่ selfheal เดิมกู้ไม่ได้)+fallback · `removeItem` เองก็ห่อ try (storage block ไม่ throw ซ้ำ). **จุดแก้:** (main.js) import helper หัวไฟล์ + 4 keys → `safeParse(key, fallbackเดิมเป๊ะ)` (paymentInfo raw=`{banks:[],promptPay:""}` แล้ว migrate IIFE ทำงานต่อเหมือนเดิม — แตะเฉพาะบรรทัด parse ไม่แตะ logic migrate); (customer_dashboard.js) `let _custCart = safeParse("bsk_cust_cart", [])`; (pages.js restore) เพิ่ม `_JSON_KEYS` allowlist (store_info/payment_info/product_settings/ac_catalog/cart_v2/last_receipt/cust_cart — ยึด backup export จริง L205-211 = store_logo/payment_info/store_info/dark_mode/product_settings/slipok_branch; JSON subset + เผื่อ legacy) → key ใน list ต้อง `JSON.parse(v)` ผ่านก่อน `setItem` ไม่ผ่าน=ข้าม+`_skipped++`+setBk แจ้ง "ข้าม N รายการ"+delay reload 2500ms; **key plain-string (dark_mode/store_logo/slipok_branch) restore ตามเดิม** (ไม่บังคับ JSON); คง deny `bsk_slipok_key` (Phase 543). **ไม่แตะ:** `selfheal.js` · `JSON.parse` ที่มี try/catch อยู่แล้ว (main.js:1254/3237 product_settings, loadAllData, auth) · logic อื่นใน pages.js. **★ deviation (report):** สรุป skip ใช้ `setBk` (status display ในมือ handler) + delay reload 2500ms แทน `showToast` ตาม prompt — เพราะ hardReload ล้าง toast ทันที (setBk คือ pattern เดิมของ handler นี้ + guaranteed visible ก่อน reload). **Tests:** `tests/boot_safe_parse_guard.test.js` ใหม่ (9: behavioral valid/missing/corrupt+removeItem/getItem-block/empty/default-storage + source main·custDash·pages). companion `dashboard_readonly_guard` marker 572→573. **★ known-risk:** (a) safeParse คืน `fallback` reference เดิมตอน miss/corrupt — fallback ทั้งหมดเป็น literal ใหม่ต่อ call ที่ call-site (ไม่ shared) = ไม่มี mutation leak; (b) restore หากไฟล์มี key JSON เสียจะข้าม (ไม่ทับ) — user เห็นแจ้ง N รายการ + ของเดิมใน localStorage คงอยู่ (ปลอดภัยกว่าเขียนทับด้วยของเสีย). **Verification:** lint:errors 0 · unit 2520/0 · e2e 14/14 · markers 573 (?v=573 ×4 + cache-v573 + version 5.69.44) · CI Tests+Deploy เขียว · live `data-app-build=573` (v5.69.44). **Status: ✅ MERGED+LIVE (#151 `1c6654e`).**

**Phase 572 (auth-event-filter — เลิก re-render ทั้งแอปทุกชั่วโมง · auth-gate · PR #150 `ee233fe` MERGED+LIVE 2026-07-08 build 572 · คิว 2 จาก AUDIT_ROUND2_UX.md = ต้นน้ำหน้าเด้ง ✅ ติ๊กออกแล้ว · owner smoke login/boot preview ผ่าน):** **บั๊ก:** `onAuthStateChange` (main.js listener) เรียก `afterLogin()` **ทุก event ที่มี session** → (1) `TOKEN_REFRESHED` (Supabase auto-refresh ทุก ~1 ชม. + ตอน tab กลับ focus) → afterLogin ทำ `loadAllData`+`renderAll` = **หน้า reset/ฟอร์มหาย/settings เด้ง** โดย user ไม่แตะ (ต้นน้ำที่ Phase 570 แก้ปลายน้ำ); (2) boot afterLogin รัน **ซ้ำ 2-3 รอบ** (boot getSession เรียก afterLogin เอง + listener รับ `INITIAL_SESSION`/`SIGNED_IN` ที่ยิงตามมา). **แก้ (main.js 4 จุด):** (1) module var `let _appSessionUserId = null;` (ก่อน `initSupabase`); (2) `afterLogin` หลัง `loadProfile` → `_appSessionUserId = state.currentUser?.id || null` (จุดเดียวครอบทุก caller: boot/email/otp/recovery/listener); (3) listener branch มี session — **token `window._sbAccessToken = session.access_token` ยัง set ทุก event ก่อน filter** (หัวใจ — ห้ามพัง) → PASSWORD_RECOVERY branch (คงเดิม) → **(c)** `if (_event === "TOKEN_REFRESHED" || _event === "USER_UPDATED") return;` → **(d)** `if (session.user.id && session.user.id === _appSessionUserId) return;` → `await afterLogin()` (เหลือเคส user ใหม่/ยังไม่ init); (4) sign-out branch (else) เพิ่ม `_appSessionUserId = null` (re-login user เดิม → afterLogin ใหม่ + กัน cross-login). **caller ตรวจแล้ว:** email login (auth_email.js) ใช้ listener (SIGNED_IN→afterLogin, ไม่เรียกตรง); OTP (auth_otp.js:236 comment) ใช้ listener; `submitNewPassword` เรียก afterLogin ตรง (USER_UPDATED listener → (c) return, ไม่ double); boot getSession→afterLogin ตรง (ไม่แตะ block นั้น) แล้ว INITIAL_SESSION/SIGNED_IN ตามมา → (d) same-user return. **ไม่แตะ:** afterLogin logic (นอกจากบรรทัดตั้ง var) · boot getSession block · createEmailAuth wrapper · recovery flow · loadAllData/renderAll. **Tests:** `tests/auth_event_filter_guard.test.js` ใหม่ (7: module var · token-before-all-filters order · TOKEN_REFRESHED/USER_UPDATED return · same-user guard · sign-out clear · afterLogin mark หลัง loadProfile · PASSWORD_RECOVERY อยู่ก่อน filter). test เดิม auth 3 ไฟล์ (recovery_graceful/boot_run/invite_setpassword) mock afterLogin หมด + ไม่ slice listener → ไม่พัง. companion `dashboard_readonly_guard` marker 571→572. **★ known-risk (report):** (a) ถ้ามี flow ที่ต้องการ full re-init สำหรับ user เดิมโดยไม่ผ่าน logout — จะถูก (d) บล็อก (ไม่มีใน codebase ปัจจุบัน; email/otp/recovery ครบแล้ว); (b) TOKEN_REFRESHED ไม่ re-render = ถ้า role/permission เปลี่ยนฝั่ง server ระหว่าง session จะไม่ reflect จน reload — เดิมก็ไม่ reliable (รอ refresh ชั่วโมงถัดไป) + เป็นพฤติกรรมที่ปลอดภัยกว่า (ไม่เด้ง). **Verification:** lint:errors 0 · unit 2511/0 · e2e 14/14 · markers 572 (?v=572 ×4 + cache-v572 + version 5.69.43) · CI Tests+Deploy เขียว · live `data-app-build=572` (v5.69.43). **★ OTP smoke = prod-only:** owner smoke preview ทำได้แค่ login(email)/logout/boot/สลับแท็บ — **OTP ต้อง smoke บน prod หลัง deploy เท่านั้น** เพราะ preview ไม่มี `OTP_SECRET` โดยดีไซน์ (Cloudflare env แยก scope Production/Preview; secret ไม่ inherit ข้าม scope). **Status: ✅ MERGED+LIVE (#150 `ee233fe`).**

**Phase 571 (POS `_checkoutKey` รอด background reload — กันบิลซ้ำ · money-path · PR #149 `93c3a67` MERGED+LIVE 2026-07-07 build 571 · คิวแรกจาก AUDIT_ROUND2_UX.md ✅ ติ๊กออกแล้ว):** **บั๊ก:** `renderPosPage` (pos.js) reset `_checkoutKey = null` **ทุกครั้ง** แต่ `renderAll()` หลัง background `loadAllData` (deferred 10s–2min หลัง save งานต่าง ๆ) ก็เรียก `renderPosPage` ด้วย → ถ้า checkout timeout แล้ว retry จังหวะที่ background reload เพิ่งล้าง key → `_ensureCheckoutKey` gen **key ใหม่** → `uq_sales_checkout_key` (partial UNIQUE) ดัก replay ไม่ได้ → **insert บิลซ้ำ = ตัดสต็อกซ้ำ + redeem เครดิตซ้ำเสี่ยง + JV ซ้ำ**. **แก้ (1 บรรทัด, pos.js `renderPosPage` ~:242):** `_checkoutKey = null;` → `if (!(state.cart || []).length) _checkoutKey = null;` + comment. เหตุผล: ตะกร้าไม่ว่าง = intent เดิมยังไม่จบ → คง key ให้ retry replay ได้ (ตรงเจตนา comment Phase 520 ที่โค้ดปัจจุบันขัดอยู่); ตะกร้าว่าง = intent จบ/ใหม่ → ล้างเหมือนเดิม. **lifecycle ตรวจแล้ว:** success (L1653 `state.cart=[]` + L1659 `_checkoutKey=null`) → หลังสำเร็จตะกร้าว่าง fix ยัง reset ปกติ; timeout/error → cart คงอยู่ + key คง → retry เจอ 23505 → replay branch (L1484) เปิดบิลเดิม. **ไม่แตะ:** `clearPosState` (L233 logout ล้างเสมอ — ยัง unconditional) · replay branch reset (L1484) · success reset (L1659) · `_resetCreditState` (L243, view/credit UX = Phase 581 แยก) · `doCheckout`/replay lookup/เครดิต/`posView`/`numpadValue`/ไฟล์อื่นใน modules. **Tests:** +2 assert ใน `tests/checkout_key_credit_guard.test.js` (renderPosPage reset ต้องอยู่ใต้ cart-empty guard + doesNotMatch bare reset; clearPosState ยัง unconditional) — **assert เดิม 11 ตัวไม่แก้** (test นับ `/_checkoutKey = null/g` >=3 ยัง match เพราะ substring คงอยู่). **★ scope note:** bump marker 570→571 ทำ `tests/dashboard_readonly_guard.test.js` (hardcode 570) แดง → อัปเป็น 571 (companion บังคับของ marker bump ทุกเฟส เหมือน Phase 570) แม้ prompt ไม่ได้ list ไฟล์นี้ตรง ๆ. **★ known-risk (report):** sale timeout ที่ client เห็น error แต่ server commit จริง + user เคลียร์ตะกร้าเอง (removeFromCart ไม่ trigger renderPosPage) + ใส่ของใหม่ → key เดิมค้าง → checkout ถัดไป replay บิลเก่า ทิ้งบิลใหม่. edge แคบ (ต้อง manual-empty ไม่ผ่าน renderPosPage + committed-but-errored) และเป็น trade-off กับ dup-bill ที่หนักกว่า — net safety ดีขึ้น; follow-up ถ้าจะปิดสนิท = reset key ตอน cart→0 ผ่าน removeFromCart. **Verification:** lint:errors 0 · unit 2504/0 · e2e 14/14 · markers 571 (?v=571 ×4 + cache-v571 + version 5.69.42) · CI Tests+Deploy เขียว · live `data-app-build=571` (v5.69.42). **🔻 PENDING follow-up (known-risk คงไว้):** ปิด edge committed-but-errored ให้สนิท = reset `_checkoutKey` ตอน cart→0 ผ่าน `removeFromCart` (ยังไม่ทำ — edge แคบ, ทำแยกเฟสถ้า owner จัดคิว). **Status: ✅ MERGED+LIVE (#149 `93c3a67`).**

**Phase 570 (settings เด้งกลับเมนู + แคตตาล็อกแอร์ถูกทับ — UI/localStorage ล้วน · PR #147 `cf13dd7` MERGED+LIVE 2026-07-07 build 570):** **แก้ 3 จุด (ไม่แตะ DB/เงิน/สต็อกจริง/service_jobs):** (1) **settings หน้าย่อยเด้งกลับเมนู** — `showRoute` (main.js) เดิม normalize hash โดยเก็บแค่ `?query` ทิ้ง subpath (`#settings/ac-catalog` → `#settings` → settings/index.js restore เป็น 'main'); แก้: main route เดิม → คง hash ทั้งก้อน (subpath+query), route เปลี่ยนจริง → `#route` ใหม่ (แก้บล็อก replaceState เดียว). (2) **แคตตาล็อกแอร์ auto-refresh ทับ** — `loadAllData` (Phase 87.5) fetch `ac_catalog.json` ทับทั้งก้อนเมื่อ specs coverage <90% → ลบรุ่น/แก้ไข user; แก้: flag `bsk_ac_catalog_user_edited` (ตั้งเมื่อ add/edit/import/set-stock ผ่าน `_writeCatalog`; เคลียร์เมื่อ manual "โหลดใหม่จาก JSON") → auto-refresh ข้ามเมื่อ flag+cache ใช้ได้; refresh เฉพาะ cache ว่าง/พัง. (3) **export/import เพิ่ม `ac_type` (+cost/sku/note)** ใน `_EXPORT_HEADERS`/`_toExportRow`/`_fromImportRow` (import รับ key อังกฤษ wall/ceiling/cassette + หัวไทย ติดผนัง/แขวน/สี่ทิศทาง → map) → owner จัดประเภท 223 รุ่น bulk ผ่าน Excel. **Verification:** guard ใหม่ `settings_subpage_hash_guard` + `ac_catalog_no_clobber_guard`; update `ac_stock_manager_guard` (export 24→28) + `dashboard_readonly_guard` (marker 570); 43 pass/0 fail · lint:errors 0 · CI Tests+Deploy เขียว · live `data-app-build=570` (v5.69.41). **หมายเหตุ diff:** `IMPLEMENT_TEAM_PROTOCOL.md` +19 = stale-base artifact sync กับ #146 (`88b90e2`), blob ปลายทางตรง `f54c628` ไม่ซ้ำ section. **Status: ✅ MERGED+LIVE (#147 `cf13dd7`).** — 🔜 **คิวถัดไป Phase 571-585 = `AUDIT_ROUND2_UX.md`** (branch `claude/app-audit-j9jpmv` `f5d2c2a`, audit รอบ 2 UX/nav/localStorage; โค้ด fix 551/552 บน branch นี้ merged แล้ว #127 — เหลือแค่ audit doc เป็นคิว, ยังไม่ merge).

**Phase 569 (Ning memory proxy — security · functions-only, build marker คง 568 เพราะไม่มี frontend asset · POS PR #144 `14a3b97` MERGED+LIVE 2026-07-06):** **เป้าหมาย:** ย้าย Ning long-term memory ออกจากยิง Supabase ตรงด้วย `SUPABASE_SERVICE_ROLE_KEY` ใน `E:\ning-agent-v2` → ผ่าน POS Cloudflare Function แทน (service-role อยู่ฝั่ง server; Ning ถือแค่ `NING_AGENT_API_KEY`). **★ audit-before-implement (owner แก้ prompt 2 รอบ):** prompt เดิม assume table มีอยู่แล้ว → verify: **ไม่มี schema ใน repo** → เพิ่ม SQL migration เป็น deliverable; +9 should-fix (import path ลึก 2 ชั้น, middleware 3 array, forget-all DELETE guard, text_hash server-side, recall fail-soft ฯลฯ). **POS (7 ไฟล์):** 3 endpoint `functions/api/v1/ning-memory/{remember,recall,forget-all}.js` — Ning-agent-gated (`authMode==="ning_agent"`→ไม่งั้น 403; reuse pattern service-job-submit + `_error_sanitizer`); **remember** รับ client แค่ userId/text/tags, `text_hash=sha256(text.trim())` **server-side** (crypto.subtle, ตรง Python `_hash`), id/created_at=DB default, upsert `on_conflict=user_id,text_hash`; **recall** filter user_id + clamp limit 1-50 + fixed select/order (ไม่รับ arbitrary), คืน desc; **forget-all** DELETE มี `user_id=eq.` เสมอ (guard reject userId ว่างก่อน DELETE — กันลบทั้ง table). `_middleware.js` เพิ่ม 3 path ครบ **3 array** (REQUIRE_AUTH+NING_AGENT+RATE_LIMITS: recall 120/remember 60/forget 20 per min). **SQL `supabase-phase569-ning-memory-facts.sql`** (owner-run, applied 2026-07-06): CREATE TABLE `ning_memory_facts` (id uuid · user_id · text · text_hash · tags jsonb · created_at) + **UNIQUE(user_id,text_hash)** (จำเป็นต่อ upsert) + index(user_id,created_at) + **ENABLE RLS deny-by-default** (ไม่มี policy → เข้าถึงทางเดียว=service-role ผ่าน Function) + NOTIFY. **Ning (`E:\ning-agent-v2`, branch phase-569 `e980d68` merged→main local):** `HttpLongTermMemory` (header X-NING-AGENT-KEY) — **recall fail-soft → return [] ตอน 429/5xx/network** (ไม่ทำ chat พัง), **remember/forget_all raise** (ผู้ใช้ควรรู้ว่าจำ/ล้างไม่สำเร็จ); `build_long_term_memory()` factory: HTTP (มี NING_MEMORY_API_BASE+KEY) → legacy Supabase (**ต้อง ENABLE_LEGACY_SUPABASE_MEMORY=1 explicit** เท่านั้น) → SQLite; `.env.example` เพิ่ม NING_MEMORY_API_BASE/KEY + mark service-role=LEGACY. **ไม่แตะ:** POS checkout/payment/refund/void · stock/CAS · accounting JV/auto_post · payroll · UI · sw/cache/build markers · `.env` จริง Ning. **Verification:** lint:errors **0** · guard `ning_memory_proxy_guard.test.js` **9/9** · POS unit **2471/2471** · Ning pytest **50/50** · blob LF (verify `git cat-file|tr -cd '\r'` — `grep $'\r'` หลอกเพราะ autocrlf) · dry-merge clean (เก็บไฟล์ 568). **★ debug diag:** เคย remember 500 "remember exception" บน preview → root cause = **preview env `SUPABASE_URL` ผิด** (fetch โยน exception ที่ URL เพี้ยน) **ไม่ใช่บั๊กโค้ด** (tail CF log ตรงไม่ได้ → surface exception ใน response ชั่วคราวบน preview แล้ว revert; Cloudflare แยก env Prod/Preview — preview ต้องเพิ่ม key + แก้ SUPABASE_URL เอง + re-deploy). **smoke:** preview (hash f8350c86) + **prod (`boonsook-pos-v5.pages.dev`) ผ่านครบ 5 ข้อ** (remember→dedup→recall→forget deleted:1→recall ว่าง; userId=smoke-ning-memory-test). **Ning runtime:** ตั้ง NING_MEMORY_API_BASE/KEY + ENABLE_LEGACY_SUPABASE_MEMORY=0 + **SUPABASE_SERVICE_ROLE_KEY ว่าง** → restart log `Long-term memory backend: HTTP (POS proxy)`. **★ process note:** เกิด **shared-clone collision** กับ Phase 568 — เลข 568 อีกทีมจับ (drawer) → renumber เป็น **569**; ระหว่างทำ อีก session commit 568 + สลับ HEAD ทับ (staged ผมลอยไป branch เขา) → กู้ด้วย branch surgery ไม่ปน; debug commits (9d5ddc5+eb61d47) ถูก `reset --soft`-drop ก่อน merge (branch สะอาด 2 commit). **แนะ:** 2 session ควรใช้ git worktree แยก (memory `feedback_shared_clone_parallel_session`). **Status: ✅ POS MERGED+LIVE (#144 `14a3b97`), Ning merged (local `e980d68`), Phase 569 ปิด.**

**build 570 (Phase 570 — settings subpage bounce + ac-catalog no-clobber + export ac_type · branch `claude/phase-570-settings-bounce-catalog` จาก origin/main `2894a11` build 568 (Phase 569 = functions-only ไม่ bump marker), รอ review):** **ที่มา (3 บั๊ก, localStorage ล้วน):** (1) หน้าย่อยใน Settings เด้งกลับเมนูทุกครั้งที่มี background reload; (2) แคตตาล็อกแอร์ auto-refresh ทับข้อมูล user; (3) export/import ไม่มี ac_type. **★ audit (verify root cause):** (1) `showRoute` (main.js:726-734) `keepQuery` เก็บแค่ `?query` ทิ้ง subpath → `#settings/ac-catalog`→`#settings` → `settings/index.js:30-31` restore → `settingsView='main'`. (2) `loadAllData` Phase 87.5 (main.js:1249-1272) `specced<len*0.9` → fetch JSON **ทับทั้งก้อน**; user เพิ่มรุ่นไม่มี spec → coverage ต่ำ → หาย. (3) `ac-catalog.js` export 24 col ไม่มี ac_type; `AC_TYPES`=wall(ติดผนัง)/ceiling(แขวน)/cassette(สี่ทิศทาง); `pick` case-insensitive multi-key. **แก้:** (1) main.js showRoute (บล็อก replaceState เดียว) → `newHash=(curMainRoute===route && curHash)?curHash:"#"+route` = คง hash ทั้งก้อน (query-only ไม่ regress). (2) main.js loadAllData → flag `bsk_ac_catalog_user_edited`: `needRefresh=!cacheUsable||(lowSpec && !userEdited)` (edited+cache ใช้ได้=ไม่ทับ; ว่าง/พัง=refresh กู้). (3) ac-catalog.js — helper `_writeCatalog` (setItem catalog + flag=1) แทน 5 จุด mutate; manual "โหลดใหม่จาก JSON" = setItem + `removeItem flag` (reset baseline); `_toExportRow`+`ac_type:acTypeOf(c)`/cost/sku/note; `_EXPORT_HEADERS` 24→28; `_fromImportRow`+`_acTypeFromImport` (ผนัง→wall/แขวน→ceiling/สี่ทิศ|cassette|4→cassette/unknown→undefined)+cost/sku/note. export 4 helper เพิ่ม `export` เพื่อ unit-test round-trip จริง. **★ decision flag vs merge:** เลือก **flag** — user data รอด 100% zero-clobber; trade-off = ไม่ได้ spec-refresh หลังแก้ครั้งแรก (ยอมรับได้: display-only + user curate เอง + refresh ตอน cache พัง). merge ซับซ้อน+เสี่ยง field-precedence. **ไม่แตะ:** `acTypeOf` fallback "wall" · `ac-stock-form` · ส่วนอื่น showRoute/loadAllData · DB/API/เงิน/สต็อกจริง. **Tests (2 ใหม่ 24 + 1 update):** `settings_subpage_hash_guard` (computeNewHash subpath/query/route-change + source-regex ไม่เหลือ keepQuery) · `ac_catalog_no_clobber_guard` (needRefresh: ว่าง/พัง→refresh · lowSpec+edited→**ไม่** · highSpec→ไม่; _acTypeFromImport อังกฤษ/ไทย/unknown; round-trip ac_type คง + cost/sku/note; source-regex flag) · **update `ac_stock_manager_guard`** (เดิม assert 24 col ห้ามมี ac_type → owner ขอกลับ: 28 col ต้องมี — CLAUDE.md §5 update-not-weaken). **Verification:** lint:errors **0** · unit **2502/2502** · e2e **14/14** · markers 568→**570** sync (data-app-build 570 + version 5.69.41 + ?v=×4 + cache-v570 + comment + `dashboard_readonly_guard` 568→570; ข้าม 569) · node --check OK · EOL LF · **ไม่มี SQL**. **★ process note:** "modified since read" ตอน bump index.html (grep ไม่ได้ Read tool) = ไม่ใช่ collision (git status มีแค่ไฟล์ผม); Phase 569 (ning-memory) merged `2894a11` #145 = collision จบ. **Status: 🔜 STOP รอ owner review — owner smoke: ตั้งค่า→แคตตาล็อกแอร์→รอ/trigger reload → **ไม่เด้ง**; เพิ่มรุ่นแขวน→refresh app→รุ่นยังอยู่; export Excel มีคอลัมน์ ac_type→เติมประเภท→import กลับ→223 รุ่นจัดกลุ่มถูก.** push branch เมื่อ reviewer สั่ง.**

**build 568 (Phase 568 — drawer-insert-idempotency: ปิด insert gap ตัวสุดท้าย (saveServiceJob) · branch `claude/phase-568-drawer-insert-idempotency` จาก origin/main build 567 `ade9898`, รอ review):** **ที่มา:** ต่อจาก Phase 567 (3 ฟอร์มช่าง) — insert path ที่ 4 = drawer สร้างงานช่างใหม่ (`main.js` `saveServiceJob` else-branch, `xhrPost("service_jobs")` timeout 15s ใน api.js) ยังไม่มี idempotency → timeout/เน็ตหลุดหลัง commit → `_serviceJobSaveGuard` (inflight, ปล่อยใน finally) → กดใหม่ = row ซ้ำ. **★ จุดนี้หนักกว่า 3 ฟอร์ม:** case `newJobAlreadyComplete` (`isNewJob && isNowComplete` — แอดมินสร้างงาน+เลือกสถานะปิดทันที) drawer **โพสต์ JV ตอน save ตรง ๆ** (`postJournalForServiceJob` :3097-3108, ต่างจาก 3 ฟอร์มที่ JV เป็น dead code) → dup row = **dup JV โดยตรง** (source_id ต่างกัน). **★ audit-before-implement (verify anchor):** (1) `appAuthFetch` (api.js:59) verify แล้ว = fetch-compat (`url, options`) + **override Authorization ด้วย token สด** (spread caller headers ก่อน แล้วทับ apikey+Auth :62-66) + forward `{...options}` (signal/body) + คืน fetch Response + 401-refresh-retry → auth+timeout เท่า xhrPost เดิม เป๊ะ (destructure identifier ใน main.js:216 → ใช้ `fetchFn: appAuthFetch` ตรงได้). (2) xhrPost คืน `res.data` = **object เดี่ยว** (unwrap `data[0]`, api.js:113) → adapter `data: row` (object) ตรง shape → downstream `res.data?.id` (JV :3099) ทำงานเท่าเดิม; optimistic `Array.isArray(res.data)` (:3033) = false ทั้งคู่ = ไม่เปลี่ยนพฤติกรรม. **★ prompt bug ที่จับได้:** prompt เขียน `token: window._sbAccessToken || cfg.anonKey` แต่ **`cfg` ไม่มีใน saveServiceJob scope** → ReferenceError → เปลี่ยนเป็น `window.SUPABASE_CONFIG?.anonKey` (token ถูก appAuthFetch override อยู่แล้ว = ค่าไม่สำคัญ แต่ต้องไม่ throw). **แก้ (main.js 3 จุด, ไม่มี SQL):** import `createInsertIntent`/`insertServiceJobWithReplay`; module var `_serviceDrawerInsertKey` (ใกล้ `_serviceDrawerItems`); `openServiceJobDrawer(job==null)` → `if(!job) _serviceDrawerInsertKey = createInsertIntent()` (edit ไม่ gen); insert branch → adapter `res = {ok:!!row, data:row, error}` + `fetchFn: appAuthFetch` + `key: _serviceDrawerInsertKey` + `replayed→showToast "กันบันทึกซ้ำ"` + `if(res.ok) reset key=null`. **JV block invariant comment** (step 4): replay → `jobId`=row เดิม → `postJournalForServiceJob` idempotent by `(source_table,source_id)` — โพสต์แล้ว skip / commit แต่ตายก่อนโพสต์ = replay โพสต์ให้ครบ → **ห้าม skip JV เมื่อ replay** (คง flow เดิม; `replayed` อยู่ scope else-branch เข้าไม่ถึง JV block อยู่แล้ว). **ไม่แตะ:** xhrPatch/edit path (PATCH by id idempotent) · `_serviceJobSaveGuard` (คงกัน double-click ชั้นแรก) · `_signalSave` · JV/void structure/auto_post · stock restore/deduct-on-close (idempotent by marker)/precheck · `modules/api.js` · `modules/_insert_idempotency.js` (reuse ไม่แก้) · ห้าม refactor อื่นใน main.js. **Tests (`service_drawer_idempotency_guard.test.js` ใหม่ 7):** source-regex slice anchor (openServiceJobDrawer body: gen เฉพาะ job==null · insert branch: เรียก helper + fetchFn appAuthFetch + ไม่เหลือ `xhrPost("service_jobs"` + adapter res shape + reset + replay toast · JV block: ไม่ถูก gate ด้วย `if(replayed)`/boolean + postJournalForServiceJob ยิงปกติ + invariant comment) + edit path xhrPatch ยังอยู่. **★ regression fix:** `service_equipment_deduct_on_close.test.js` slice window `14000→16000` (โค้ด insert-idempotency +~1.4k char ดัน `_equipDeductOnClose` เกิน window เดิม — บทเรียน doCheckout-guard-window; วัด offset แล้ว widen ตาม precedent Phase 542). **Verification:** lint:errors **0** · unit **2469/2469** · e2e **14/14** (build-sync ผ่าน) · markers 567→568 sync (data-app-build 568 + version 5.69.40 + ?v=×4 + cache-v568 + comment + `dashboard_readonly_guard` 567→568) · node --check main.js OK · EOL LF · **ไม่มี SQL** (client_uuid+UNIQUE applied phase567 2026-07-06). **Status: ✅ MERGED+LIVE build 568 — PR #143 squash `8bf11cf` (2026-07-06); CI Tests + Deploy เขียวทั้งคู่; live marker `data-app-build="568"` + version 5.69.40 + sw `cache-v568` ยืนยันบน boonsook-pos-v5.pages.dev. **ไม่มี SQL** (client_uuid+UNIQUE applied phase567). 🎉 **ปิด insert gap ครบ 4 path** (3 ฟอร์ม Phase 567 + drawer Phase 568) — **Phase 567b (drawer) = ปิดแล้วด้วย 568 นี้** (ลบออกจากคิว residual). owner smoke (prod): admin เปิด drawer สร้างงานใหม่ 1 งาน → 1 row; แก้งานเดิม (edit) → ปกติไม่มี toast แปลก; (จำลอง retry: throttle network/กดซ้ำ) → row เดียว JV ใบเดียว. 🟡 **finding ใหม่ (micro-fix แยก, ไม่เร่ง):** optimistic push งานใหม่ใน drawer (`main.js` ~:3058 `else if (Array.isArray(res.data) && res.data[0])`) = **dead code** ตั้งแต่ `xhrPost` unwrap `data[0]` เป็น object เดี่ยว (api.js:113 → `res.data` ไม่ใช่ array) — งานใหม่จาก drawer จึง**โผล่หลัง background reload (100ms) เท่านั้น** ไม่ใช่ optimistic ทันที (Phase 568 คง behavior เดิมเป๊ะ = adapter `data:row` object ตรง shape xhrPost). แก้ = เปลี่ยนเป็น `if (!isEditing && res.data?.id && !มีใน state) unshift` — ยกเป็น micro-fix เฟสแยก. **★ process note:** build นี้เกิด shared-clone collision กับ Phase 569 (ning-memory) — commit เกือบหลง branch phase-569 (กู้ทัน ด้วย branch-ref surgery); push/merge ด้วย ref โดยไม่ checkout กัน HEAD churn (memory `feedback_shared_clone_parallel_session`).** push branch เมื่อ reviewer สั่ง (เหมือน 566/567).**

**build 567 (Phase 567 — service_jobs idempotency: client_uuid กันใบงานช่างซ้ำจาก timeout/retry (3 ฟอร์ม) · branch `claude/phase-567-service-insert-idempotency` จาก origin/main build 566 `28b7705`, รอ review):** **ที่มา:** 3 ฟอร์มช่าง (`service_form.js`/`ac_install.js`/`solar.js`) POST `/service_jobs` ด้วย raw `fetch` + ปุ่ม save re-enable ใน `finally` → server commit แล้วแต่ตอบช้า (service_form มี AbortController 15s; ac_install/solar ไม่มี timeout เลย) หรือเน็ตหลุดหลัง commit → user กดบันทึกใหม่ = **ใบงานซ้ำ 2 row**. inflight guard เดิม (`if(saveBtn.disabled)return` + set disabled) กันแค่ double-click ระหว่าง in-flight **ไม่กัน re-submit หลัง error**. **★ audit-before-implement (owner แก้ prompt ตาม 2 รอบ):** (1) **prompt รอบแรกเข้าใจผิดว่า service_form โพสต์ JV ตอน save** — verify แล้ว **ไม่จริง**: ทั้ง 3 ฟอร์มมี `const COMPLETION_STATUSES = []` (ว่างเปล่า, Phase 88.15 เจตนา "ฟอร์มช่างไม่ trigger JV เอง — JV เกิดผ่าน admin drawer") → `isClosure` always false → `postJournalForServiceJob` block เป็น **dead code ไม่เคยรัน**. double-JV จึงเกิด**ทางอ้อม**: dup row 2 ใบ → แอดมินปิด/approve ทั้งคู่ผ่าน drawer → JV 2 ใบ (source_id ต่างกัน = auto_post idempotency ช่วยไม่ได้). กัน dup row = root-cause fix. (2) **grep ทุก writer เจอ 4 insert path** pattern เหมือนกัน: service_form.js/ac_install.js/solar.js (raw fetch) + `main.js:3000` (drawer saveServiceJob สร้างใหม่, xhrPost). partial UNIQUE `WHERE client_uuid IS NOT NULL` → path ที่ไม่ส่ง key ไม่ถูกป้องกัน → owner ตัดสินใจ scope = **3 ฟอร์มพร้อมกัน, drawer = Phase 567b แยก** (ต้องทำต่อ ไม่งั้น drawer ยัง dup ได้). **แก้:** helper กลางใหม่ `modules/_insert_idempotency.js` (pure, `fetchFn` param default `globalThis.fetch` → unit-test ได้): `createInsertIntent()` gen key (`crypto.randomUUID` + fallback แบบ `_ensureCheckoutKey`); `insertServiceJobWithReplay({cfg,token,record,key,timeoutMs=15000,fetchFn})` → แนบ `client_uuid=key` → POST (AbortController timeout) → คืน `{row, replayed:false}`; **409** → `lookupByKey` (`?client_uuid=eq.<key>&limit=1`) คืน `{row เดิม, replayed:true}`; **AbortError/network** → replay-lookup ก่อน throw (เจอ→replayed:true); **400 column-missing (PGRST204/42703)** → retry POST ไม่มี client_uuid (degrade บันทึกได้ ไม่ dedup) + `console.warn`; error อื่น (401/500) → throw ไม่ retry. 3 ฟอร์ม: import helper + module var `_svInsertKey`/`_acInsertKey`/`_solInsertKey` + **gen ที่ render entry** (`renderXxxPage` เรียกตอน nav + "สร้างใบใหม่" หลัง save เท่านั้น — ไม่ re-render ระหว่างกรอก/timeout → retry ใช้ key เดิม dedup ได้; verify: save-error ไม่ re-render) + POST ผ่าน helper + `= null` reset หลังสำเร็จ + `replayed→showToast "ใบงานนี้บันทึกแล้ว (กันบันทึกซ้ำ)"` + **optimistic push กัน dup** (`some(j=>String(j.id)===String(insertedRow.id))` ก่อน push). **★ deviation จาก prompt (จด):** ac_install/solar เดิม**ไม่มี timeout** → ผ่าน helper แล้วได้ 15s timeout เพิ่ม (consistent, พฤติกรรมดีขึ้น ไม่ regress). **ไม่แตะ:** JV dead-code block/`postJournalForServiceJob`/`auto_post`/net·labor·discount calc/stock transfer·deduct (Phase 482)/slip upload/status·closure logic/`service_jobs` RLS เดิม/`main.js` saveServiceJob (=567b). **SQL owner-run** `supabase-phase567-service-client-uuid.sql`: `ADD COLUMN IF NOT EXISTS client_uuid uuid` + `CREATE UNIQUE INDEX ... WHERE client_uuid IS NOT NULL` + `NOTIFY pgrst 'reload schema'` + verify (mirror time-clock phase92-22). **Tests (`service_idempotency_guard.test.js` ใหม่ 16):** behavioral helper (fetchFn mock — ปกติ+แนบ client_uuid · 409→replay row เดิม POST ครั้งเดียว · AbortError→replay เจอ · AbortError ไม่เจอ→throw · column-missing→retry ไม่มี client_uuid · 401→throw ไม่ replay) + source-regex 3 ฟอร์ม (import helper · เรียก insertServiceJobWithReplay · ไม่เหลือ raw `rest/v1/service_jobs` · gen+reset key · push กัน dup). **Verification:** lint:errors **0** · unit **2462/2462** · e2e **14/14** (build-sync ผ่าน) · markers 566→567 sync (data-app-build 567 + version 5.69.39 + ?v=×4 + cache-v567 + comment + `dashboard_readonly_guard` marker 566→567) · node --check 4 ไฟล์ OK · EOL LF. **🔴 residual/next: Phase 567b — drawer `main.js:3000` (saveServiceJob สร้างงานใหม่) มี insert gap เดียวกัน ยังไม่แก้** (แอดมินสร้างงานใหม่ผ่าน drawer ยัง dup ได้; xhrPost path — ต้อง reuse helper เดียวกันหรือ variant). **Status: ✅ MERGED+LIVE build 567 — PR #142 squash `3393908` (2026-07-06); CI Tests + Deploy เขียวทั้งคู่; live marker `data-app-build="567"` + version 5.69.39 + sw `cache-v567` ยืนยันบน boonsook-pos-v5.pages.dev. **SQL applied**: `supabase-phase567-service-client-uuid.sql` owner รัน + verify query ผ่าน 2026-07-06 (column `client_uuid` + index `idx_service_jobs_client_uuid_unique` มีจริง — ดู DB_MIGRATIONS_APPLIED) → dedup active แล้ว. owner smoke (prod): สร้างใบงานแอร์ 1 ใบ → 1 row; กดบันทึกรัว/throttle network จำลอง retry → row เดียว (toast "กันบันทึกซ้ำ" ถ้า replay); ลองทั้ง service_form/solar. 🔴 **residual/next-in-queue: Phase 567b — drawer `main.js:3000` (saveServiceJob แอดมินสร้างงานใหม่) insert gap เดียวกัน ยังไม่แก้** (reuse helper `_insert_idempotency.js` เดิมได้ แต่เป็น xhrPost path — ต้อง verify wiring; แอดมินสร้างงานใหม่ผ่าน drawer ยัง dup ได้จนกว่าจะทำ).**

**build 566 (Phase 566 — POS "ยอดที่ต้องเก็บจริง" เดียวทุกวิธีจ่าย: VAT exclusive โอน/บัตร + fix ปุ่มเสร็จสิ้นค้างตอนใช้เครดิต · branch `claude/phase-566-pos-vat-exclusive-payable` จาก origin/main build 565 `cd7cc38` (#140), รอ review):** **ที่มา:** สมการ "ยอดที่ต้องเก็บจริง" ใน POS กระจาย inline หลายจุดไม่ตรงกัน — cash-input ถูก (Phase 88.21 VAT-exclusive + 517b-3 หักเครดิต) แต่ (a) โอน/QR (`_tPayable`) + บัตรเครดิต (`pendingPaidAmount`) ใช้ยอด **ก่อน VAT** → เปิด VAT exclusive จะเก็บขาดเท่า VAT; (b) `updateCollectBtn` (live-numpad, `bindNumpad` เรียกทุก keystroke แบบ **ไม่ re-render**) ใช้ `baseAmount=quickPayAmount` (ไม่มี fallback `cartSum`) + threshold `paid>=amount` **ไม่หักเครดิต** → บั๊กสด: บิลใช้เครดิตแล้วพิมพ์เลขพอดี ปุ่ม "เสร็จสิ้น" ค้าง disabled (เพราะ threshold=ยอดเต็ม)/ขายจากตะกร้า quickPayAmount ว่าง → threshold=0. **★ audit-before-implement (จับ 2 จุดใน prompt):** (1) **เลขบรรทัดใน prompt เพี้ยนทั้งหมด ~+13 บรรทัด ไม่ตรง cd7cc38** (prompt: calcVAT :144-163 / cash :583-589 / card :573 / transfer :652-654 / updateCollectBtn :1171-1180 / doCheckout :1349-1361 — จริง: :157-176 / :596-605 / :586 / :665-667 / :1190-1221 / :1379-1392) → **ยึดโครงสร้างโค้ด ไม่ยึดเลขบรรทัด**; (2) prompt เขียน "ห้ามเปลี่ยนพฤติกรรมตอน VAT ปิด แม้แต่สตางค์เดียว" ขัดกับการแก้บั๊กเครดิต (ซึ่ง manifest ตอน VAT ปิด) → ตีความตามเจตนา: **path ไม่มีเครดิต + VAT ปิด = เท่าเดิมเป๊ะ**, path มีเครดิตคือสิ่งที่ตั้งใจแก้. verify แล้ว: payment-select `amount`(:478) + transfer `amount`(:665) = `quickPayAmount||cartTotal` (base ก่อน VAT) → `calcPayable(amount,...)` recompute VAT ถูก ไม่ double-apply; `doCheckout` (:1379-1392) คิด actualTotal/creditUsed/_payable ถูกอยู่แล้ว = **ยอดที่บันทึกจริงถูกเสมอ** (บั๊กเป็น display/threshold layer → VAT เปิดจะเก็บเงินขาดหน้าเคาน์เตอร์แต่งบถูก). **แก้ (`modules/pos.js` ไฟล์เดียว):** เพิ่ม pure export `calcPayable(baseAmount, paymentInfo, creditUsed=0, currentDate=todayBkk())` → `{amount, credit, payable, vat}` (ยกสมการ cash-input ออกมา: `amount = exclusive ? vc.total : base`; `credit = clamp(round2(creditUsed), 0..amount)`; `payable = max(amount−credit,0)`) แล้ว reuse **4 จุด**: (a) card `pendingPaidAmount = calcPayable(amount, state.paymentInfo, _creditUsed).payable`; (b) transfer-qr `_tPayable = calcPayable(...).payable` + โชว์บรรทัดย่อย "(base + VAT x)" ใต้ยอดตอน exclusive (mirror cash-input); (c) cash-input refactor เป็น helper (`cashPay.vat/amount/credit/payable` — display line เดิมใช้ครบ, **พฤติกรรมเท่าเดิม**; helper เพิ่ม `Math.max(credit,0)` floor เชิงป้องกัน = เท่าเดิมเมื่อ credit≥0 ซึ่ง clamp ที่ payment-select แล้ว); (d) `updateCollectBtn` cashBtn branch → `payable = calcPayable(quickPayAmount || cartSum(st.cart), pi, _creditUsed).payable` + threshold/เงินทอน live ใช้ `payable`. **ไม่แตะ:** `doCheckout` math/JV/`auto_post`/`calcVAT` (สูตร/เงื่อนไข enabled)/CAS/inflight guard/RLS/refund (VAT refund = Phase 567 แยก)/confirm-proof/slip-verify/loyalty/stock precheck/payment-select `_payable`(:490 คงสูตร inline เดิม โดยตั้งใจ). **Tests:** `tests/pos_payable_guard.test.js` (ใหม่ 12: calcPayable math — VAT ปิด→{100,payable100} · ปิด+เครดิต30→70 · exclusive7%→{107,payable107} · exclusive+เครดิต30→77 · inclusive7%+เครดิต30→70 · เครดิต>ยอด→payable0 clamp · เครดิตติดลบ→clamp0; source-regex slice branch body [cash/transfer/card/updateCollectBtn] ยืนยันเรียก calcPayable + ไม่เหลือสูตร inline เดิม + updateCollectBtn มี fallback cartSum) + **อัปเดต `credit_use_jv_split_guard.test.js`** (บรรทัด 101/106: source-regex ที่ assert สูตร inline เดิม → ชี้ helper `calcPayable`/`cashPay.payable`/`_tPay.payable` **โดยคงเจตนา** "เก็บ payable=amount−credit ไม่ใช่ยอดเต็ม" + เสริม transfer ครอบ VAT-exclusive — CLAUDE.md §5 อนุญาต: implementation detail เปลี่ยนโดยพฤติกรรมคงเดิม, พิสูจน์ด้วย calcPayable math test). **Verification:** lint:errors **0** · unit **2446/2446** · e2e **14/14** (build-sync ผ่าน) · markers 565→566 sync (data-app-build 566 + version 5.69.38 + style.css/selfheal/main/boot ?v=566 + cache-v566 + comment) · ไม่มี stale v=565 (เหลือแค่ comment history sw.js) · EOL LF · **ไม่มี SQL**. **Status: ✅ MERGED+LIVE build 566 — PR #141 squash `ec27126` (2026-07-06); CI Tests + Deploy เขียวทั้งคู่; live marker `data-app-build="566"` + version 5.69.38 + sw `cache-v566` ยืนยันจริงบน boonsook-pos-v5.pages.dev. owner smoke VAT-ปิด (บน prod): ขายเงินสด 100 พิมพ์ 100→ปุ่มติด ทอน 0 · บิล 100 ใช้เครดิต 30→พิมพ์ 70 ปุ่ม "เสร็จสิ้น" ติด + ทอน live ถูก · โอน→QR โชว์ 70. 🔴 **residual → รวมแก้ Phase 567:** (1) **payment-select full-credit path ยังไม่คิด VAT exclusive** — `_payable` (pos.js:504) + `_fullCredit` detect (:510) ใช้ `amount`=ยอดก่อน VAT (ตั้งใจไม่แตะใน 566 ตาม scope) → เปิด VAT exclusive หน้า "เก็บเงินด้วย" โชว์ "ยอดชำระ" ต่ำกว่าจริง + full-credit trigger ผิด (เครดิตคลุม base แต่ไม่คลุม base+VAT); `doCheckout` ยังบันทึกถูก (คิด actualTotal เอง) = display-layer bug. (2) **smoke VAT-on ทำได้เฉพาะ toggle จริงบน prod DB** — `vatEnabled/vatRate/vatPriceMode` เก็บใน `app_settings` (`main.js:569` upsert prod ตรง); preview/local แชร์ DB prod เดียวกัน → **ไม่มีทาง smoke VAT-on แบบ isolated** (เคย flag ใน follow-up: prompt สมมติ preview มี DB แยก = ผิด) → จัดเป็น **ขั้นตอนของวันเปิด VAT จริง** (ร้านยัง sole-prop ไม่จด VAT). logic VAT-on พิสูจน์แล้วด้วย unit `calcPayable` (exclusive 7%→107 ทั้ง 3 วิธี, deterministic ไม่แตะ DB).**

**build 564 (Phase 564 — dashboard: แถบเด่น "งานช่างรออนุมัติ" + deep-link ไปหน้าอนุมัติ · branch `claude/phase-564-dashboard-pending-review` จาก origin/main build 563 (#138), รอ review):** **ที่มา:** admin ต้องรู้จากหน้าแรกว่ามีงานช่างส่ง "รออนุมัติ" (Phase 563 ทำให้ drawer โชว์สถานะถูก แต่ต้องเปิดใบรับงานเองถึงเห็น) → เพิ่มแถบเด่นบน dashboard คลิกเข้าอนุมัติได้เลย. **★ audit จับ prompt error:** prompt บอก "reuse isServiceJobPendingReview ที่ import แล้ว" ใน dashboard.js — **แต่ verify แล้วยังไม่ได้ import** (grep เปล่า) → ถ้าตามตัวอักษร = ReferenceError หน้าขาว; **แก้โดยเพิ่ม import เอง**. **แก้ (`dashboard.js` + `service_jobs.js`):** (1) `service_jobs.js` export `setServiceJobsFilter(key)` — set `_sjFilter` (module-private :53) รับเฉพาะ `["all","open","review","closed","cancelled"]` (key ไม่รู้จัก=ไม่เปลี่ยน). ไม่มี circular import (service_jobs ไม่ import dashboard — verified). (2) `dashboard.js` import `isServiceJobPendingReview` (service_status.js) + `setServiceJobsFilter` (service_jobs.js). (3) `pendingReviewCount` = **mirror chip review เป๊ะ**: `state.serviceJobs.filter(ตัด cancelled+[ลบแล้ว]).filter(isServiceJobPendingReview).length` (= service_jobs cReview :19 → เลขแถบ = เลข chip). **★ deviation จาก prompt:** prompt ใช้ `!j.deleted_at && !note.includes([ลบแล้ว])` — เปลี่ยนเป็น mirror cReview (`!(cancelled && [ลบแล้ว])`) เพื่อ guarantee เลขตรง chip. (4) แถบแทรกใต้ hero (`dash-today` close ~:533) ก่อน period-tabs; render เฉพาะ `pendingReviewCount > 0` (ไม่มีงาน=ไม่โผล่); สีม่วง `dash-clickable data-go="service_jobs" data-sj-filter="review"`; ทุกค่าเป็นเลข/ข้อความคงที่ (ไม่มี field ผู้ใช้ → ไม่ต้อง escHtml). (5) click handler เดิม `.dash-clickable[data-go]` เพิ่ม `if (card.dataset.sjFilter) setServiceJobsFilter(card.dataset.sjFilter)` **ก่อน** showRoute → เปิดหน้าใบรับงาน filter review ให้เลย. **ไม่แตะ:** approve/reject · postJournalForServiceJob · stock · normalizeServiceJobStatus · service_jobs render/save · status DB · fetch/write (dashboard read-only — ไม่มี `fetch(` ตรง, guard `dashboard_readonly` เขียว). **Tests (`dashboard_pending_review_guard.test.js` ใหม่ 7 + marker):** source dashboard (import helper · count ใช้ isServiceJobPendingReview + ตัด deleted · แถบ conditional `>0` · data-go+data-sj-filter=review · handler setServiceJobsFilter ก่อน showRoute · read-only ไม่มี fetch/write) + service_jobs (`export setServiceJobsFilter` + guard 5 key + behavioral callable ไม่ throw) + `dashboard_readonly_guard` marker 563→564. **Verification:** lint:errors **0** · unit **2434/2434** · e2e **14/14** (build-sync ผ่าน) · markers 563→564 sync (data-app-build 564 + version 5.69.36 + ?v=×4 + cache-v564 + comment) · node --check dashboard/service_jobs OK · EOL LF · **ไม่มี SQL**. **Status: 🔜 STOP รอ owner review — owner smoke: มีงานช่างส่งรออนุมัติ → เปิดภาพรวมบริษัท เห็นแถบม่วง "📨 มีงานช่างรออนุมัติ N งาน" ใต้ยอดขายวันนี้ (N ตรง chip review) → คลิก → เด้งหน้าใบรับงาน filter "รออนุมัติ" เปิด เห็นงานนั้น → อนุมัติได้ · ไม่มีงานรอ → แถบไม่โผล่ · ไม่ error console.**

**build 563 (Phase 563b — drawer แก้ไขงานช่าง: dropdown สถานะโชว์ "รออนุมัติ" ถูกตอนงาน pending_review · branch `claude/phase-563-drawer-pending-review-status` จาก origin/main build 562 (#137), ✅ MERGED+LIVE `cefe6a4` PR #138):** **ที่มา:** เปิด drawer แก้ไขงานที่ "รออนุมัติ" (ช่างส่ง รอแอดมิน) → dropdown สถานะโชว์ "รอดำเนินการ" แทน "📨 รออนุมัติ". **★ ราก (verified):** งาน "รออนุมัติ" เก็บ DB เป็น `status="pending"` + note marker `[รออนุมัติแอดมิน]` (`pending_review` = UI-pseudo, normalize ทิ้งตั้งแต่ Phase 383/545). `openServiceJobDrawer` (`main.js:2429`) restore ด้วย `$("serviceStatus").value = job?.status || "pending"` = "pending" ดิบ → dropdown เลือก option "รอดำเนินการ". helper `isServiceJobPendingReview(job)` (detect ถูก: status=pending_review **หรือ** pending+marker) import แล้ว (main.js:11) + ใช้ที่ approve banner (:2558) — แต่ไม่ได้ใช้จุด restore. **แก้ (main.js:2429 บรรทัดเดียว):** `$("serviceStatus").value = isServiceJobPendingReview(job) ? "pending_review" : (job?.status || "pending")`. option `value="pending_review"` มีจริงใน dropdown (index.html:709 "📨 รออนุมัติ (ช่างส่ง — รอแอดมิน)"). **★ round-trip neutral (display-only, ไม่เปลี่ยน DB):** save handler (:2929) `status: normalizeServiceJobStatus($("serviceStatus").value)` → `"pending_review"→"pending"`; (:2930) `note: serviceJobNoteWithReviewMarker(..., $("serviceStatus").value)` → คง/ไม่ dup marker → บันทึกกลับ = pending+marker เดิม. **verify ไม่ชน draft:** `applyDraftFields` (:2465) + `loadServiceDraft` (:2416) รันเฉพาะ **เพิ่มงานใหม่** (`if(!job)`) → แก้ไข (job≠null) ไม่ทับ serviceStatus. option-disable block (:2436-2445) ไม่แตะ .value + pending_review ไม่ใช่ completion → ไม่ถูก hide. approve/reject buttons (:2566/2575) override .value ตอนคลิก = ไม่ชน. **ไม่แตะ:** normalizeServiceJobStatus/save flow/postJournalForServiceJob/stock/approve banner logic/dropdown options (index.html)/status DB. **Tests (`service_drawer_status_guard.test.js` ใหม่ 7 + marker):** behavioral `isServiceJobPendingReview` (pending_review→true · pending+marker→true · pending ไม่มี marker→false · progress→false) + round-trip (`normalize("pending_review")→pending` · marker คง+idempotent) + source main.js (restore ใช้ helper ternary ไม่ใช่ status ดิบ · save ยังใช้ normalize+marker) + guard เดิม (service_job_close_admin/service_status) เขียวครบ + `dashboard_readonly_guard` marker 562→563. **Verification:** lint:errors **0** · unit **2427/2427** · e2e **14/14** (build-sync ผ่าน) · markers 562→563 sync (data-app-build 563 + version 5.69.35 + ?v=×4 + cache-v563 + comment) · node --check main.js OK · EOL LF · **ไม่มี SQL**. **Status: 🔜 STOP รอ owner review — owner smoke: เปิดงาน "รออนุมัติ" → dropdown โชว์ "📨 รออนุมัติ (ช่างส่ง — รอแอดมิน)" · งาน pending ปกติ (ไม่มี marker) โชว์ "รอดำเนินการ" · done/delivered โชว์ตรง · กด "อนุมัติ+ลงรายได้" ที่ banner → JV/stock ปกติ · save งานรออนุมัติโดยไม่แก้ → DB ยัง pending+marker (ไม่เปลี่ยน).**

**build 562 (Phase 563 — admin เปิดแอปไป dashboard (ไม่ใช่ tech_home) + ย้ายเมนู "หน้าหลักช่าง" ไปกลุ่ม "งานหลัก" · branch `claude/phase-563-admin-landing-techhome-menu` จาก origin/main build 561 (#136), ✅ MERGED+LIVE `c1bb311` PR #137):** **ที่มา (owner request):** เปิดแอปด้วย admin แล้ว **land ที่ "หน้าหลักช่าง" (tech_home) ก่อน** + ปุ่มเมนูหน้าหลักช่างอยู่ท็อปสุด "งานประจำวัน" — owner อยากให้ admin เปิดมาเจอ "ภาพรวมบริษัท" (dashboard) และปุ่มเมนูย้ายไปกลุ่ม "งานหลัก"; **หน้าช่าง (technician) ให้ land tech_home เป็นหน้าแรกเหมือนเดิม (ถูกต้องแล้ว).** **★ ราก (verified `main.js:1107`):** fresh-login `restorePage = (savedRoute && allowed.includes(savedRoute)) ? savedRoute : allowed[0]`; `ROLE_PAGES.admin = ALL_ROUTES` และ `ALL_ROUTES[0] = "tech_home"` (เพิ่มหัวแถวตั้งแต่ Phase 549) → admin ที่ไม่มี saved route → `allowed[0]="tech_home"`. (sales/accountant `allowed[0]="dashboard"`, technician/customer ถูกอยู่แล้ว → **มีแค่ admin ที่เพี้ยน**). **แก้ (2 ไฟล์):** (1) **`main.js`** เปลี่ยน default จาก `allowed[0]` → role-aware `_roleHome` (`technician→"tech_home"` · `customer→"customer_dashboard"` · else `"dashboard"`) + `_defaultHome = allowed.includes(_roleHome) ? _roleHome : (allowed[0]||"dashboard")` (guard ว่า role home อยู่ในสิทธิ์จริง); `restorePage = savedRoute-ถ้ามี : _defaultHome`. saved route (hash/localStorage) ยัง honor เหมือนเดิม. (2) **`index.html`** ย้าย `<button data-route="tech_home">🧰 หน้าหลักช่าง</button>` จากใต้ `นาวประจำวัน` (บรรทัดแรก) → ใต้ label `งานหลัก` (ก่อนกลุ่มงานขาย). **technician landing ไม่เปลี่ยน** (`_roleHome=tech_home` + ROLE_PAGES.technician[0]=tech_home). **ไม่แตะ:** ROLE_PAGES/สิทธิ์/allowedPages/route logic อื่น · dashboard.js/tech_home.js เนื้อหา · POS/JV/stock/SQL. **Tests (`admin_landing_techhome_guard.test.js` ใหม่ 5 + marker):** source main.js (`_roleHome` map 3 role · `restorePage` ใช้ `_defaultHome` ไม่ใช่ allowed[0] ตรง · technician ROLE_PAGES ยังนำ tech_home) + index.html (indexOf order: ปุ่ม tech_home อยู่หลัง label "งานหลัก" · dashboard เป็นตัวแรก "งานประจำวัน" · dashboard ก่อน tech_home · ปุ่มยังอยู่ ไม่ถูกลบ) + `dashboard_readonly_guard` marker 561→562. **Verification:** lint:errors **0** · unit **2420/2420** · e2e **14/14** (build-sync ผ่าน) · markers 561→562 sync (data-app-build 562 + version 5.69.34 + ?v=×4 + cache-v562 + comment) · node --check main.js OK · EOL LF · **ไม่มี SQL**. **Status: 🔜 STOP รอ owner review — owner smoke: logout→login admin (ไม่มี hash) → เปิดมาที่ **ภาพรวมบริษัท** (ไม่ใช่หน้าหลักช่าง) · ปุ่ม "หน้าหลักช่าง" อยู่ในกลุ่ม "งานหลัก" · login ช่าง → ยัง land "หน้าหลักช่าง" เป็นหน้าแรก · deep-link `#tech_home` ยังเข้าได้ทุก role ที่มีสิทธิ์.**

**build 561 (Phase 562 — dashboard KPI เดือน/ปี + trend/chart ดึงยอดขายจริงจาก DB (เลิกพึ่ง state.sales cap 50) · branch `claude/phase-562-dashboard-fetch-sales` จาก origin/main build 560 (#135), ✅ MERGED+LIVE `21b43a2` PR #136):** **ที่มา:** การ์ด "ยอดขาย/กำไรขั้นต้น ${period}" + "กำไรสุทธิเดือนนี้" + trend/chart/sparkline บนภาพรวมบริษัท (`dashboard.js`) reduce จาก `state.sales` ที่ `loadAllData` cap แค่ 50 บิลล่าสุด (main.js) → ร้านขาย >50 บิล/เดือน tab "เดือนนี้/ปีนี้" ตัวเลขต่ำกว่าจริงทุกครั้ง ("วันนี้" ถูกเพราะ <50/วัน). **แก้ (display/report-only, `dashboard.js` ไฟล์เดียว — ลอกโครง async-cache Phase 508 credit):** (1) module cache `_dashSalesRows/_dashSalesState(idle→loading→loaded|error)/_dashSalesSeq/_dashSalesCacheKey`. (2) invalidate เมื่อ cacheKey `"<userId>:<role>:<yearStart>"` เปลี่ยน (สลับ user/role/ปี → reset idle). (3) source select `const _salesForAgg = (_dashSalesState==="loaded" && _dashSalesRows) ? _dashSalesRows : _stateSales` (`_stateSales = visibleSalesForRole(state.sales,...)` = fallback ≤50 ระหว่างโหลด/error — ไม่ throw/หน้าขาว). period(:413)/todaySales/month/recent/sparkline(:526)/trend(:734)/chart(:932)/proPanels(:935,942) เปลี่ยน `allSales`→`_salesForAgg` (param ใน renderProPanels/panels คงชื่อ). (4) async trigger (ท้ายฟังก์ชัน ข้าง credit): fetch เฉพาะ `_dashSalesState==="idle"` (★ กัน re-render loop — error/loaded/loading ไม่ refetch) → `fetchSalesSince(_yearStart="YYYY-01-01")` (sales_fetch.js Phase 492, paginated, `select=*`, `created_at.gte`, ไม่ cap 1000) → seq-guard → **role-filter `visibleSalesForRole(res.rows,...)`** (กัน non-admin เห็นยอดคนอื่น) → `renderDashboard(sameArgs)` re-render ทั้งหน้า (trigger เห็น non-idle → ไม่ refetch/ไม่ loop). (5) ป้ายบนการ์ด period: loading "⏳ โหลดยอดเต็ม…" / error "⚠️ (title: แสดง ≤50)". caption trend ซื่อตรงตาม state (loaded → "รายได้=ยอดขายปีนี้จาก DB · ค่าใช้จ่าย ~200"). **★ read-only:** dashboard.js **ไม่มี `fetch(` ตรง** (เรียกผ่าน import `fetchSalesSince`) → `dashboard_readonly_guard` "must not call fetch()" เขียว (เหมือน Phase 508 credit). **ไม่แตะ:** postJournal/auto_post · POS/checkout · stock · main.js loader · state.sales write · credit(508)/service(553) cache · **expenses ยัง state cap 200** (out-of-scope) · SQL. **Tests:** `dashboard_sales_fetch_guard.test.js` (ใหม่ 10 source-guard: import fetchSalesSince · cache vars · `_salesForAgg` fallback · period/month/trend/chart อ่าน _salesForAgg · fetch เฉพาะ idle (loop-safe) · seq-guard · cacheKey user/role/year · rows role-filtered · error→rows null · read-only ไม่มี fetch(/write) + `dashboard_trend_chart` (renderChart arg allSales→_salesForAgg) + `dashboard_readonly_guard` marker 560→561. **Verification:** lint:errors **0** · unit **2415/2415** · e2e **14/14** (build-sync ผ่าน) · markers 560→561 sync (data-app-build 561 + version 5.69.33 + ?v=×4 + cache-v561 + comment) · ไม่มี stale 560 · EOL LF · node --check OK · **ไม่มี SQL**. **known-residual (จด):** (a) `periodExpenseTotal`/`monthExpenseTotal` ยังอ่าน state.expenses cap 200 → กำไรสุทธิเดือน/ปี ฝั่งรายจ่ายยัง cap (ร้านนี้รายจ่าย/เดือนน้อย = ยังไม่กระทบ; follow-up ต้องมี expenses fetch helper); (b) fetch จากต้นปีปัจจุบัน → trend เดือนปีก่อน (Dec–Jan cross-year) ไม่ครบ (ดีกว่า cap 50 เดิม); (c) fetch error ค้าง state=error ทั้ง session จนสลับ user/role/ปี (ไม่ auto-retry — hard-refresh ได้; fallback ≤50 + ป้าย ⚠️ ไม่เงียบ). **Status: 🔜 STOP รอ owner review — owner smoke: ขาย >50 บิลในเดือน → tab "เดือนนี้"/"ปีนี้" ยอดขาย+กำไร = ครบจริง (เทียบ profit_report เดือนเดียวกัน ต้องตรง/ใกล้) · tab "วันนี้" เท่าเดิม · เปิดหน้าเห็น fallback แว่บเดียวแล้วเด้งเป็นเลขเต็ม (ไม่หน้าขาว/ไม่ค้าง loading).**

**build 560 (Phase 561 — POS แคชเชียร์ ล็อกบัญชีรับโอนตามกลุ่มลูกค้า "หน้าร้าน" ที่ผูกใน Settings · branch `claude/phase-561-pos-lock-bank` จาก origin/main build 559 (#134), รอ review):** **ที่มา:** หน้าแคชเชียร์รับโอน (`pos.js` transfer-qr) โชว์ dropdown ทุกบัญชี (`validBanks.length>1`) → owner อยากล็อกบัญชีเดียว **โดย reuse binding เดิม** (Phase 439 `bank.customerGroup`) = บัญชีที่ผูกกลุ่ม "หน้าร้าน"; **ไม่เพิ่ม setting ใหม่** (owner เปลี่ยนการผูกใน Settings → POS เปลี่ยนตาม). **★ ประวัติ:** รอบแรกทำเป็น setting ใหม่ `posBankCoa` (push build 560 commit `0fe61cc`) → owner เปลี่ยนทิศเป็น reuse customerGroup → **reset branch กลับ origin/main แล้วทำใหม่** (approach posBankCoa ถูกทิ้ง, ไม่มีใน commit นี้). **★★ audit จับ money bug (เหมือนรอบแรก):** JV Dr ตอน checkout resolve `validBanks[selectedBankIdx]` อีกครั้ง แยกจาก render → lock ต้อง **set module-level `selectedBankIdx`** ไม่ใช่แค่ override activeBank ใน render. **แก้ (`pos.js` ไฟล์เดียว):** pure export `_resolvePosBankIdx(validBanks, group, fallbackIdx)` → `{idx, locked}`: `findIndex(b=>b.customerGroup===group)` เจอ → `{idx:ตำแหน่ง, locked:true}`; ไม่เจอ → `{idx:fallbackIdx, locked:false}`. render transfer-qr: `POS_GROUP="หน้าร้าน"` → เรียก helper → **`selectedBankIdx = _posBankRes.idx`** (กุญแจ money guard) → `activeBank = validBanks[selectedBankIdx]`. locked → ซ่อน `#posBankPicker` โชว์ป้าย "🔒 บัญชีรับเงินหน้าร้าน: … (ผูกไว้ในตั้งค่า)"; ไม่ผูก + `validBanks.length>1` → dropdown เดิม + เตือนเบา "ยังไม่ผูกบัญชีกลุ่มหน้าร้าน — เลือกเอง หรือตั้งใน ตั้งค่า"; ไม่ผูก + บัญชีเดียว → พฤติกรรมเดิม (ไม่ nag). **ไม่แตะ JV block** (`validBanks[selectedBankIdx]`). **★ deviation จาก prompt (จด):** prompt สั่ง import `resolveBankForCustomerGroup` แต่ helper นั้นคืน **snapshot object ไม่ใช่ index** — เราต้องการ index เพื่อ steer selectedBankIdx → ใช้ `findIndex` ตรง (ตาม step 2) ไม่ import (กัน unused-var). **ไม่แตะ:** postJournalForSale/BANK_COA format/checkout/stock/VAT/credit/service drawer QR (560)/ใบเสร็จ-ใบเสนอราคา picker/**settings/payment.js** (binding มีแล้ว)/**main.js·customer_groups.js** (helper มีแล้ว)/SQL. **Tests (`pos_bank_lock_guard.test.js` ใหม่ 11 + marker):** behavioral `_resolvePosBankIdx` (ผูก→lock+idx=ตำแหน่ง · **money guard: validBanks[idx]=บัญชีที่ผูก** · ไม่ผูก→fallback · out-of-range→0 · empty→idx0 · first-match-wins) + source pos.js (`POS_GROUP="หน้าร้าน"` · `selectedBankIdx=_posBankRes.idx` · ซ่อน picker เมื่อ locked · JV `validBanks[selectedBankIdx]` ไม่เปลี่ยน · BANK_COA format เดิม) + `dashboard_readonly_guard` marker 559→560. **Verification:** lint:errors **0** · unit **2405/2405** · e2e **14/14** (build-sync ผ่าน) · markers 559→560 sync (data-app-build 560 + version 5.69.32 + ?v=×4 + cache-v560 + comment) · ไม่มี stale 559 (เหลือแค่ comment changelog #134) · EOL LF · **ไม่มี SQL**. **Status: 🔜 STOP รอ owner review — owner smoke: Settings ผูก SCB=กลุ่ม "หน้าร้าน" (field มีอยู่แล้ว)→POS ขายโอน เห็น SCB 1135 อย่างเดียว (ไม่มี dropdown, ป้าย 🔒)→จบบิล→**JV Dr=1135** · เปลี่ยนผูก KBANK=หน้าร้าน→POS โชว์ KBANK แทน (ไม่แก้โค้ด) · ยังไม่ผูก→dropdown+เตือน.**

**build 558 (Phase 558 — งานช่าง: โชว์ QR + เลขบัญชีเมื่อเลือก "โอน/QR" ในใบงานช่าง · branch `claude/phase-558-service-qr-display` จาก origin/main build 557, รอ review):** **ที่มา:** drawer ใบงานช่าง (`service_form.js`) ช่อง "วิธีรับเงิน" มี option `transfer` (🏦 โอน/QR → Dr 1130) แต่ช่างต้องสลับไปหน้า POS เพื่อโชว์ QR ให้ลูกค้าสแกน. **แก้ (UI additive + read-only):** (1) template — ใต้ grid สถานะ/วิธีรับเงิน เพิ่ม `<div id="svPayQrBox"></div>` (ว่างตอน render แรก). (2) pure export `_svPayQrHtml(paymentInfo, method)` — `method!=="transfer"` → คืน `""` (เคลียร์กล่อง); transfer → render QR หลัก (`pi.qrImage` หรือใบแรกที่มี `qrImage`) + พร้อมเพย์ (`pi.promptPay`) + ทุกบัญชี `pi.banks[]` (ชื่อธนาคาร/เลข/ผู้ถือ/สาขา + qr ต่อใบ); ไม่มีข้อมูลเลย → ข้อความชวนไป **ตั้งค่า → ข้อมูลการชำระเงิน** (ไม่ error). source เดียวกับ `customer_dashboard.js:457-503` (state.paymentInfo). **ทุก field ห่อ `escHtml`** (bankName/bankAccount/bankHolder/bankBranch/qrSrc/promptPay) กัน XSS. (3) wire — `#svPaymentMethod` `change` → `_renderPayQr` + เรียก `_renderPayQr()` ครั้งแรก **หลัง `applyDraftFields`** (restore paymentMethod) → งาน/ร่างที่ payment=transfer เปิด drawer โชว์ทันที. **ไม่แตะ:** save/status dropdown/`payment_method` option value/slip-verify (`svSlipVerifyBtn`)/JV/stock/SQL/POS transfer-qr (`pos.js` คนละหน้า); ไม่มี fetch/write — อ่าน `state.paymentInfo` อย่างเดียว. CSS: inline style attribute (ตามคอนเวนชันไฟล์) ไม่มี `<style>` block/ไม่แตะ style.css. **Tests (`service_form_qr_guard.test.js` ใหม่, 12):** behavioral pure helper (transfer→QR+บัญชี+พร้อมเพย์ · cash/""→"" · ไม่มีข้อมูล→settings hint · การ์ดว่างถูกข้าม · XSS escape · qr src escape) + source (มี #svPayQrBox · handler change+เงื่อนไข transfer · อ่าน paymentInfo/banks/qrImage/promptPay ไม่ hardcode · ทุก field escHtml · read-only ไม่มี fetch/POST · ไม่อ้าง postJournal/deductStock/save) + marker 557→558. **Verification:** lint 0 · unit **2383/2383** · e2e **14/14** · markers 557→558 (data-app-build/version 5.69.30 + ?v=×4 + cache-v558 + comment) · EOL LF. **Status: 🔜 STOP รอ review — owner smoke (มือถือ 390px): เปิดใบงาน เลือก "โอน/QR" → เห็น QR+เลขบัญชีทันที; เลือกเงินสด → หาย; งานที่ payment=transfer เปิด drawer → โชว์เลย; ร้านที่ยังไม่ตั้ง QR → เห็นข้อความชวนไป settings (ไม่ error console).**

**build 557 (Phase 557 — bundle COGS ครบวงจร: sale_items.unit_cost ต้นทาง + SQL backfill · branch `claude/phase-557-bundle-cogs-source-backfill` จาก origin/main build 556, รอ review):** **ที่มา:** #130 (build 555) แก้ COGS ของ bundle เฉพาะ `sales.gross_profit` (KPI dashboard) แต่เหลือ 2 ช่อง: **(1) ต้นทาง** — `doCheckout` เขียน `sale_items.unit_cost = round2(prodRef?.cost||0)` = cost ตัว bundle "แม่" (มักตั้ง 0) ขณะที่ `profit_report.js` (:103/:186/:352) + `profit_by_product.js` (:97) อ่านกำไรจาก `sale_items.unit_cost` (fallback `products.cost` = แม่ = 0 อีกชั้น) → กำไรบิลชุด **เฟ้อทุกบิลใหม่** ต่อไปแม้ #130 merge. **(2) ย้อนหลัง** — บิล bundle เก่าค้าง `unit_cost=0` + `gross_profit` เฟ้อใน DB. **แก้:** (A) extract pure `export _bundleUnitCost(product, products, bundleRecipes)` = `Σ(child.cost × recipe.qty)` ต่อหน่วย (= `expandBundleForRevert(recipe,1)`; recipe.qty blank→1 mirror deduct-side); คืน `null` เมื่อ ไม่ใช่ bundle / ไม่มี recipe / Σ cost=0 → caller fallback parent cost. ใช้ **2 จุด single-source**: `_computeGrossProfit` (แทน inline expansion #130 — DRY, ผลเลขเท่าเดิมทุก case) + `itemPayload.unit_cost = round2(_bundleUnitCost(...) ?? (prodRef?.cost||0))`. `state.bundleRecipes` preload มีจาก #130 (loadAllData) — ไม่ fetch เพิ่มใน checkout. (B) SQL owner-run `supabase-phase557-bundle-cogs-backfill.sql` 4 STEP: STEP1 verify (นับแถว bundle unit_cost=0 + recipe cost>0) → STEP2 UPDATE `sale_items.unit_cost` (scope เดียวกับ STEP1) → STEP3 UPDATE `sales.gross_profit=round(subtotal−Σ qty×unit_cost,2)` (เว้น gross_profit/subtotal NULL + note `[ลบแล้ว]`) → STEP4 verify (เหลือ 0 + spot 3 บิล) + ROLLBACK guidance (temp table). **🔴 ไม่แตะโครงแถว sale_items** — บรรทัด bundle ยัง `product_id` ตัวแม่ 1 แถว (bundle_revert.js Phase 477 + refunds + receipts พึ่งโครงนี้; verify แล้ว refunds/receipts ไม่อ่าน unit_cost) — แก้เฉพาะ "ค่า" unit_cost. **ไม่แตะ:** stock deduct/revert, refunds, receipts, JV/auto_post, VAT, checkout_key/inflight, profit_report/profit_by_product (ฝั่งอ่าน—ถูกเองเมื่อ unit_cost ถูก). **Tests (ต่อยอด pos_gross_profit):** 6 case คำนวณเดิมเขียว (ไม่แก้ expectation) + `_bundleUnitCost` (Σ children · ไม่ใช่ bundle→null · ไม่มี recipe→null · cost 0→null · qty blank→1) + source-regex (itemPayload ใช้ _bundleUnitCost+round2 · _computeGrossProfit ใช้ _bundleUnitCost · โครงแถว product_id=item.id · SQL STEP2/STEP3 scope) + readonly marker 556→557. **known-risk (จด):** backfill ใช้ **cost children ปัจจุบัน** (ระบบไม่เก็บ historical cost — ฐานเดียวกับ fallback profit_report เดิม); สูตร/ต้นทุนที่แก้หลังขายทำให้ backfill คลาดได้. **Status: 🔜 STOP รอ owner review → หลัง review: owner รัน SQL STEP1 (ดูจำนวนก่อน)→STEP2-3→STEP4→log DB_MIGRATIONS_APPLIED → smoke: ขาย bundle test → sale_items.unit_cost=Σ child cost (≠0) · profit_report เดือนนี้กำไร bundle ลดสมจริง · void บิล bundle test → children คืนครบ (กัน regress 477).**

**build 556 (Phase 556 — receipts re-post JV เมื่อแก้ช่องทาง/ธนาคาร · branch `claude/phase-556-multipay-repost-jv` จาก origin/main build 555, รอ review):** **ที่มา:** ใบเสร็จ paid มี JV RV (Dr เลือกตอน post ครั้งแรก: transfer/โอน/qr/bank→1130 มิฉะนั้น 1110 + override sub-account ด้วย bank_coa_code ผ่าน `_resolveReceiptDebitAccount`). **3 จุดแก้ field แล้วไม่ repost JV** → JV ค้าง Dr เดิมเงียบ: (a) multipay saveBtn `rcMultiPaySave` (~1276, เปลี่ยน payment_method หลัก) (b) dropdown `rcEditPayMethod` (~972) (c) `rcEdSave` (~1146, เปลี่ยน bank_coa_code). **แก้:** (1) pure export `receiptJvRepostNeeded({prevMethod,newMethod,prevBank,newBank,status})` — repost เฉพาะ paid + (ข้าม boundary transfer(1130)/อื่น(1110) วัดด้วย `_payIs` normalize Thai/Eng **หรือ** bank sub-account เปลี่ยน) → กัน churn "เงินสด"→"cash"/cash↔cheque (Dr เดิม). (2) async `_repostReceiptJvIfChanged(r,{...})` → `voidJvForSource("receipts",id)` + `postJournalForReceipt({...r, payment_method:newMethod, bank_coa_code:newBank, status:"paid", paid_at})`; **period ปิด (Phase 551 BEFORE DELETE guard) → void ลบไม่ได้ → repost ชน 409 `reason:"duplicate"` → toast เตือนปลดล็อก/manual JV (ไม่ dup เงียบ)**; fail-safe ไม่ throw/ไม่ rollback การ PATCH (§4.8). (3) wire 3 จุดผ่าน helper เดียว. **ไม่แตะ:** postJournalForReceipt/voidJvForSource/`_resolveReceiptDebitAccount` internals, credit_payment JV, POS/stock/SQL/RLS; เงื่อนไข paid/effective/idempotent(source_table+source_id) คงเดิม. **Tests:** receipt_multipay_repost_guard (ใหม่, 10) — pure decision (Thai↔Eng no-repost · cash↔cheque no-repost · cross-transfer repost · bank-change repost · not-paid skip) + source (gate ใช้ _payIs · helper void+repost · 409-duplicate toast · 3 call sites) + guard เดิม (receipt_bank_jv/receipt_bank_override) เขียวครบ · readonly marker 555→556. **Verification:** lint 0 · unit **2363/2363** · e2e **14/14** · markers 555→556 sync (data-app-build/version 5.69.28 + ?v=×4 + cache-v556 + comment) · EOL LF. **🔧 review-fix (owner reviewer, commit ต่อ branch, ไม่ bump):** branch เตือนงวดปิด "period-locked" ด้วย ไม่ใช่แค่ "duplicate" — trace: JV งวดปิด → void=0 → repost เจอ period-lock pre-check (`_postJournal` auto_post.js:252-265) คืน `reason:"period-locked"` **ก่อน** unique-409 "duplicate" (:344) → branch เดิมไม่ยิง = JV ค้างเงียบ. แก้: extract pure `receiptRepostResultToast(postRes)` (export, unit-tested behavioral — ครอบ posted/period-locked/duplicate/failed/silent) + helper delegate → +3 behavioral test (period-locked→เตือน). **known-risk (จด ไม่แก้):** multi-payment แบ่งครึ่ง (สด+โอน) → JV ยัง Dr เดียวตาม method ยอดมากสุด (main) ไม่ split ตามสัดส่วน = ข้อจำกัดเดิม postJournalForReceipt (candidate phase ถัดไป). **Status: 🔜 STOP รอ owner review (smoke: แก้ใบเสร็จ paid เงินสด→โอน → JV Dr 1130 เลขใหม่; เปลี่ยนธนาคาร → Dr sub-account ใหม่; แก้ ref/"เงินสด"→"cash" → เลข JV เดิม).**

**build 555 (Phase 555 — bundle gross profit / audit S12 · branch `claude/phase-555-bundle-gross-profit` จาก origin/main build 554, รอ review):** **ที่มา:** audit S12 — KPI "กำไรขั้นต้น" (`sales.gross_profit`) ของบิลที่มีสินค้า "ชุด (bundle)" เพี้ยน: `_computeGrossProfit` (`pos.js:90`) คิด COGS จาก `products.cost` ของตัว bundle แม่ (มักตั้ง 0) ไม่ใช่ผลรวมต้นทุน children ที่ตัดสต็อกจริง → COGS ต่ำ → gross_profit เกินจริง (reporting-only, ไม่กระทบยอด/VAT/JV/สต็อก). **แก้:** (1) `_computeGrossProfit` +param `bundleRecipes` — bundle line (`p.is_bundle` + มี recipe) ขยายสูตรด้วย `expandBundleForRevert` (pure helper เดิม, reuse deduct-side math) → COGS = Σ(child.cost × recipeQty × lineQty); ไม่มี recipe → **fallback parent cost = พฤติกรรมเดิม ไม่ regress**. (2) preload สูตรทั้งหมดที่ `loadAllData` (Phase 2 batch, `product_bundles` select bundle_id/child_product_id/qty) → `state.bundleRecipes = { [bundleId]: [rows] }` (key=string); fail → {} → fallback. (3) wire `pos.js` checkout ส่ง `state.bundleRecipes`. **ไม่แตะ:** POS total/VAT/change/JV/stock/`sale_items.unit_cost` (parent cost เดิม — related แต่นอก scope S12; ยกเป็น candidate ถ้าจะให้ COGS ใน report ตรง 100%), checkout flow, SQL/RLS. **Tests:** pos_gross_profit +6 (bundle COGS=Σchildren · lineQty×N · no-recipe fallback · empty recipes fallback · mixed cart · non-bundle ไม่แตะ recipe · source loadAllData+expand) — เดิมเขียวครบ (แก้ source-guard call ให้รวม bundleRecipes) · readonly marker 554→555. **Verification:** lint 0 · unit **2350/2350** · e2e **14/14** · markers 554→555 sync (data-app-build/version 5.69.27 + ?v=×4 + cache-v555 + comment) · EOL LF. **known residual:** `sale_items.unit_cost` ยังเก็บ parent cost สำหรับ bundle (report COGS ราย item ยังคลาด — gross_profit บิลรวมถูกแล้ว); product_bundles ไม่ paginate (recipe เล็ก). **Status: 🔜 STOP รอ owner review (reviewer จะยิง preview + smoke #pos ขาย bundle เทียบ gross_profit).**

**build 554 (Phase 554 — cash_recon fetch per-day + เก็บหนี้สดเข้าลิ้นชัก · branch `claude/phase-554-cash-recon-fetch` จาก origin/main build 553, รอ review):** **ที่มา:** หน้ากระทบยอดเงินสด (cash_recon) คำนวณ "ควรมีในลิ้นชัก" ผิด 2 เหตุ — (1) `computeCashRecon` อ่าน `state.sales`/`state.expenses` ที่ถูก cap (sales 50 @`main.js:1148`, expenses 200 @`main.js:1203`) → วันขาย/จ่ายเกิน cap = cashIn/cashOut ขาด = "เกิน/ขาด" ปลอม; (2) เงินสดจากรับชำระลูกหนี้ (`credit_payments` method=cash) ไม่ถูกนับเข้า expected เลย → วันมีเก็บหนี้สด ลิ้นชักโชว์ "เกิน" เสมอ. **แก้ (display/report-only):** (1) `computeCashRecon` รับพารามใหม่ `sales`/`expenses` (fallback state ถ้าไม่ส่ง = backward-compat unit test เดิม; explicit `[]` ชนะ state กัน cap หลุด) + `creditCashPayments` → filter method=cash+วันนั้น บวกเข้า `creditCashIn`, return `creditCashRows`/`creditCashIn`. (2) +3 fetch helper `fetchCashReconSales/Expenses/CreditPayments` (fail-safe return [], ไม่ throw) — ดึง "วันที่เลือก" ตรงจาก DB. (3) render: date-guard `_crDataDate` (Promise.all 3 fetch, cache ต่อวัน กัน refetch ทุก keystroke) → ส่งเข้า compute + บรรทัด "🧾 รับชำระลูกหนี้ (เงินสด)" + `expected = opening + cashIn + creditCashIn − cashOut − cashRefundOut`. **🔴 prompt bug ที่ engineer จับได้ (verify schema):** prompt สั่ง filter `credit_payments.created_at` **แต่คอลัมน์จริง = `paid_at`** (`supabase-rls-policies.sql:415`) → ถ้าใช้ created_at PostgREST คืน 400 → fetch [] เงียบ → เก็บหนี้สดไม่ถูกนับเลย (ฟีเจอร์ล้มเงียบ). แก้เป็น `paid_at` + guard test บังคับ credit fetch ห้ามมี created_at. sales=created_at, expenses=expense_date=eq ถูกต้อง. **Tests:** cash_recon +7 (param override · fallback state · explicit[] ชนะ state · credit cash method+วัน · default 0 · expected formula source · fetch helpers date-column ถูก) — เดิม 13 เขียวครบ. **Verification:** lint 0 · unit **2344/2344** · e2e **14/14** (build-sync ผ่าน) · markers 553→554 sync (data-app-build/version 5.69.26 + ?v=×4 + cache-v554 + comment) · EOL LF. **ไม่แตะ:** POS/checkout/pos.js, auto_post/JV, credit_tracker write path, RLS/SQL, สูตร revenue อื่น. **known residual (จด ไม่แก้):** fetch ต่อวันไม่ paginate เกิน 1000 แถว (วันเดียว sales/credit_payments >1000 หายากมาก — ถ้าถึงจริง = phase แยกใส่ range header). **🔴 LIVE SMOKE (preview, owner logged-in, read-only) จับบั๊กเพิ่ม 1 → fix commit `d255508`:** sales fetch คืน **400** เพราะ select `status` แต่ `sales` **ไม่มีคอลัมน์ status** (ยกเลิกบิลใช้ note "[ลบแล้ว]"; pos.js insert ไม่เขียน status; โค้ดเดิม select * เลยไม่ error). เอา status ออกจาก select + guard กันกลับมา. **หลัง fix: ทั้ง 4 fetch = 200** (refunds/sales/expenses/credit_payments-paid_at) · 02/07 แสดง "ขายเงินสด 2 บิล ฿30 · ควรมี ฿30 · โอนแยก ฿10" ถูก · ไม่มี console error. **Status: ✅ live-smoke ผ่าน · CI Tests success `d255508` · 🔜 STOP รอ owner merge เข้า main (ยังไม่ push main).**

**build 553 (Phase 553 — service income ใช้วันปิดงาน · branch `claude/phase-553-service-income-closed-at` จาก origin/main `638059c`, รอ review):** **ที่มา:** owner เจอ 2026-07-02 การ์ด "รายได้วันนี้ (รวมบริการ)" หน้า dashboard/income_overview ต่ำกว่าสมุดรายวัน (journal SV ~5,100 แต่การ์ด 210). **ราก:** dashboard/income นับรายได้งานบริการด้วย `created_at` (วันสร้างงาน) แต่ JV/P&L รับรู้ด้วย `closed_at` (วันปิดงาน — Phase 542, `auto_post.js:839` `job.closed_at || job.created_at`) → งานสร้างวันหนึ่งปิดอีกวัน ตัวเลขไม่ตรง. **แก้ (display-only):** (1) `dashboard.js` +helper export `serviceIncomeDate(j)= closed_at||created_at||""` (single source, วางใต้ `isServiceIncomeJob`); today/month service income (`:429`/`:438`) เปลี่ยน dateMatch จาก `dateBkk(j.created_at)` → `dateBkk(serviceIncomeDate(j))`. (2) `income_overview.js` import `serviceIncomeDate` + เปลี่ยน serviceTotal/byCat/txCount/trend เดือน (4 จุด) ใช้ serviceIncomeDate. **web order + POS คง `created_at`** (ห้ามแตะ `todayRevenue`/`webTotal` — และ web-order JV **ก็ใช้ closed_at เหมือนกัน** แต่ยกเว้นเฟสนี้ = candidate **Phase 554**). **Tests:** dashboard_income_guard +6 (serviceIncomeDate fallback + closed-this-month/created-last · created-this/closed-next · closed null fallback · source-regex :429/:438) · income_overview_guard +1 (serviceTotal=serviceIncomeDate, web/POS=created_at, กันมือลั่น) · readonly_guard marker 552→553. **Verification:** lint 0 · unit **2337/2337** · e2e **14/14** (build-sync ผ่าน) · markers 552→553 sync (data-app-build/version 5.69.25 + ?v=×4 + cache-v553 + comment) · EOL LF. **ไม่แตะ:** `modules/accounting/**`, main.js, pos.js, refunds.js, SQL, สูตร todayRevenue/webTotal/posTotal. **known residual (จด ไม่แก้):** (a) `state.serviceJobs` cap ≤50 (loadAllData) → ถ้างานบริการวันนี้ >50 การ์ดยัง undercount แม้ fix นี้ = phase แยก; (b) web-order JV basis mismatch → Phase 554 candidate. **Status: 🔜 STOP รอ owner (reviewer) audit diff+guards+markers ก่อน merge เข้า main. Preview: push → workflow ยิงเอง.**

**build 551+552 (App Audit batch — branch `claude/app-audit-j9jpmv`, ✅ merged PR #127 → main `638059c`):** **ที่มา:** owner สั่ง full app audit (5 ด้าน: money/accounting/security/XSS/offline-tz) → เจอ Blocking หลายจุด → engineer แก้ตามความเสี่ยง. **build 551** (Blocking): (1) **card→1130 JV** — `postJournalForSale` regex `/credit_term|เครดิต/` จับ "บัตรเครดิต" (substring "เครดิต") → ทุกบิลบัตรลง **Dr 1200 ลูกหนี้** แทน 1130 = A/R พองผิดถาวร. แก้: extract `_selectSaleMappingKey(sale)` (pure): `is_credit`/เงินเชื่อ→sale_credit_term(1200) · บัตร→sale_credit(1130) · โอน/qr→transfer · else cash. (2) **numpad stale closure** — `pos.js` จอเงินสด handler "เสร็จสิ้น" ใช้ `paid` จาก closure ตอน render แต่ numpad mutate `numpadValue` ไม่ re-render → พิมพ์ numpad ล้วน=ปุ่มตายเงียบ / กด quick+พิมพ์เพิ่ม=ทอนขาด. แก้: handler อ่าน `round2(Number(numpadValue))` สดตอนคลิก. (3) **calendar.js stored XSS** — ไม่ import escHtml เลย, render `job.customer_name/description/job_address` (จากฟอร์มสาธารณะ) ดิบเข้า innerHTML + `title:'${customer_name}'` ใน style attr. แก้: import escHtml + ห่อทุก sink + ย้าย title เป็น attribute จริง. (4) **SQL owner-run (✅ applied+verified 2026-07-02):** `supabase-phase551-period-lock-delete-guard.sql` (trigger `trg_check_period_locked` → BEFORE INSERT OR UPDATE OR **DELETE** — เดิม `voidJvForSource` hard-DELETE ทะลุงวดปิด; verify `events=DELETE,INSERT,UPDATE`) + `supabase-phase551-service-insert-admin-guard.sql` (BEFORE INSERT `service_jobs` → non-admin status∈completion=42501; ปิด residual 4a phase545 gate เฉพาะ UPDATE; verify INSERT/BEFORE). +6 guard test. **build 552** (should-fix): (1) **refund single-flight** — `refunds.js` disable ปุ่มหลัง await confirm → กดรัว=คืน/restock/JV/เครดิตซ้ำ (TOCTOU). แก้: ห่อ handler `createInflightGuard`. (2) **TZ month-bounds** — `utils.js` +`monthBoundsBkk`/`monthsAgoStartBkk` (pure int, TZ-safe) แทนสูตรเดิมที่ปน offset+07:00 กับ setMonth/toISOString → วันสิ้นเดือนหาย (verify `_monthBounds("2026-02")`→`03-03`) ใน hr_overview/leave_management/expense_overview/payroll_overview. (3) **salary JV skip** — `postJournalForExpense` (writer เดียว) ข้ามหมวด salary/labor_hire/payroll (เดิม expenses insert/edit repost Dr 5200 ซ้ำ payroll period JV; backfill skip ภายนอกอยู่แล้ว แต่ caller อื่นลืม). +4 guard. **Verification:** lint 0 · unit **2331/2331** · EOL LF · markers 550→**552** sync (data-app-build+?v=×4+cache-v552+comment) · CI Tests เขียวทั้ง 3 commit. **🎉 owner smoke ผ่าน (preview 552, DB จริง):** เงินสด numpad พิมพ์เอง 100→ทอน 80/100 ถูก · บัตรเครดิต JV **SV2026070015 Dr 1130** ✅ · JV งานช่าง Dr 1130/Cr 4220 ถูก. **ไม่แตะ:** stock CAS/checkout idempotency/RLS อื่น/VAT/todayRevenue(POS+web). **known residual (จด ยังไม่แก้):** dashboard/income_overview service income ใช้ `created_at` ไม่ตรง JV `closed_at` (Phase 542) → การ์ด "รายได้วันนี้ (รวมบริการ)" ต่ำกว่าสมุดรายวัน (owner เจอ 2026-07-02) → **Phase 553 prompt เขียนแล้ว รอ implement team**; + service_jobs โหลด cap 50 (main.js:1161) การ์ดเดือนอาจไม่ครบ = phase แยก. **Status: 🔜 branch `claude/app-audit-j9jpmv` — smoke ครบ กำลัง merge (ลบ workflow preview ชั่วคราวก่อน).**

**build 550 (Phase 550 — tech_home morning-load "อุปกรณ์ต้องเบิกขึ้นรถวันนี้" · read-only · ไม่มี SQL):** **Baseline:** main `39cf482` (build 549 — PR #125). **ที่มา:** owner ส่ง mockup หน้าช่าง (richer) — focus = **สต็อกที่ช่างต้องจัดขึ้นรถทุกเช้า จากงานวันนี้**. **★ audit (verify DB ก่อนทำ):** (1) mockup "อุปกรณ์ต้องเบิก + คงเหลือในคลัง" ต้องมี **"อุปกรณ์วางแผนต่องาน"** — งานวันนี้ 6 (เปิด) มี items_json=**0** (ยังไม่ระบุ). (2) แต่ verify: `_serviceDrawerEquipReadonly` = readonly เฉพาะงานปิด (main.js:2436) → **งานเปิดใส่อุปกรณ์ได้อยู่แล้ว** (picker เดิม) + **ไม่ตัดสต็อกจน close** (Phase 482) → **(A) reuse items_json ได้ทันที ไม่ต้อง schema/write** (owner เลือก A). **แก้ (`modules/tech_home.js` additive):** helper `truckLoadPlan(serviceJobs, warehouses, warehouseStock, products, today)` — งานวันนี้ (`_jobDay=today` active + items_json) → item ที่ `warehouse_id ∈ mobile trucks`: `status∈(pending,progress)→need` (ต้องเบิก) · `∈(done,delivered,closed)→used` (ใช้แล้ววันนี้) → group per (คัน,สินค้า) sum → เทียบ `warehouseStock` (`remain`) → `_loadStatus(need,remain)`: out(`remain≤0` หมด/`remain<need` ไม่พอ)·low(`≤need+2`)·ok·used(need=0) → sort สถานะ(out→low→ok→used)+need desc, slice 20/คัน. render panel หลัก "🚚 อุปกรณ์ต้องเบิกขึ้นรถวันนี้" (border สีตาม outCount/needCount) แถว: ชื่อ+ป้ายสถานะสี · "ต้องเบิก N · คงเหลือในรถ M · +K ใช้แล้ว" + ปุ่ม "เบิกคันนี้ →" (wh_kunkhao/wh_kundaeng) · tile "ต้องเบิกขึ้นรถ"=needCount · greeting sub. คง `recentTruckUsage` (2 วัน) เป็น panel รอง. **ไฟล์:** `modules/tech_home.js` (+truckLoadPlan/_loadStatus/loadHtml/panel) · `tests/tech_home_truck_load_guard.test.js` (ใหม่ 2 behavioral — need/used/remain/status + skip cancelled/other-day/non-truck) · `tests/tech_home_readonly_guard.test.js` (คง 4/4) · build 549→**550** (data-app-build+?v=×4+sw cache-v550+comment+`dashboard_readonly_guard`). **Verification:** lint 0 · unit **2294/2294** · e2e **14/14** (clean; markers 550) · EOL LF · guard read-only เขียว. **ไม่แตะ:** deduct-on-close/drawer/POS/accounting/SQL/RLS/write. **★ known limitation:** panel มีของก็ต่อเมื่อ **ช่างใส่อุปกรณ์ในงาน** (picker พร้อม, งานเปิด=ไม่ตัดจนปิด). **★★ REVIEW PATCH (owner, 2026-07-02):** ปัญหา = จับงานด้วย today/tomorrow เข้มเกิน + `_jobDay` fallback created_at → งานเก่า pending (นัดก่อนวันนี้ หรือ **ไม่มีวันนัด**) หาย/ผิดถัง เพราะ empty "วันนี้ไม่มีงาน". **แก้ (`modules/tech_home.js`):** (1) `serviceJobBuckets(serviceJobs,today,tomorrow)` — `_isOpenJob`=pending/progress (ไม่ done/delivered/closed/cancelled/[ลบแล้ว]) · `_schedDay`=scheduled_date **ไม่ fallback** → **today** `===today` · **tomorrow** `===tomorrow` · **backlog** `<today || ไม่มีวันนัด`. left panel section "งานวันนี้"+"งานค้าง/รอดำเนินการ"; tile "เสร็จแล้ว"→"งานค้าง". (2) `truckLoadPlan` refactor: รับ `jobsForLoad` (=todayJobs+backlogJobs) แทน (serviceJobs,today) → รวม need ต่อรถ · **คืนทุกคัน is_mobile เสมอ** (คันขาว/คันแดง แม้ว่าง="ไม่มีของต้องเบิกคันนี้") · ตัด concept `used`/`_STATUS_ORDER.used`. panel "…วันนี้"→"อุปกรณ์ต้องเบิกขึ้นรถ" + subtitle "งานวันนี้+งานค้าง". behavioral test 2→**5** (buckets today/backlog/tomorrow · วันนี้ว่างแต่ backlog ไม่ว่าง · need/remain/status+ทั้ง2คัน · empty คืน2คัน · no-mutate input). unit 2294→**2297**·e2e 14/14·lint0 (build ยัง 550 — patch บน branch/PR เดิม, ไม่ re-bump). **★★★ MOBILE PATCH (owner, 2026-07-02):** หน้าหลักช่างบนมือถืออ่านไม่รู้เรื่อง — เพราะ `.tech-home-grid` มี `grid-template-columns:1.5fr 1fr` เป็น **inline** และ**ไม่มี CSS/responsive เลย** → ≤640px ยัง 2-col คอลัมน์ขวา (panels) บีบ ~40% แคบ ข้อความแตกทีละคำ. **แก้:** (1) `tech_home.js` ย้าย layout inline → `class="tech-home-grid"` (2) `style.css` เพิ่ม base `.tech-home-grid{2-col}` + `@media (max-width:640px){ .tech-home-grid{grid-template-columns:1fr} + font/line-height tweak scope ใต้ #page-tech_home (panel h3 16px, dash-today-amount 26px, .muted line-height 1.55, ตัด inner max-height scroll) }` — font tweak scope `#page-tech_home` กันรั่ว `.panel/.muted` หน้าอื่น. +`tests/tech_home_mobile_guard.test.js` (4 — breakpoint 640/single-column · base 2-col · JS ไม่ inline 2-col · font scope). **CSS-only render** — ไม่แตะ logic/data/fetch/stock; ship ใต้ **v=550** (ไม่ re-bump: 550 ยังไม่ live prod=549 → CACHE_NAME v549→v550 busts เอง). unit 2297→**2301**·e2e 14/14·lint0. **Status: 🔜 branch `claude/phase-550-truck-load-plan` (PR #126, +review patch +mobile patch) — read-only → owner preview smoke (มือถือ 390px) ก่อน merge.**

**build 549 (Phase 549 — tech_home read-only landing · feature · ✅ MERGED+LIVE `39cf482` PR #125):** **Baseline:** main `4378297` (build 548 — AH4 PR #124). **ที่มา:** owner ส่ง handoff zip (`จำลอง UX UI จากแอป.zip` → INTEGRATION.md + `tech_home.js` + guard test) จากทีมอื่น ให้ **audit ก่อน integrate**; owner สั่ง scope แคบ = **tech_home read-only + scheduled_date fix เท่านั้น · ห้ามขยายเป็นรับงาน/เบิก/ตัด/ปิดงาน** (live system, risk ต่ำสุด). **★ audit (verify จาก source+DB):** read-only จริง (guard 4/4 + no fetch/write/stock-helper/journal, escHtml ครบ, `.filter().sort()` array ใหม่ ไม่ mutate state, bindings=showRoute) · integration facts จริง (utils export escHtml/todayBkk/dateBkk · ROLE_PAGES.technician@646 · LAZY_ROUTES@92 · titles@750 · calendar/stock_movements route จริง · full_name จริง) · **1 บั๊ก:** handoff ใช้ `j.scheduled_at`/`due_date` แต่ **column จริง (DB) = `service_jobs.scheduled_date` (DATE-only "YYYY-MM-DD")** — `scheduled_at`/`due_date` ไม่มี → เดิม `_jobDay` fallback `created_at` = กรอง "งานวันนี้" ด้วยวันสร้างงาน (โชว์ผิด). **แก้:** `_jobDay = j.scheduled_date ? slice(0,10) : dateBkk(created_at)` + sort ด้วย scheduled_date + ตัด time-label (data ไม่มีเวลา). **แก้ `_isActiveJob`** ตัด `deleted_at` (ไม่มี column) เหลือ `[ลบแล้ว]` note + cancelled. **★ owner iterate (panel สต็อก):** เปลี่ยนจาก "ของใกล้หมดทั้งร้าน" → **"สต็อกในรถ 2 คัน" (คันขาว id3/คันแดง id4 = warehouse `is_mobile`)** ตามที่ owner สั่ง "focus แค่สต็อกในรถ". **★ owner iterate #2 (จุดประสงค์จริง):** ไม่ใช่ "ของเหลือน้อยในรถ" (บางรายการขึ้นชิ้นเดียวโดยตั้งใจ) — แต่ = **"ของที่ช่างใช้ล่าสุด จากการตัดสต็อกงาน → เติมเข้ารถพรุ่งนี้"**. **★ data probe (verify DB):** ลอง warehouse_stock 2 รถ = phantom 0-row ~481/คัน + ไม่ตั้ง min_stock(par) → par-based ทำไม่ได้ (dead end). **★ owner iterate #3 (final — "คันไหน?"):** owner ถาม "ช่างใหม่รู้ได้ไง ของนี้ใช้จากคันไหน → เติมคันไหน". stock_movements ไม่มี warehouse_id (dead end) → **verify DB: `service_jobs.items_json` เก็บ `warehouse_id/warehouse_name` ต่ออุปกรณ์** (ตอนช่างเลือกคลัง/คันใน picker) = **ที่เดียวที่บอกคัน** เช่น #195 พี่มัง→รถคันแดง(id4), #191 แม่บุญล้อม→รถคันขาว(id3). **แก้เป็น (final): `recentTruckUsage`** — filter active jobs ที่มี items_json (sort id desc, slice 25) → flatten items เฉพาะ `warehouse_id ∈ mobile trucks` (state.warehouses is_mobile) → **group per รถ** {name×qty, ลูกค้า(job.customer_name), day} slice 12/คัน. render group ต่อคัน (🚐 คันแดง ใช้ 5 · 🚐 คันขาว ใช้ 33) + ปุ่ม "เติมคันนี้ →" `data-go=wh_kundaeng/wh_kunkhao`. panel "🔧 ของที่ช่างใช้ล่าสุด · แยกตามรถ" + "ประวัติ →" `data-go="stock_movements"` (guard-allowed). tile "ช่างใช้วันนี้"=Σ items day===today. **★ owner iterate #4 (refine):** (1) **รวม qty ต่อสินค้า/คัน** (byP Map, sum qty + jobs count + lastDay) (2) **กรอง 2 วันล่าสุด** (วันนี้+เมื่อวาน, `days=2` param) (3) เรียง lastDay/qty desc + label วันนี้/เมื่อวาน. ⚠️ **data sparse:** items_json บันทึกแค่ 8/60 งานล่าสุด → 2 วันได้แค่ 1 งาน (คันแดง จานดาวเทียม×1) → ถ้าอยากเห็นมากขึ้น = ขยาย `days` (owner ตัดสิน) หรือแก้ให้ทุกงานบันทึก items_json (เฟสอื่น). **ไฟล์:** `modules/tech_home.js` (ใหม่ · read-only: shopJobsForDay + `recentJobUsage`(stockMovements งานช่าง out) + renderTechHome) · `tests/tech_home_readonly_guard.test.js` (ใหม่ 4 — FORBIDDEN write-paths/stock/journal, allow route-nav, export, state-read, showRoute-only) · `main.js` +4 additive (`LAZY_ROUTES` tech_home + `ALL_ROUTES` tech_home[admin] + `ROLE_PAGES.technician`[แรก] + `titles`) · `index.html` +2 (ปุ่ม `data-route="tech_home"` ในกลุ่มงานประจำวัน + `<section id="page-tech_home" class="page hidden">`) · build 548→**549** (data-app-build+?v=×4+sw cache-v549+comment+`dashboard_readonly_guard`). PRECACHE_URLS = core-only → lazy tech_home ไม่ต้องใส่. **ไม่แตะ:** logic เดิม · SQL/RLS · POS/checkout/stock write/service-job write/accounting · ไม่มีปุ่มรับงาน/เบิก/ตัด/ปิดงาน (data-go = นำทางไปหน้าเดิมที่มี guard เท่านั้น). **Verification:** lint:errors 0 · unit **2292/2292** (tech_home guard 4 + dashboard guard ไม่ regress) · e2e: markers 549 curl-verified (title/build/button/section served ถูก) — **local static-load smoke 3 fail = stuck-SW escalate; fail บน clean 548 ด้วย (stash-verify) → ไม่ใช่จากแก้นี้ → CI fresh-env authoritative** · EOL LF · node --check main.js/tech_home OK. **★ incident:** `git stash push -- <untracked paths>` ล้มเหลว → `git stash pop` ดึง stash เก่า session อื่น (phase-92-22e-wip) มา conflict time_clock → กู้ `git checkout HEAD -- time_clock.js/test` (stash เก่าไม่ drop, session อื่นปลอดภัย) [[feedback-parallel-work-check]]. **owner smoke (preview):** login ช่าง → เห็นปุ่ม 🧰 หน้าหลักช่าง → เปิดได้ (งานวันนี้/พรุ่งนี้/low-stock จาก state จริง) · Network ไม่มี POST/PATCH/DELETE · admin เปิดดูได้ · ทุกหน้าเดิม regression ok. **residual/เฟสถัดไป (แยก):** งานของฉัน (assigned_to ที่มีอยู่) · BOM/planned_parts · ปุ่มรับงาน/เบิกจริง (ต่อ path เขียนเดิม). **Status: 🔜 branch `claude/phase-549-tech-home` — read-only additive → owner preview smoke ก่อน merge.**

**build 548 (Phase 548 — AH4: auto_post JV round2 + exact balance · §4.3 accounting · ✅ MERGED+LIVE `4378297` PR #124 · smoke #220/#221 balanced):** **Baseline:** main `e2e5791` (build 547 — PR #123). **★ cross-team audit (analyst) confirmed:** อีกทีม (Codex) cross-check ถูกทั้ง 4 จุด (S6✅ปิด Phase541 · S12🔴open · S3 code-fixed · scope gap: ผมพลาด `BLUEPRINT_PHASE_545.md` มี AH4/AC1). reviewer verify ทุก claim จาก source เอง. **บั๊ก (AH4, verified `auto_post.js:238`):** `_postJournal` validate balance ด้วย tolerance `Math.abs(totalDebit-totalCredit) > 0.01` — DB balance trigger Phase 498 (`trg_check_je_lines_balance` DEFERRED) บังคับ **exact `NUMERIC(14,2)`** → JV ที่มี satang drift ≤1 หลุด client แต่ DB reject lines → **orphan header** (§4.3 accounting correctness). **แก้ (`modules/accounting/auto_post.js` `_postJournal` เท่านั้น):** `const _rd = (n)=>round2(Number(n||0))` · `rLines = lines.map(l=>({...l, debit:_rd(l.debit), credit:_rd(l.credit)}))` · `totalDebit/totalCredit = _rd(rLines.reduce(...))` · guard `if (totalDebit !== totalCredit)` (exact) · journal_lines POST ใช้ `rLines` (ค่า round2 ตรง header + DB) · `_usedCredit` เช็คบน `rLines`. ค่า 2dp เดียวกันไหลทั้ง validate+header+lines → ไม่มี drift. **behavior:** JV balanced (Dr=Cr exact) → เหมือนเดิม ผ่าน; JV drift ≤1 สตางค์ → เดิมผ่าน client แต่กลายเป็น orphan ที่ DB → **ใหม่ fail คลีน** (`status:failed reason:unbalanced`) = safer. lines จาก `buildSaleDebitLines/splitSaleVatLines` round2 อยู่แล้ว → real-world ไม่ regress. **ไม่แตะ:** mapping/VAT split/Dr-Cr/effective-gate/period-lock check/doc_no retry loop/rollback logic. **ไฟล์:** `modules/accounting/auto_post.js` · `tests/auto_post_ah4_round2_guard.test.js` (ใหม่ 4 — round2 rLines · header จาก rLines · exact compare ไม่มี ±0.01 · lines POST ใช้ rLines) · อัปเดต `tests/auto_post.test.js` (balance guard เดิม pin `Math.abs>0.01` → exact+round2) + `tests/auto_post_orphan_rollback_guard.test.js` (`_usedCredit` `lines`→`rLines`) · `DB_MIGRATIONS_APPLIED.md` (+log **S3/phase536** verified live) · build 547→**548** (data-app-build+?v=×4+sw cache-v548+comment+`dashboard_readonly_guard`). **Verification:** lint:errors 0 · unit **2288/2288** (auto_post 3 ไฟล์เดิม + jv_docno_clash ไม่ regress) · e2e **14/14** (clean; markers 548) · EOL LF. **S3 log:** phase536 (`error_log_select_admin USING(is_admin())`) verified live 2026-07-01 (admin rows=1 / non-admin rows=0). **canonical open หลัง 548:** AC1 (defensive, low) · S12 (reporting) · S15 (latent) · S8 (edge) · AC2/AC3/AH1 (JV roadmap deferred) · N1/N2/N4. **owner smoke (go-live):** ขายบิลจริง → JV posted balanced (เหมือนเดิม, ไม่มี fail ใหม่กับบิลปกติ). **Status: 🔜 branch `claude/phase-548-ah4-round2-exact` — accounting บน go-live day → owner review/smoke ก่อน merge (แนะนำ preview).**

**build 547 (Phase 547 — products page live filter search · Codex · UI-only · ✅ MERGED+LIVE `e2e5791` PR #123):** **Baseline:** main `f9b5ae2` (build 546 — PR #122). **เปลี่ยน:** หน้าสินค้ากลับมา **live filter ขณะพิมพ์** (`modules/products.js` renderView: `_prodSearchEl.addEventListener("input")` → `_scheduleProdSearch` → `setTimeout(_runProdSearch(true), 220)` debounce; `compositionstart/end` IME guard → `_prodSearchComposing` กัน re-render ระหว่างพิมพ์ไทย, ค้นหลัง composition commit; Enter/blur เดิมคงไว้). **🔄 ย้อน build 524** (ค้นเมื่อ Enter/blur, กัน mobile keyboard เด้ง) — Codex ทำ, **owner ยืนยัน intentional**. **★ review (analyst) flag แล้ว:** debounce ไม่ลบ root cause (renderView สร้าง `#prodSearchInput` ใหม่ + refocus → เด้งได้หลัง 220ms) → **owner เทสต์ preview บนมือถือจริงผ่าน** (`codex-phase-547-products-liv.pages.dev`, PR #123) ก่อนอนุมัติ merge. guard test `products_search_ime_guard.test.js` **กลับด้าน** (เดิม assert `must NOT render on input`=Phase 524 → ใหม่ assert live filtering+debounce 220+composition guard). **ไฟล์:** `modules/products.js` + `tests/products_search_ime_guard.test.js` (Codex) + build 546→**547** (data-app-build+?v=×4+sw cache-v547+comment+`dashboard_readonly_guard`) เพิ่มโดย reviewer (Codex ไม่ bump). **Verification:** lint:errors 0 · unit **2284/2284** · e2e **14/14** (clean; markers 547) · EOL LF · node --check/git diff --check ผ่าน. **ไม่มี SQL.** **memory:** อัปเดต `project-products-search-on-enter` = live search กลับมาแล้ว (เดิม 🚫 อย่า re-add live → owner reverse หลังเทสต์ preview ผ่าน). **residual/known:** ถ้ามือถือรุ่นอื่นยังเด้ง = follow-up filter list-in-place (แต่ระวังปุ่มแก้-ลบ ตาม memory). **Status: 🔜 branch `codex/phase-547-products-live-search` PR #123 — owner preview ผ่าน → merge.**

**build 546 (Phase 546 — service job delete/cancel = admin-only · §4.4 · ✅ MERGED+LIVE `f9b5ae2` PR #122 · SQL applied+verified 2026-07-01):** **Baseline:** main `c542311` (build 545 — PR #121, SQL applied 2026-06-30). **บั๊ก (verified):** "ลบ" ใบรับงาน = **soft-delete** (`service_jobs.js` delete handler@345: `status='cancelled'` + note `[ลบแล้ว] ลบโดยแอดมิน ...` + `restoreServiceJobStock`; list ซ่อน row ที่ cancelled+`[ลบแล้ว]`@64) แต่ปุ่ม 🗑️ ลบ@254 render ทุก role + handler **ไม่มี role gate** (note บอก "ลบโดยแอดมิน" แต่ไม่ enforce) → technician/sales/accountant/customer ลบได้ทั้ง UI + REST PATCH ตรง. **แก้ (`modules/service_jobs.js`):** (1) `const _isAdmin = state.profile?.role === "admin"` ใน `renderServiceJobsPage`; ปุ่ม 🗑️ ลบ wrap `${_isAdmin ? \`...\` : ''}` (**ไม่ซ่อนทั้งหน้า** — technician เห็น/แก้งานได้ตามเดิม). (2) handler `[data-del-job]` click: guard `if (state.profile?.role !== "admin") { showToast?.("เฉพาะแอดมินลบงานได้","error"); return; }` **ก่อน** confirm/PATCH/`restoreServiceJobStock`. (3) **DB** `supabase-phase546-service-delete-admin-guard.sql`: `_guard_service_jobs_delete()` (SECURITY DEFINER) + `trg_service_jobs_delete_guard BEFORE UPDATE ON service_jobs` — block `auth.uid() IS NOT NULL AND NOT is_admin() AND ((NEW.status='cancelled' AND OLD.status IS DISTINCT FROM NEW.status) OR (NEW.note LIKE '%[ลบแล้ว]%' AND OLD.note NOT LIKE '%[ลบแล้ว]%'))` → RAISE 42501. **transition/add-only:** (a) cancel transition เท่านั้น (งาน cancelled อยู่แล้ว+แก้ note ผ่าน) · (b) marker เพิ่มใหม่เท่านั้น (แก้ note ปกติผ่าน); `auth.uid() IS NULL` (service_role) ไม่ gate; **independent จาก 545** (trigger คนละตัว — cancel ไม่ใช่ completion → 545 ไม่ยิง). **policy:** technician/sales/accountant/customer ลบ/ยกเลิกไม่ได้; admin ลบได้เดิม (JV/stock restore ทำงาน). **ไม่แตะ:** 545 close guard · POS/credit/loyalty/accounting · main.js · RLS อื่น. **ไฟล์:** `modules/service_jobs.js` · `supabase-phase546-service-delete-admin-guard.sql` (ใหม่ owner-run) · `tests/service_job_delete_admin_guard.test.js` (ใหม่ 5 — UI admin-only+ไม่ซ่อนหน้า · handler guard-before-confirm/PATCH/restore · SQL cancel-transition/[ลบแล้ว]-add-only/is_admin/42501/null-uid · 545 intact) · build 545→**546** (data-app-build+?v=×4+sw cache-v546+comment+`dashboard_readonly_guard`). **Verification:** lint:errors 0 · unit **2283/2283** (service_status/close_admin/submit_api/equipment_deduct ไม่ regress) · e2e **14/14** (clean; markers 546 curl-verified) · EOL LF. **🔴 owner รัน SQL** `supabase-phase546-service-delete-admin-guard.sql` → log `DB_MIGRATIONS_APPLIED.md` (PENDING). **known residual:** drawer dropdown "ยกเลิก" (main.js) สำหรับ non-admin ยังเลือกได้ → DB trigger 42501 (ไม่ใช่ delete-button path; ถ้าจะ smooth UX = follow-up ซ่อน option cancelled แบบ 545 — scope นี้คุม delete button + DB ครบแล้ว). **owner smoke (หลังรัน SQL + deploy):** technician (logout+hard refresh) → service_jobs: **ไม่เห็นปุ่ม 🗑️ ลบ**, ยังเห็น/แก้งานได้ · PATCH status=cancelled ตรง REST → **42501** · เพิ่ม note `[ลบแล้ว]` ตรง REST → **42501** · แก้ note ธรรมดา → ผ่าน · admin → ลบได้ปกติ. **Status: 🔜 branch `claude/phase-546-service-delete-admin-only` — รอ owner review + รัน SQL + smoke.**

**build 545 (Phase 545 — service job close = admin-only · §4.4 · ✅ MERGED+LIVE `c542311` PR #121 · SQL applied+verified 2026-06-30):** **Baseline:** main `41a85f6` (build 544 — PR #119 + docs #120). **บั๊ก (verified):** ปิดงานช่าง (status→done/delivered/closed) ทริก auto-post JV (`postJournalForServiceJob`@main.js:3049) + ตัดสต็อกจริง (`_equipDeductOnClose`@3070) แต่ `saveServiceJob`@2865 **ไม่มี role gate เลย** → technician/sales/ใครก็ปิดงานได้. **★ audit-before-implement (owner ยืนยัน):** vocabulary — `pending_review`/`in_progress` **ไม่ใช่ DB status** (constraint Phase 383 = `pending/progress/done/delivered/closed/cancelled`); `pending_review` = UI-pseudo → `normalizeServiceJobStatus`→`pending`+note marker; ด่านจริงต้องเป็น DB (client = UX, bypass REST ได้ + JV/stock เป็น client-side → bypass ได้ row delivered ที่ไม่มี JV = บัญชีเพี้ยน); owner เลือก **transition-only** (ไม่ absolute — แก้งานปิดแล้วได้). **แก้ 3 ชั้น:** (1) **`service_status.js`** (pure, testable): `SERVICE_COMPLETION_STATUSES=["done","delivered","closed"]` · `isServiceCompletionStatus(s)` · `isServiceCloseTransition(from,to)` = `!completion(from) && completion(to)`. (2) **`main.js` client:** `openServiceJobDrawer`@2412 หลัง set status → loop options: `!requireAdmin() && isServiceCompletionStatus(opt.value) && opt.value!==curStatus` → `opt.disabled=opt.hidden=true` (ซ่อน completion สำหรับ non-admin; คงสถานะปัจจุบันงานที่ปิดแล้วให้แสดง; admin re-enable ทุกครั้ง). `saveServiceJob` หลัง validation ก่อน `let res` (ก่อน closed_at@2923/JV/stock): `if (!requireAdmin() && isServiceCloseTransition(state.editingServiceJobOrigStatus, payload.status)) { _signalSave(false,...); return showToast("เฉพาะแอดมิน...","error"); }`. (3) **DB** `supabase-phase545-service-close-admin-guard.sql`: `_guard_service_jobs_close()` (SECURITY DEFINER) + `trg_service_jobs_close_guard BEFORE UPDATE ON service_jobs` — `IF OLD.status DISTINCT FROM NEW.status AND OLD.status NOT IN (completion) AND NEW.status IN (completion) AND auth.uid() IS NOT NULL AND NOT public.is_admin() → RAISE 42501`. reuse `public.is_admin()` (phase495). **transition-only:** OLD เป็น completion อยู่แล้ว → ไม่ block (แก้ note งานปิดได้); `auth.uid() IS NULL` (service_role) ไม่ gate. **policy ไม่เปลี่ยน:** technician ยังเข้า service_jobs/แก้งาน/ส่ง pending_review (→pending, ไม่ใช่ completion = ผ่าน); admin ปิดงาน JV+stock ทำงานเดิม. **ไม่แตะ:** POS/credit/loyalty/accounting/stock helper อื่น/RLS อื่น/effective-gate/period-lock. **ไฟล์:** `modules/service_status.js` · `main.js` · `supabase-phase545-service-close-admin-guard.sql` (ใหม่ — owner-run) · `tests/service_job_close_admin_guard.test.js` (ใหม่ 8 — pure helpers [completion set · transition-only · pending_review→pending allowed] + source client [drawer hide · save guard ก่อน closed_at/JV/stock + return] + source SQL [BEFORE UPDATE/transition-only/is_admin/42501/auth.uid null bypass]) · build 544→**545** (data-app-build+?v=×4+sw cache-v545+comment+`dashboard_readonly_guard`). **Verification:** lint:errors 0 · unit **2278/2278** (4 service guard เดิม service_status/submit_api/equipment_deduct/pos_jv ไม่ regress) · e2e **14/14** (clean run; markers 545 ครบ curl-verified) · EOL LF. **🔴 owner ต้องรัน SQL** `supabase-phase545-service-close-admin-guard.sql` ใน Supabase SQL Editor (DDL — Claude/CI ไม่รัน) → log ใน `DB_MIGRATIONS_APPLIED.md` (ตอนนี้ = PENDING). **known residual:** INSERT path ไม่ถูก trigger gate (non-admin INSERT งานใหม่ status=delivered ตรง REST — แต่ UI ไม่เปิดให้ + save guard client คุม transition `""→completion`; ถ้าต้องการปิด INSERT ด้วย = follow-up เพิ่ม BEFORE INSERT). **owner smoke (หลังรัน SQL + deploy):** technician login (logout+hard refresh ก่อน) → แก้งานช่าง: dropdown **ไม่มี** ส่งมอบแล้ว/ปิดงาน/เสร็จแล้ว · เลือกรออนุมัติได้ (ไม่มี JV/stock) · ลอง PATCH status=delivered ตรง REST → **42501** · admin → ปิดงานได้ JV+stock ทำงาน. **Status: 🔜 branch `claude/phase-545-service-close-admin-only` — รอ owner review + รัน SQL + smoke (security + ต้อง apply SQL ก่อนถึงจะครบด่าน).**

**build 544 (Phase 544 — internal stock issue for lump-sum sales docs · feature · ไม่มี SQL · ✅ MERGED+LIVE `1b291ae` PR #119):** **Baseline:** main `0e15765` (build 543 — S14 PR #118). **เป้าหมาย:** งานขายเหมาจ่าย — ลูกค้าเห็นแค่ line เหมาจ่าย (จาก `delivery_invoice_items`) แต่หลังบ้านตัดสต็อกจริงได้. **★ audit-before-implement (owner ตัดสิน idempotency = per-(doc+product+warehouse)):** verify จาก source: `_appApplyStockMovement` signature ตรง prompt + CAS/floor + insufficient→return ก่อน log (ไม่มี phantom) ✓ · **stock_movements schema = `id,product_id,type,qty,note,created_by,created_at` เท่านั้น (main.js:3887) — ไม่มี source_*/warehouse_id col** → idempotency ต้องผ่าน **note marker** + คลังฝังใน note · helper คืนแค่ `{ok:true}` ไม่คืน id + ไม่เช็คผล log insert → ต้อง re-query ยืนยัน · print/PDF/share อ่าน `#diDocPreview.innerHTML` → internal UI ต้องอยู่นอก. **แก้ (`modules/delivery_invoices.js` เป็นหลัก):** core (export, testable): `internalStockMarker(docType,docId,productId,warehouseId)`=`[SALES_INTERNAL_STOCK:...]` · `internalStockNote(...)` · `issueInternalStockRow(...)` (query marker→มีแล้ว skip / `_appApplyStockMovement` out+allowNegative:false / re-query ยืนยัน `logConfirmed`) · `fetchIssuedInternalStock(docType,docId)` (prefix per doc). UI (amend — owner ขอ "เหมือนหน้างานช่าง"): แผง "ตัดสต็อกภายใน" ใน `renderInvoicePreview` **วางก่อน `#diDocPreview` (sibling นอก)** — role-gated `["admin","sales"]`; **reuse picker เดียวกับงานช่าง** (`openEquipmentPicker`/`renderEquipmentList`/`precheckEquipmentStock` จาก service_equipment.js): ปุ่ม "➕ เพิ่มสินค้า" → picker warehouse-first (chips คลัง/หมวด/ค้นหา/ตะกร้าสด multi-add, onPick={product_id,warehouse_id,name,qty,unit_price,...}) → `_diInternalCart` → cart `renderEquipmentList` (qty/ลบ mirror main.js:2455) → ปุ่ม "ยืนยันตัดสต็อก (N)" precheck ทั้งตะกร้า (`precheckEquipmentStock`) ก่อน loop; handler wrap `_internalStockGuard.run` (single-flight) → toast (ok+logConfirmed=success · ok+!logConfirmed=warning "ตัดแล้วแต่ log ไม่ยืนยัน" · skipped=already-issued · insufficient/error). **idempotency per (doc,product,warehouse)** — สินค้า+คลังเดิมซ้ำไม่ได้ แต่ append สินค้า/คลังอื่นได้. **ไม่เพิ่มเข้า `delivery_invoice_items`/`receipt_items`/`quotation_items`** (customer doc คงเหมาจ่าย — verify ใน guard). **ไม่แตะ:** POS checkout/credit/loyalty/JV/receipt rendering/quotation pricing/customer item rendering/print template/SQL · ไม่ mutate warehouse_stock ตรง (helper CAS owns). **known residual:** (a) idempotency note-marker = read-then-write (TOCTOU) → กันด้วย inflight (same-session) + marker + button disable; cross-device พร้อมกัน residual (manual action ความถี่ต่ำ; floor กันติดลบ) (b) helper ไม่เช็ค log insert → ตัดแล้ว log fail → `logConfirmed:false` → warning (ไม่เงียบ) (c) UX = single-row submit append (เพิ่มหลายรายการ = กดทีละตัว; functionally = multi-row). **ไฟล์:** `modules/delivery_invoices.js` · `tests/sales_internal_stock_issue_guard.test.js` (ใหม่ 12) · `tests/dashboard_readonly_guard.test.js` · build 543→**544** (data-app-build+?v=×4+sw cache-v544+comment). **Verification:** lint:errors 0 · unit **2269/2269** · e2e: ไฟล์ markers 544 ครบ (curl static-server ยืนยัน data-app-build/selfheal/main/boot/style ?v=544 + sw cache-v544; unit "bumped to 544" ผ่าน) — **local e2e fail = stuck SW serve cached เก่า (environmental, fail บน clean ด้วย) → CI fresh-env authoritative** (542/543 pattern เดียวกันผ่าน CI). · EOL LF. **owner smoke:** เปิดใบส่งสินค้าเหมาจ่าย → "ตัดสต็อกภายใน" เลือกสินค้า+คลัง+qty 1 → ยืนยัน → stock_movements row ใหม่ note มี marker + stock ลดถูก · print/PDF/แชร์ ไม่เห็น internal · กดซ้ำสินค้า+คลังเดิม → block · qty เกิน → error stock ไม่เปลี่ยน. **residual audit:** S12 bundle gross_profit · S15 OTP KV fail-open. **Status: 🔜 branch `claude/phase-544-sales-internal-stock-issue` — รอ owner review + smoke (แตะ stock จริง — owner smoke preview ก่อน merge แนะนำ).**

**build 543 (Phase 543 / S14 — SlipOK key → server-side proxy · §4.4 · functions+client · ✅ MERGED+LIVE `0e15765` PR #118):** **Baseline:** main `3dc96c5` (build 542 — AH6 PR #117). **บั๊ก (verified from source):** `customer_dashboard.js:88 _verifySlip` อ่าน `localStorage["bsk_slipok_key"]` + ยิง `api.slipok.com/api/line/apikey/{branch}` ตรงจากเบราว์เซอร์ ด้วย `x-authorization: <key>` → คีย์ SlipOK ของร้านรั่ว (localStorage + Network tab; XSS ขโมยได้). `payment.js:177/341` แสดง+เก็บ key ใน localStorage. **★ audit-before-implement:** ตรวจ `/api/verify-slip` = **Gemini OCR** (GEMINI_API_KEY) จริง — prompt ถูก → SlipOK เป็นคนละ service → สร้าง `/api/verify-slipok` แยก. caller (`cd:989`) **ไม่ใช้ `result.raw`** → ตัด raw ปลอดภัย. pages.js export **ไม่ export key อยู่แล้ว** (`:211` comment) แต่ restore (`:242`) re-add `bsk_*` ทุกตัว → ต้องกัน. **แก้ (4 จุด):** (1) **`functions/api/verify-slipok.js`** (ใหม่): POST, `SLIPOK_API_KEY` required (ไม่มี→`{valid:false,configured:false,error:"no_api"}` graceful) + `SLIPOK_BRANCH_ID` default "0"; validate data-URL base64 (cap ~6MB)→Blob/FormData(`files`+`amount`)→fetch `api.slipok.com` `x-authorization`; normalize `{valid,amountMatch,amount,sender,receiver,transRef,date}` (**ไม่มี raw**); reuse `_error_sanitizer.logServerError` (log server, client generic); invalid/upstream-error→generic message ไม่ echo upstream. (2) **`customer_dashboard.js _verifySlip`**: `fetch("/api/verify-slipok")` ด้วย `Authorization: Bearer window._sbAccessToken` + body `{image, expected_amount}`; ไม่มี token/401→`{error:"auth","เซสชันหมดอายุ"}`; คืน proxy result (shape เดิม); **เลิก localStorage key + api.slipok.com**. (3) **`payment.js`**: ลบ input key (`setSlipOkKey`) → ข้อความ env-instruction read-only; save → `removeItem("bsk_slipok_key"/"bsk_slipok_branch")` cleanup (ไม่ setItem key อีก). (4) **`pages.js`**: restore deny-list `bsk_slipok_key` (`_RESTORE_DENY`). **middleware:** `verify-slipok` ใน RATE_LIMITS (20/60) + REQUIRE_AUTH_ENDPOINTS (customer JWT ผ่าน — **ไม่ STAFF_ONLY** เพราะ customer dashboard ใช้). **ไม่แตะ:** `/api/verify-slip` (Gemini)/service_form/ac_install/solar/checkout/sales/accounting/stock/RLS/SQL/GEMINI/LINE. no_api graceful (caller `cd:996` `error==="no_api"`→`_custSlipVerified=true` ผ่าน manual). **ไฟล์:** `functions/api/verify-slipok.js` (ใหม่) · `functions/_middleware.js` · `modules/customer_dashboard.js` · `modules/settings/payment.js` · `modules/settings/pages.js` · `tests/slipok_secret_proxy_guard.test.js` (ใหม่ 10 — source: cd ไม่มี key/api.slipok/x-auth + เรียก proxy+JWT · payment ไม่ setItem key+removeItem · pages restore deny · function env/no-hardcode/no-raw/logServerError · middleware auth+rate+not-staff-only · behavioral: no_api graceful · valid→normalized ไม่มี raw + **key ไม่โผล่ใน response** · bad-image 400 ไม่ยิง upstream) · build 542→**543** (data-app-build+?v=×4+sw cache-v543+comment+`dashboard_readonly_guard`). **Verification:** lint:errors 0 · unit **2257/2257** (verify_slip_auth + function_error_sanitizer เดิมไม่ regress) · e2e build-sync เขียว (APP_BUILD smoke = local SW-state flake; fail บน clean 542 ด้วย → ไม่ใช่จากแก้นี้ → CI fresh env เขียว) · EOL LF · grep proof: `SLIPOK_API_KEY`/`api.slipok.com`/`x-authorization` (call) เฉพาะ `verify-slipok.js`; `bsk_slipok_key` ใน code เฉพาะ cleanup/deny. **🔴 owner ต้องตั้ง Cloudflare env หลัง merge:** `SLIPOK_API_KEY` (+ optional `SLIPOK_BRANCH_ID`). **owner smoke:** customer login → upload slip → Network เรียก `/api/verify-slipok` (ไม่เห็น x-authorization/key) · localStorage ไม่มี `bsk_slipok_key` · env ไม่ตั้ง → "SlipOK ยังไม่พร้อม" graceful · staff `/api/verify-slip` (service/ac/solar) ยังใช้ได้. **residual:** S12 bundle gross_profit · S15 OTP KV fail-open. **Status: 🔜 branch `claude/phase-543-slipok-server-proxy` — รอ owner review/merge + ตั้ง env.**

**build 542 (Phase 542 / AH6 — service JV docDate = closed_at (วันปิดงาน=วันรับรู้รายได้) · §4.3 · ไม่มี SQL · ✅ MERGED+LIVE `3dc96c5` PR #117):** **Baseline:** main `58b2235` (build 541 — S6 PR #116). **บั๊ก (verified from source):** `postJournalForServiceJob` (`auto_post.js:802`) ลง JV ตอนงานปิด (status delivered/closed/done) แต่ `docDate = job.created_at` (วันสร้าง/นัด). งานสร้างเดือนนึงปิดอีกเดือน → JV ลงผิดงวด. **คาบ go-live 1 ก.ค. ร้ายสุด:** สร้าง มิ.ย. (pre-effective) ปิด ก.ค. → docDate=created_at(มิ.ย.) < `_isAfterEffective` → JV **skip ทั้งใบ = รายได้หาย** (ไม่ใช่แค่ผิดเดือน). **★ audit-before-implement (owner อนุมัติ fold):** prompt บอก `main.js:3036 fullJob = full row มี closed_at` — **verify แล้วผิด:** main.js **ไม่เคย set closed_at** (grep ทั้งไฟล์=0; set แค่ใน ac_install/service_form/solar) → งานปิดผ่าน **drawer ทั่วไป** (transitionedToDone) closed_at=null → fix auto_post เฉย ๆ = **no-op สำหรับ path ปิดงานหลัก**. ต้อง fold main.js ด้วย. **แก้ (2 ไฟล์):** (1) **`auto_post.js:802`**: `const _docBasis = job.closed_at || job.created_at; const docDate = _docBasis ? dateBkk(_docBasis) : todayBkk();` (fallback created_at เมื่อ closed_at null; effective gate `_isAfterEffective(docDate)` ยังทำงานบน docDate ใหม่). (2) **`main.js saveServiceJob`** (ก่อน xhrPatch/Post ที่ :2924/2928): stamp `payload.closed_at = new Date().toISOString()` เฉพาะตอน `_isClosing && !_wasClosed && !payload.closed_at` (เพิ่งเข้าสถานะปิด done/delivered/closed + ไม่เคยปิด + ยังไม่ set) → persist ลง DB + fullJob (:3029 spread payload) → auto_post ใช้ได้. **ไม่ทับของเดิม** → edit งานที่ปิดแล้ว (editCompleteWithChange) ไม่เลื่อนวันปิด. **inline form (ac_install:548/service_form:534/solar:837) set closed_at เองอยู่แล้ว + ส่ง created_at=now → ไม่แตะ** (docDate=วันปิด correct อยู่แล้ว). **behavior ตั้งใจ:** งานสร้างก่อน 1 ก.ค. ปิดหลัง → เดิม skip → ใหม่ post (closed≥effective) = ถูก (รับเงินหลัง go-live ต้องลงบัญชี). **ไม่แตะ:** mapping/keyMap/lines/amount/Dr-Cr/payment_method override · effective gate logic (:803) · period-lock (closed_at ตกงวดปิด → trigger block = ถูก ห้าม bypass) · postJournalForSale/Expense/Receipt/DeliveryInvoice · effective gate ของ sale/expense (created_at ถูกแล้ว). **ไฟล์:** `modules/accounting/auto_post.js` · `main.js` · `tests/service_job_docdate_guard.test.js` (ใหม่ 6 — behavioral: created มิ.ย.+closed ก.ค.→posted งวด ก.ค. (docNo SV202607) · created+closed ก.ค.→ก.ค. · closed_at null+created ก.ค.→fallback posted · created มิ.ย.+closed_at null→skip pre-effective docDate มิ.ย. · source auto_post `closed_at||created_at` · source main.js stamp closed_at) · `tests/service_equipment_deduct_on_close.test.js` (widen slice window 12000→14000 เพราะ closed_at block ดัน deduct-on-close — ไม่ลด assertion) · build 541→**542** (data-app-build + ?v=×4 + sw cache-v542 + comment + `dashboard_readonly_guard`). **Verification:** lint:errors 0 · unit **2248/2248** (auto_post + auto_post_detailed + service_equipment_deduct เดิมไม่ regress) · e2e **14/14** (build 542 sync) · EOL LF. **closed_at column มีอยู่แล้ว** (`supabase-phase88-accounting-foundation.sql:115` + `phase88-service-mappings.sql:20`) → **ไม่ต้องรัน SQL**. **owner smoke (หลัง go-live หรือ test data docDate≥1ก.ค.):** ปิดงานช่างผ่าน drawer ทั่วไป (สร้างวันก่อน ปิดวันนี้) → JV docDate=วันปิด (สมุดรายวัน SV เดือนปิด) · งานสร้าง มิ.ย. ปิด ก.ค. → JV ลง ก.ค. (ไม่ skip) · ปิดซ้ำ/edit → ไม่เลื่อนวันปิด. **residual:** S12 bundle gross_profit · S14 SlipOK key localStorage · S15 OTP KV fail-open. **Status: 🔜 branch `claude/phase-542-service-jv-closed-at` — รอ owner review + merge (🔴 ควร live ก่อน 1 ก.ค.).**

**build 541 (Phase 541 / S6 — loyalty clawback reads DB source-of-truth · BUG_AUDIT 2026-06-25 · §4.1 · client read-fix, ไม่มี SQL · ✅ MERGED+LIVE `58b2235` PR #116):** **Baseline:** main `1791d9a` (build 540 — S5 redeem RPC PR #115). **บั๊ก (verified from source `loyalty.js`):** `reverseEarnedPointsForSale(saleId, options)` (เรียกตอน void/refund clawback) อ่าน earn rows / idempotency / remaining จาก `state.loyaltyPoints` = in-memory cache cap ≤500 (`project_loadalldata_contract`: loyalty_points โหลด ≤500). บิล/earn ที่อยู่นอก cache → `earnRecords=[]` → skip "no earn records" ทั้งที่ DB มี earn จริง → **ไม่คืนแต้มตอน void/refund** (หรือ remaining คิดจาก cache บางส่วน → cap ผิด). **★ audit-before-implement:** prompt เขียน signature `reverseEarnedPointsForSale(sale, ctx)` + ใช้ `sale.customer_id`/`sale.id` — **ผิด!** ของจริง `(saleId, options={})` (saleId ตัวแรก) + 2 caller (`refunds.js:608` · `sales.js:320`) ส่ง `(_selectedSale.id, {state,customerId,refundId})` → ถ้าทำตาม prompt = พัง void+refund ทันที. owner อนุมัติ fold audit ก่อน implement. **แก้ (`loyalty.js reverseEarnedPointsForSale` เท่านั้น):** import `fetchAllRowsRaw` (fetch_paginated.js — raw-URL paginate, **throw เมื่อ HTTP error** ไม่ truncate เงียบ); (1) fetch earn+sale_reverse ของ sale: `loyalty_points?ref_id=eq.{saleId}&ref_type=in.(sale,sale_reverse)` → earnRecords + idempotency (DB sale_reverse row) ; (2) resolve customer = `options.customerId ?? earnRecords[0].customer_id` (จาก DB ไม่ใช่ `sale.customer_id`) ; (3) remaining = fetch `loyalty_points?customer_id=eq.{cid}` → SUM(earn)−SUM(redeem) จาก DB ; reverseAmount = min(totalEarned, max(remaining,0)). **DB fetch fail → return `{ok:false, skipped:false, retrySafe:true}`** (ไม่ silent skip ไม่ insert) → caller Phase 539 (`sales.js`) push `failures` → warning toast / `refunds.js:619` console.warn. idempotency คงครบ: DB pre-check + phase497 23505 (`uq_loyalty_sale_reverse`) → idempotent skip. **ลบ `customerRemainingFromState`** (private dead helper หลังเลิกอ่าน state — กัน eslint warn + กันเข้าใจผิด). **คง exported state helpers `getSaleEarnedPoints`/`hasReversedLoyaltyForSale`** (loyalty_reverse_sale.test ใช้ตรง). **signature/2 caller ไม่แตะ.** **ไม่แตะ:** earnPoints · redeem RPC (540) · checkout/credit 2180/redeem_customer_credit · stock/refund-accounting/SlipOK/OTP · schema/RLS (S6 = client read-fix ล้วน ไม่มี SQL). **ไฟล์:** `modules/loyalty.js` · `tests/loyalty_reverse_sale.test.js` (เพิ่ม `installMockDb` mock globalThis.fetch ให้ reverse tests เดิม [DB-sourced] + 4 S6 behavioral [state ว่าง+DB earn→คืน · DB reverse→ไม่ซ้ำ · cap จาก DB · fetch-fail→skipped:false ไม่ insert] + source guard [fetch ก่อน no-earn branch + ไม่อ่าน state]) · build 540→**541** (data-app-build + ?v=×4 + sw cache-v541 + comment + `dashboard_readonly_guard`). **Verification:** lint:errors 0 · unit **2242/2242** (loyalty_redeem_atomic + pos_loyalty_auto_earn เดิมไม่ regress) · e2e **14/14** (build 541 sync) · EOL LF. **owner smoke:** void/refund บิลที่มี loyalty earn (โดยเฉพาะบิลเก่านอก cache ≤500) → row ใหม่ `type='redeem'/ref_type='sale_reverse'/ref_id=saleId` points ≤ remaining · void/refund ซ้ำ → ไม่มี row ซ้ำ (idempotent). **residual:** S12 bundle gross_profit · S14 SlipOK key localStorage · S15 OTP KV fail-open. **Status: 🔜 branch `claude/phase-541-loyalty-clawback-db-source` — รอ owner review + merge (ไม่ต้องรัน SQL).**

**build 540 (Phase 540 / S5 — loyalty redeem atomic RPC + staff-only · BUG_AUDIT 2026-06-25 · §4.1 · ✅ MERGED+LIVE `1791d9a` PR #115 · SQL applied prod 2026-06-28):** **Baseline:** main `abb2b0c` (build 539 — S7/S8 PR #114). **บั๊ก (verified from source `loyalty.js:295-337`):** `redeemPoints` คำนวณ `remaining` จาก `getCustomerPoints` (อ่าน `state.loyaltyPoints` = in-memory cap ≤500, racy) แล้ว `_appXhrPost('loyalty_points', {type:'redeem'})` = read-then-insert จาก client → 2 เครื่อง/แท็บแลก customer เดียวกันพร้อมกัน ผ่าน in-memory check ทั้งคู่ → balance ติดลบ (over-redeem). **★ audit-before-implement เจอ security gap ใน prompt (owner เห็นด้วย+เพิ่ม requirement):** prompt เดิมให้ RPC `SECURITY DEFINER` + `GRANT EXECUTE TO authenticated` แต่ **ไม่มี role check** → SECURITY DEFINER bypass RLS + customer (OTP) อยู่ใน role `authenticated` → ลูกค้า login เรียก RPC redeem แต้มใครก็ได้ = ข้าม phase505 `deny_customer_loyalty_points` (RESTRICTIVE, loyalty_points อยู่ GROUP A) = privilege escalation. (มีหลักฐานว่า `redeem_customer_credit` ต้นแบบ guard ที่ DB อยู่แล้ว — pos.js:1188 comment.) **แก้ (3 ไฟล์ + SQL):** (1) **`supabase-phase540-loyalty-redeem-atomic.sql`** (owner รัน): fn `redeem_loyalty_points_atomic(p_customer_id bigint, p_points numeric, p_note text)` `SECURITY DEFINER SET search_path=public RETURNS loyalty_points` — **role guard แรกสุด** `IF NOT COALESCE(public.is_staff(),false) THEN RAISE…42501` + `IF COALESCE(public.is_customer_role(),false) THEN RAISE…42501` (auth.uid() ใน DEFINER = caller จริง; ★ STEP0 พบ policy เดิม `loyalty_points_rw WITH CHECK is_staff()` → guard ต้อง require is_staff ให้ match ไม่ loosen — owner ปรับ + ผม sync source ตรง prod) → validate (`23514` ถ้า customer_id null/points≤0) → `pg_advisory_xact_lock(hashtextextended('loyalty:'||id,0))` ต่อ customer → balance = SUM(earn)−SUM(redeem) จาก DB → `remaining < p_points` → `23514` → `INSERT type='redeem', ref_type='redemption', ref_id=NULL` `RETURNING *`; `GRANT EXECUTE TO authenticated` (customer ถูกกันใน fn ไม่ใช่ที่ grant) + `NOTIFY pgrst`. STEP0 ใน SQL = inspect `pg_policy` ของ loyalty_points ก่อน (RLS on? INSERT/SELECT role? phase505 deny present?) → guard ต้อง ≥ ของเดิม. (2) **`modules/loyalty.js redeemPoints`**: เลิก direct insert → raw `fetch(cfg.url+"/rest/v1/rpc/redeem_loyalty_points_atomic")` (mirror `_redeemCheckoutCredit` pos.js:1169) body `{p_customer_id:Number(customerId), p_points:Number(points), p_note}`; `!r.ok` → parse `await r.text()` map `23514`→"แต้มไม่พอแลก" / อื่น→HTTP; success → toast+loadAllData+`{ok:true}`. **precheck `getCustomerPoints` คงไว้แต่เป็น UX เท่านั้น** (RPC = source of truth). return `{ok,error}` คงเดิม → caller `:766 if(r?.ok)` clear form ยังทำงาน. **ref_type='redemption' ตรงกับเดิม** (`:322`) ไม่ break ประวัติ; phase497 unique index `WHERE ref_type='sale_reverse'` ไม่ชน. **ไม่แตะ:** `earnPoints` (ยัง direct insert loyalty_points), `reverseEarnedPointsForSale` (clawback), checkout/credit 2180/redeem_customer_credit, stock/refund/SlipOK/OTP, RLS อื่น. **redeemPoints ถูกเรียกที่เดียว (`loyalty.js:765` manual page) ไม่เข้า checkout** (verified). **ไฟล์:** `supabase-phase540-loyalty-redeem-atomic.sql` (ใหม่, owner-run) · `modules/loyalty.js` · `tests/loyalty_redeem_atomic_guard.test.js` (ใหม่ 9 — SQL: SECURITY DEFINER/role-guard-42501-before-balance/advisory-lock/balance-23514/insert-grant · JS: rpc call+Number cast+23514 map/no-direct-insert-in-redeem/earn-untouched/form-clear-on-ok) · build 539→**540** (data-app-build + ?v=×4 + sw cache-v540 + comment + `dashboard_readonly_guard`). **Verification:** lint:errors 0 · unit **2237/2237** (loyalty_reverse_sale + pos_loyalty_auto_earn + loyalty_settings เดิมไม่ regress) · e2e **14/14** (build 540 sync) · EOL LF. **🔴 owner SQL ก่อน merge:** STEP0 inspect → apply `supabase-phase540` → `SELECT prosecdef … = true` → smoke: staff redeem ≤ balance ✓ / over-balance → 23514 / customer JWT → 42501 / `NOTIFY pgrst`. **manual smoke:** หน้า loyalty แลกแต้มลูกค้ามีแต้มพอ → ผ่าน+แต้มลดถูกหลัง reload · แลกเกิน → ไม่ผ่าน+ฟอร์มไม่ถูก clear · ไม่มี console error. **known residual (audit คงเหลือ):** S6 clawback cap · S12 bundle gross profit · S14 SlipOK key localStorage · S15 OTP KV fail-open. **Status: ✅ SQL applied prod 2026-06-28 (owner) — STEP0 policy=`loyalty_points_rw WITH CHECK is_staff()` + deny_customer present · `prosecdef=true` · smoke staff redeem✓/over→23514/customer→42501 (rows left=0) · DB_MIGRATIONS_APPLIED.md logged. SQL file ใน branch sync ตรง prod (เพิ่ม `is_staff()` guard) + guard test assert is_staff. 🔜 branch `claude/phase-540-loyalty-redeem-atomic` — รอ review/merge.**

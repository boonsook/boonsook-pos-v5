...
**อัปเดตล่าสุด:** 5 มิถุนายน 2026 (Phase 371 revert-out-of-scope-stock-unresolved-flag — build 371) · ⏸️ **STOP — รอ owner/Codex review**
**Version:** 5.66.0 (build 371) — Phase 371 revert-out-of-scope-stock-unresolved-flag (scope correction: Phase 370 ใส่ "Part 2" reconcile-flag [post-save PATCH `service_jobs` note += `⚠️[STOCK_UNRESOLVED]`] เข้า ac_install/service_form/solar + test ทั้งที่ owner สั่งทำ **Part 1 อย่างเดียว** → Phase 371 ถอด Part 2 ออกหมด: ลบ block ใน `if (stockOpsFailed)` ทั้ง 3 ไฟล์ → เหลือแค่ `showToast?.(...)` **byte-identical กับ build 369** [ยืนยัน `git diff 67834c9 HEAD -- modules/*` เหลือเฉพาะ Part 1]. test Part 2 พลิกเป็น **absence-assert** กัน flag กลับมา. **คง Part 1** [`isHome`→`throw` block ก่อน POST] ทั้ง 3 ไฟล์. forward commit — ❌ ไม่ใช้ `git revert e852c78` [จะลบ Part 1 ด้วย]. ❌ ไม่แตะ non-home throw/confirm dialog/deduct logic[369/368]/POST payload/JV/schema. test service_job_block_save_guard 12, unit 1090, e2e 11. bump 370→371)

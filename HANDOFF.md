...
**อัปเดตล่าสุด:** 4 มิถุนายน 2026 (Phase 367 fix-stock-oversell-negative-guard — floor CAS, build 367) · ⏸️ **STOP — รอ owner/Codex review ก่อนเริ่ม Phase B (pre-checkout/sync)**
**Version:** 5.66.0 (build 367) — Phase 367 fix-stock-oversell-negative-guard (severity สูง oversell/§4.1-4.2: `atomicDecrementStock` เขียน after=before-qty โดยไม่เช็ค before≥qty → ขายเกินสต็อก=เขียนติดลบ [พบจริง warehouse_stock product 1809 = -1]. ใส่ **floor ที่ CAS** เป็น last line of defense: ก่อนคำนวณ after ทุก attempt ถ้า before<qty → return {ok:false, insufficient:true, before, error:"insufficient ..."} ไม่ PATCH/ไม่เขียนติดลบ; CAS refetch รอบใหม่เจอ stock ต่ำกว่า qty ก็ fail insufficient แทน retry. caller `_deductStockForSaleItem` [main.js]: dec.insufficient → toast "⚠️ สต็อกไม่พอ: {ชื่อ} (เหลือ X)"; logic skipProductsCas เดิมครอบ products.stock. ❌ ไม่แตะ CAS retry/race เดิม/atomicAddToField/pre-checkout/cart/products↔warehouse sync [Phase B]/schema/RLS/SQL. +4 tests [stock_cas 20], unit 1061, e2e 11)

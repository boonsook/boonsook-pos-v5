...
**อัปเดตล่าสุด:** 3 มิถุนายน 2026 (Phase 359 team-center-owner-action-surface — เพิ่ม read-only action surface, build 359) · ⏸️ **STOP — รอ owner/Codex review ก่อนเริ่ม Phase 360**
**Version:** 5.66.0 (build 359) — Phase 359 team-center-owner-action-surface (เพิ่ม filter chips [ทั้งหมด/รออนุมัติ/งานต้องดู/งานแอร์/ลูกค้า/สินค้า] กรองใน memory จาก ctx.state; drill-down modal read-only แสดง เลขเอกสาร/ลูกค้า/สถานะ/วันที่/ยอด/source เท่าที่มีจริง ปุ่มเฉพาะ ไปหน้าต้นทาง/คัดลอก prompt/ปิด ไม่มี save·approve·submit; owner prompt generator buildPrompt → clipboard text เท่านั้น; subtitle "มุมมองอ่านอย่างเดียวสำหรับเจ้าของร้าน"; chat → "บันทึก Draft/Prompt"; modal z-index 9995 เหนือ nav/FAB; reuse money/formatDate/parseAirJobMeta. ❌ fetch/POST/PATCH/PUT/DELETE · ❌ mutate state · ❌ POS/stock/accounting/payroll/service · ไม่เปิด sales/customer. guard 9→12, unit 1033)
**Previous:** 5.66.0 (build 358) — Phase 358 team-center-ui-polish-readonly (game/avatar board → owner dashboard ด้วย work cards จาก ctx.state; ลบ avatar/room/map; chat → "บันทึกย่อ/Draft"; layout overflow-safe; guard 9)

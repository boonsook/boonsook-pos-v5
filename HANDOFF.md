...
**อัปเดตล่าสุด:** 3 มิถุนายน 2026 (Phase 360 team-center-list-search-polish — เพิ่ม search/sort/list polish, build 360) · ⏸️ **STOP — รอ owner/Codex review ก่อนเริ่ม Phase 361**
**Version:** 5.66.0 (build 360) — Phase 360 team-center-list-search-polish (search box ใต้ filter — ค้นจาก ctx.state ใน memory [เลขเอกสาร/ลูกค้า/สถานะ/source/รุ่น-BTU] ไม่ fetch/ไม่ save query, ไม่พบ→"ไม่พบรายการตามคำค้นนี้"; sort ล่าสุด/เก่าสุด/ยอดมาก/สถานะ — clone [...items] ก่อน sort ไม่ mutate state; reorder overview cards [งานที่ต้องดู→รออนุมัติ→งานแอร์→เอกสารล่าสุด→ลูกค้า→สินค้า]; rows มี status/date/amount/source chips; prompt มีบริบทขึ้น [ประเภท]+สิ่งที่ควรตรวจ clipboard เท่านั้น; re-render เฉพาะ #teamListBody คง focus. ❌ fetch/POST/PATCH/PUT/DELETE · ❌ mutate state · ❌ POS/stock/accounting/payroll/service · ไม่เปิด sales/customer. guard 12→14, unit 1035)
**Previous:** 5.66.0 (build 359) — Phase 359 team-center-owner-action-surface (filter chips + drill-down modal read-only + prompt generator clipboard; ปุ่มเฉพาะ ไปหน้าต้นทาง/คัดลอก prompt/ปิด; guard 12)

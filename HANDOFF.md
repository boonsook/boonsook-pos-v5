...
**อัปเดตล่าสุด:** 4 มิถุนายน 2026 (Phase 365 team-center-documents-list-category-readonly — หมวด "เอกสาร" ในมุมมอง list read-only, build 365) · ⏸️ **STOP — รอ owner/Codex review ก่อนเริ่ม Phase 366**
**Version:** 5.66.0 (build 365) — Phase 365 team-center-documents-list-category-readonly (filter category "เอกสาร" รวม quotations+receipts+deliveryInvoices ในมุมมอง list ผ่าน mergeRecentDocs wrapper {it,type}; renderDocsListBody แยก pipeline พก per-item type: search itemSearchText(w.it)+date dateInRange(w.it)+sort sortDocs[clone]+rows rowHtml(w.it,w.type)+export buildDocsSummary→clipboard; pure helper เติม branch receipt/delivery [itemSearchText/amountOf/mdItemLine]; stats หมวดเอกสาร=นับตามชนิด+สถานะ+ป้าย ≤50 ไม่มียอดรวมข้ามชนิด; overview docs group + ปุ่มดูทั้งหมด→documents. single-type categories เดิมไม่กระทบ. ❌ fetch/POST/PATCH/PUT/DELETE · ❌ mutate state · ❌ POS/stock/accounting/payroll/service · admin-only. guard 24→28, unit 1049, e2e 11)

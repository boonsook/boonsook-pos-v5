...
**อัปเดตล่าสุด:** 3 มิถุนายน 2026 (Phase 364 team-center-recent-documents-merge-readonly — รวมเอกสารล่าสุดหลายชนิด read-only, build 364) · ⏸️ **STOP — รอ owner/Codex review ก่อนเริ่ม Phase 365**
**Version:** 5.66.0 (build 364) — Phase 364 team-center-recent-documents-merge-readonly (recent "เอกสารล่าสุด" รวม quotations+receipts+deliveryInvoices: mergeRecentDocs tag doc-type ต่อ item + spread + sort created_at desc บน array ใหม่ ไม่ mutate state, top 3; แถวมี doc-type badge + เลขที่ + ลูกค้า + ยอด + วันที่ + status chip; rowHtml/findItem/detailHtml รองรับ receipt→receipts, delivery→delivery_invoices [drill-down read-only navigate]; field ไม่มี→"—". ไม่ทำ stretch list-category. ❌ fetch/POST/PATCH/PUT/DELETE · ❌ mutate state · ❌ save/approve/submit/delete · ❌ POS/stock/accounting/payroll/service · admin-only. guard 21→24, unit 1045, e2e 11)
**Previous:** 5.66.0 (build 363) — Phase 363 team-center-aggregate-summary-readonly (summarizeStats + stats bar + markdown stats + overview breakdown; ป้าย ≤50; guard 21)

...
**อัปเดตล่าสุด:** 8 มิถุนายน 2026 (Phase 405 receipt-acct-badge-refresh — build 405, **cosmetic · display-only**) · ป้าย "เอกสารบัญชี" บนใบเสร็จ POS ที่เด้งตอนจบบิลค้าง "ยังไม่ลงบัญชี" **ทุกบิล** (deterministic) ทั้งที่ลงบัญชีจริง — **ordering bug ไม่ใช่ data bug**: `pos.js doCheckout` เปิดใบเสร็จ + ยิง lookup JV (sale_trace 92.17) **ก่อน** `postJournalForSale` สร้าง JV (`:1250` openReceiptDrawer ก่อน `:1257` auto_post) + badge เติมครั้งเดียวไม่ re-poll · แก้: `main.js _refreshReceiptAcctTrace(saleId)` re-run `_fillReceiptAcctTrace` (guard `String(lr.id)===String(saleId)` + drawer เปิด) exposed `window._appRefreshReceiptAcctTrace`; `pos.js` post `.then()` เรียกหลัง JV ลงสำเร็จ (ห่อ try กัน error เข้า checkout) · post fail→ไม่ refresh (ยังเหลือง=honest); เปิดบิลอื่น/drawer ปิด→no-op · ❌ ไม่แตะ posting/auto_post/money/stock/JV/ลำดับ await การขาย — display refresh ล้วน · lint 0 / unit 1385 / +guard receipt_acct_trace_refresh · bump 404→405 · ⏸️ **STOP — pushed branch `claude/phase-405-receipt-acct-badge-refresh`, รอ owner smoke (ขายบิล → ใบเสร็จเด้งเขียวเอง ~1-2 วิ; post fail→ยังเหลือง) ก่อน merge; deploy ปกติ**
**Version:** 5.66.0 (build 405) — Phase 405 receipt-acct-badge-refresh (**cosmetic · display-only**: ป้ายเอกสารบัญชีบนใบเสร็จ POS ค้าง "ยังไม่ลงบัญชี" ทุกบิลทั้งที่ลงบัญชีจริง — **deterministic ordering bug** ไม่ใช่ data bug. badge=live lookup [Phase 92.17 sale_trace `findJournalForSale` key source_table='sales'+source_id=sale.id] ยิงตอนเปิดใบเสร็จ. **checkout flow `pos.js doCheckout`:** `openReceiptDrawer()` [`:1250`, ยิง lookup] → `await loadAllData()` → `postJournalForSale()` [`:1257`, สร้าง JV fire-and-forget] → lookup รัน**ก่อน** JV เกิดเสมอ → "missing"="ยังไม่ลงบัญชี" + ไม่ re-poll → ค้างจนปิด-เปิดใหม่. **fix:** [1] `main.js _refreshReceiptAcctTrace(saleId)`: no-op ถ้าไม่มี lastReceipt / `String(lr.id)!==String(saleId)` [กันรีเฟรชผิดบิล+type mismatch bigint/string] / `#receiptAcctTrace` ไม่มี [drawer ปิด] → ไม่งั้น re-run `_fillReceiptAcctTrace(lr)` [read-only lookup เดิม]; expose `window._appRefreshReceiptAcctTrace`. [2] `pos.js` post `.then(()=>{try{window._appRefreshReceiptAcctTrace?.(saleId)}catch{console.warn}})` **ก่อน** `.catch` เดิม [post fail→.catch→ไม่ refresh=ยังเหลือง honest; refresh error→try กัน ไม่กระทบ checkout]. ❌ ไม่แตะ posting/auto_post/JV/money math/stock/loyalty/ลำดับ await การขาย/sale_trace internals — เพิ่ม display-refresh hook ล้วน. +guard receipt_acct_trace_refresh [source-wiring: id-guard String ทั้งสองข้าง/re-run fill/no write-post-stock · .then refresh+try+.catch คงอยู่+ลำดับ then<refresh<catch]; dashboard_readonly bump 405; unit 1385, lint 0, bump 404→405. **known:** ถ้า postJournalForSale resolve ก่อน JV queryable [RLS/replication lag — ไม่น่าเกิด same-DB] → ยังเหลือง, เปิดใหม่หาย. **🔴 owner smoke:** [1] ขายบิล → ใบเสร็จเด้ง → ป้ายเปลี่ยนเป็น "📒 SV… ลงบัญชีแล้ว" เองใน ~1-2 วิ [ไม่ต้องปิด-เปิด]; [2] เปิดบิลเดิมจากรายการขาย → เขียว; [3] [ถ้าจำลองได้] auto_post fail → ยังเหลือง ไม่ false-green. **STOP — รอ owner smoke ก่อน merge/push main**)

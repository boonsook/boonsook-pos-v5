...
**อัปเดตล่าสุด:** 3 มิถุนายน 2026 (Phase 358 team-center-ui-polish-readonly — ปรับ "ศูนย์ทีม AI" เป็น owner dashboard, build 358) · ⏸️ **STOP — รอ owner/Codex review ก่อนเริ่ม Phase 359**
**Version:** 5.66.0 (build 358) — Phase 358 team-center-ui-polish-readonly (ปรับ `team_center` จาก game/avatar board → owner dashboard ด้วย work cards จาก ctx.state จริง: รออนุมัติใบเสนอราคา/งานที่ต้องดู/งานบริการ/งานแอร์จากแคตตาล็อก[reuse parseAirJobMeta]/ลูกค้า/สินค้า/เอกสารล่าสุด → กดการ์ด showRoute; field ไม่มีจริง→"ยังไม่มีข้อมูล" ไม่ hardcode 0; ลบ avatar/room/map/dot; integration = placeholder "ยังไม่เชื่อมต่อ · รอ owner อนุมัติ"; ผู้ช่วย AI label "ตัวอย่าง"; chat → "บันทึกย่อ/Draft ในหน้านี้เท่านั้น"; layout overflow-safe. ❌ fetch/POST/PATCH/PUT/DELETE · ❌ mutate state · ❌ POS/stock/accounting/service · ไม่เปิด sales/customer. guard 8→9, unit 1030)
**Previous:** 5.66.0 (build 357) — Phase 357 team-center-readonly (หน้าใหม่ `team_center` admin-only + read-only dashboard; integration placeholder; agent concept; chat local draft; +8 guards)

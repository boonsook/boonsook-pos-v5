...
**อัปเดตล่าสุด:** 8 มิถุนายน 2026 (Phase 402 service-job-equipment-from-stock — build 402, **MONEY/STOCK §4.1+§4.2 · deduct-only scope (owner-approved)**) · drawer "เพิ่มงานช่าง" (openServiceJobDrawer) เพิ่ม section 🔧 อุปกรณ์จากสต็อก — picker เลือกสินค้า+คลังที่มีสต็อก+qty → **ตัดสต็อกตอนสร้างงานใหม่** ผ่าน `window._appApplyStockMovement` (CAS/floor) + precheck `aggregateNeedByKey` (ไม่พอ→บล็อก) · โมดูลใหม่ `modules/service_equipment.js` (import โดย main.js เท่านั้น — ไม่แตะ service_form.js) · งานเดิม (มี items_json)→อุปกรณ์ read-only กันตัดซ้ำ · total_cost รวมอุปกรณ์ (JV ใช้) · lint 0 / unit 1369 / e2e 12 / stock_cas+auto_post 46 ไม่แดง · ⏸️ **STOP — pushed branch `claude/phase-402-service-job-equipment-from-stock`, รอ Codex/owner review (ยังไม่ push main); 🔴 owner money/stock smoke บังคับ ก่อน merge**

...
**อัปเดตล่าสุด:** 8 มิถุนายน 2026 (Phase 400 mobile-fix-vertical-text — build 400, **layout-only · ไม่แตะ logic**) · แก้บั๊กมือถือ "ข้อความไทยแตกแนวตั้ง" 2 จุด (audit ทั้ง codebase เจอแค่ 2): (1) 🔴 accounting/opening_balance.js — ชื่อบัญชีแตก 1 ตัว/บรรทัด → 3 row block เป็น class `.ob-*` (flex-wrap + .ob-name min-width + @media≤600px input เต็มกว้าง); (2) 🟡 refunds.js panel เหตุผลคืนสินค้า — +flex-wrap+min-width ให้ text ไม่คับ · คง input id `ob_${code}`+escHtml, ไม่แตะ balance calc/JV/refund logic · lint 0 / unit 1356 / e2e 12 · ⏸️ **STOP — pushed branch `claude/phase-400-mobile-fix-vertical-text`, รอ Codex/owner review (ยังไม่ push main)**
**Version:** 5.66.0 (build 400) — Phase 400 mobile-fix-vertical-text (**layout-only · ไม่แตะ logic**: บั๊กมือถือ ~360px "ข้อความไทยแตกแนวตั้ง" จาก flex row ไม่ wrap + control fix-width บีบ text. audit ทั้ง codebase 2026-06-08 เจอ 2 จุด [ที่เหลือกันด้วย flex-wrap/min-width/auto-fit อยู่แล้ว — ไม่แตะ]. **(1) accounting/opening_balance.js [หลัก]:** 3 row block [asset/liability/equity เหมือนกัน] จาก inline-style flex [width:160px input บีบชื่อจน ~0] → class `.ob-row`/`.ob-emoji`/`.ob-code`/`.ob-name`/`.ob-input`; **คง `id="ob_${f.code}"` [save อ้าง] + type/step/min + `escHtml(f.label)` เป๊ะ**. **style.css [global, ปลายไฟล์]:** `.ob-row` flex-wrap · `.ob-name` flex:1 1 auto + **min-width:110px** [กันยุบ] · `.ob-input` 160px · `@media(max-width:600px)` `.ob-input` width:100%+flex:1 1 100% [เต็มกว้างจอแคบ]. **(2) refunds.js [hardening]:** row panel เหตุผลคืนสินค้า [204-205]: +`flex-wrap:wrap` + reason div `flex:1 1 120px;min-width:120px` [bar 200px wrap ลงล่างบนจอแคบ, เหตุผลกว้างเต็ม; desktop เหมือนเดิม] · คง `escHtml(r)`. ❌ layout-only: ไม่แตะ live balance calc[107+]/ปุ่มบันทึก/JV idempotency/debit=credit [OB] · refund action/total/pct/bar logic [refunds] · *_FIELDS/topReasons values · input id · หน้าอื่นที่ audit ว่า clean · ไม่ inline media query [OB ใช้ class] · ไม่แตะ JS logic. +4 tests mobile_row_layout [class ใช้แล้ว+ไม่เหลือ inline width:160px · id คงอยู่+escHtml · css min-width+@media · refunds flex-wrap+min-width]; dashboard guards เดิมเขียว; unit 1356, e2e 12, lint 0, bump 399→400. **owner smoke มือถือ ~360px:** #accounting_opening_balance → ชื่อบัญชี**ไม่แตกแนวตั้ง** + input กรอกง่าย + กรอก 1 ช่อง live balance ยังถูก · #refunds → panel เหตุผลอ่านง่ายไม่คับ · desktop ทั้ง 2 หน้าเหมือนเดิม. **STOP — รอ review ก่อน merge/push main**)

// Phase 502b — solar equipment picker: cashier cart via shared helper
// (modules/picker_cart.js). Run: node --test tests/solar_picker_cart_guard.test.js
//
// Separate from solar_picker_guard.test.js (453c warehouse-first) — this locks
// ONLY the 502b cart layer. Source-regex (modal binds DOM).
...
test("solar imports + uses the shared picker-cart helper", () => {
...
test("502b touch double-add guard: shouldSkip ก่อน add", () => {
  ...
  const pushIdx = itemClick.indexOf("_solItems.push(");
  assert.ok(guardIdx >= 0 && pushIdx >= 0 && guardIdx < pushIdx, "touch-guard ต้องมาก่อน add");
  assert.match(picker, /class="solpk-item"[\s\S]*?touch-action:manipulation/, "ปุ่มสินค้ามี touch-action");
});

test("502b interactive cart (_solItems) + badge", () => {
  assert.match(picker, /id="solpkCart"/, "มี container ตะกร้า");
  assert.match(picker, /id="solpkDone"/, "มีปุ่มเสร็จ");
  assert.match(picker, /data-badge-pid=/, "สินค้ามี slot badge ×N");
  assert.match(picker, /items:\s*_solItems/, "ตะกร้าอ่านจาก _solItems");
  assert.match(picker, /onChangeQty/, "ต่อ qty change");
  assert.match(picker, /onRemove/, "ต่อ remove");
});

test("502b ไม่ทำ 453c/stock พัง: dedup คงเดิม + ไม่ตัดสต็อก", () => {
  assert.match(itemClick, /_solItems\.push\(\{/, "push เข้า _solItems คงเดิม");

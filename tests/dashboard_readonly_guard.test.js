...
test("build markers are all bumped to 522 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="522"/, "data-app-build must be 522");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=522`), `${asset}?v= must be bumped to 522`);
    assert.ok(!indexHtml.includes(`${asset}?v=521`), `${asset}?v= must not still be 521`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v522'/, "sw.js CACHE_NAME must be v522");
  assert.ok(!/boonsook-pos-v5-cache-v521'/.test(sw), "sw.js CACHE_NAME must not still be v521");
});

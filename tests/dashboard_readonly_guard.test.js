...
test("build markers are all bumped to 460 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="460"/, "data-app-build must be 460");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=460`), `${asset}?v= must be bumped to 460`);
    assert.ok(!indexHtml.includes(`${asset}?v=459`), `${asset}?v= must not still be 459`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v460'/, "sw.js CACHE_NAME must be v460");
  assert.ok(!/boonsook-pos-v5-cache-v459'/.test(sw), "sw.js CACHE_NAME must not still be v459");
});

...
test("build markers are all bumped to 576 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="576"/, "data-app-build must be 576");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=576`), `${asset}?v= must be bumped to 576`);
    assert.ok(!indexHtml.includes(`${asset}?v=575`), `${asset}?v= must not still be 575`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v576'/, "sw.js CACHE_NAME must be v576");
  assert.ok(!/boonsook-pos-v5-cache-v575'/.test(sw), "sw.js CACHE_NAME must not still be v574");
});

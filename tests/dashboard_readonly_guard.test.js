...
test("build markers are all bumped to 516 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="516"/, "data-app-build must be 516");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=516`), `${asset}?v= must be bumped to 516`);
    assert.ok(!indexHtml.includes(`${asset}?v=515`), `${asset}?v= must not still be 515`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v516'/, "sw.js CACHE_NAME must be v516");
  assert.ok(!/boonsook-pos-v5-cache-v515'/.test(sw), "sw.js CACHE_NAME must not still be v515");
});

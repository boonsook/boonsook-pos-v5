...
test("build markers are all bumped to 483 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="483"/, "data-app-build must be 483");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=483`), `${asset}?v= must be bumped to 483`);
    assert.ok(!indexHtml.includes(`${asset}?v=482`), `${asset}?v= must not still be 482`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v483'/, "sw.js CACHE_NAME must be v483");
  assert.ok(!/boonsook-pos-v5-cache-v482'/.test(sw), "sw.js CACHE_NAME must not still be v482");
});

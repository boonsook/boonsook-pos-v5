...
test("build markers are all bumped to 484 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="484"/, "data-app-build must be 484");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=484`), `${asset}?v= must be bumped to 484`);
    assert.ok(!indexHtml.includes(`${asset}?v=483`), `${asset}?v= must not still be 483`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v484'/, "sw.js CACHE_NAME must be v484");
  assert.ok(!/boonsook-pos-v5-cache-v483'/.test(sw), "sw.js CACHE_NAME must not still be v483");
});

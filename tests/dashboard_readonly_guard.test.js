...
test("build markers are all bumped to 461 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="461"/, "data-app-build must be 461");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=461`), `${asset}?v= must be bumped to 461`);
    assert.ok(!indexHtml.includes(`${asset}?v=460`), `${asset}?v= must not still be 460`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v461'/, "sw.js CACHE_NAME must be v461");
  assert.ok(!/boonsook-pos-v5-cache-v460'/.test(sw), "sw.js CACHE_NAME must not still be v460");
});

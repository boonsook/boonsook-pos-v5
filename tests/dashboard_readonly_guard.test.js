...
test("build markers are all bumped to 388 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="388"/, "data-app-build must be 386");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=388`), `${asset}?v= must be bumped to 388`);
    assert.ok(!indexHtml.includes(`${asset}?v=387`), `${asset}?v= must not still be 387`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v388/, "sw.js CACHE_NAME must be v388");
  assert.ok(!/boonsook-pos-v5-cache-v387'/.test(sw), "sw.js CACHE_NAME must not still be v387");
});

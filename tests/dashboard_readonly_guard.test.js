...
test("build markers are all bumped to 551 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="551"/, "data-app-build must be 551");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=551`), `${asset}?v= must be bumped to 551`);
    assert.ok(!indexHtml.includes(`${asset}?v=550`), `${asset}?v= must not still be 550`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v551'/, "sw.js CACHE_NAME must be v551");
  assert.ok(!/boonsook-pos-v5-cache-v550'/.test(sw), "sw.js CACHE_NAME must not still be v550");
});

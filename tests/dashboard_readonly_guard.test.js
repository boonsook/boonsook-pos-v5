...
test("build markers are all bumped to 528 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="528"/, "data-app-build must be 528");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=528`), `${asset}?v= must be bumped to 528`);
    assert.ok(!indexHtml.includes(`${asset}?v=527`), `${asset}?v= must not still be 527`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v528'/, "sw.js CACHE_NAME must be v528");
  assert.ok(!/boonsook-pos-v5-cache-v527'/.test(sw), "sw.js CACHE_NAME must not still be v527");
});

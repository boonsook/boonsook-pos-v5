...
test("build markers are all bumped to 400 (index.html + sw.js)", () => {
  assert.match(indexHtml, /data-app-build="400"/, "data-app-build must be 400");
  for (const asset of ["style.css", "selfheal.js", "main.js", "boot.js"]) {
    assert.ok(indexHtml.includes(`${asset}?v=400`), `${asset}?v= must be bumped to 400`);
    assert.ok(!indexHtml.includes(`${asset}?v=399`), `${asset}?v= must not still be 399`);
  }
  assert.match(sw, /boonsook-pos-v5-cache-v400/, "sw.js CACHE_NAME must be v400");
  assert.ok(!/boonsook-pos-v5-cache-v399'/.test(sw), "sw.js CACHE_NAME must not still be v399");
});

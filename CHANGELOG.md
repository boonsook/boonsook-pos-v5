...
- feat(tech,build 550): **หน้าหลักช่าง — "🚚 อุปกรณ์ต้องเบิกขึ้นรถวันนี้" (morning-load)** — เพิ่ม panel หลัก: รวม `items_json` ของ**งานวันนี้** (`_jobDay=today`, active) ต่อรถ `is_mobile` (คันขาว/คันแดง) → **งานยังไม่ปิด = ต้องเบิก** (`need`, ยังไม่ตัดสต็อก) · **ปิดวันนี้ = ใช้แล้ว** (`used`) → เทียบ `warehouse_stock` คันนั้น (`remain`) → **สถานะสี** (🔴 ของหมด `remain≤0` / 🔴 ไม่พอ `remain<need` / 🟡 เหลือน้อย `≤need+2` / 🟢 เบิกได้) · group ต่อคัน + ปุ่ม "เบิกคันนี้ →" (wh_kunkhao/wh_kundaeng) · KPI "ต้องเบิกขึ้นรถ"=needCount · คง panel "ของที่ช่างใช้ล่าสุด 2 วัน" เป็นตัวรอง. **READ-ONLY** — `state.serviceJobs/warehouses/warehouseStock/products`; ไม่มี write/fetch/deduct (guard `tech_home_readonly` 4/4 + behavioral `tech_home_truck_load` 2 — need/used/status/skip cancelled-other-day-non-truck). ★ **ต้องช่างใส่อุปกรณ์ตอนรับงาน** (picker เดิม, งานเปิด=ไม่ตัดจนปิด) ระบบถึงเตือน — งานวันนี้ตอนนี้ยังไม่ใส่ → panel ว่างจนเริ่มใส่ (workflow ไม่ใช่บั๊ก). ไม่แตะ deduct-drawer/POS/SQL. unit 2294·e2e 14/14·lint0. build 549→550.

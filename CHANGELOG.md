...
- `606-b2c` feat(customer,security): **ลูกค้าเห็น "งานบริการของฉัน" ได้จริง + ยืนยันปิดงานแบบ flow-aware ผ่าน authenticated proxy** (build 605 / v5.69.73)
  ...
  - ใหม่: `/api/v1/customer-service-jobs` (GET/POST) — service-role proxy ที่ derive identity จาก JWT เท่านั้น · ownership exact (`customer_phone` หรือ `created_by`) + re-check ทุก row · response allowlist 9 field (ไม่มี note/identity) · POST = CAS ผูก id+status+flow+ownership, affected rows ต้อง = 1, 0 rows → 409
  ...
  - +guards customer_service_jobs_proxy (32) + customer_dashboard_service_flow (23) · mutation proof 22/22 · **ไม่มี SQL · ไม่ activate flow v2 · ไม่แตะ main.js/payment/JV/stock**

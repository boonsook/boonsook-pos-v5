...
- Phase 626 fix(documents): **ใบส่งสินค้า → ใบเสร็จ ต้อง fail closed ก่อนสร้างเอกสาร** (build 626 / v5.69.93 · data integrity)
  ...
  - +tests/di_to_receipt_failclosed.test.js (unit 41, รันโค้ดจริงใน `node:vm` + ordered request ledger) + tests/e2e/phase626_di_to_receipt_failclosed.spec.js (Chromium 30 — caller จริงทั้ง dropdown และปุ่มใน preview × 390×844/desktop). Baseline RED unit 30/41 · e2e 24/30; patched GREEN 41/41 · 30/30; mutation RED 11/11 (ERR_ASSERTION ล้วน). lint 0 errors · unit 3314/3314 · e2e 84/84 · diff --check/LF/no BOM/build markers PASS. ⚠️ gate นี้เป็น client-side ไม่ atomic (multi-device race ยังต้องพึ่ง DB constraint ที่ยังไม่มี) และไม่แก้ใบเก่าย้อนหลัง. STOP รอ independent review; ไม่ push/PR/merge/deploy

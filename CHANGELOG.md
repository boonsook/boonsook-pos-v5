...
- Phase 630 fix(documents): **ใบเสนอราคา → ใบส่งสินค้า fail closed ก่อนสร้างเอกสาร** (build 629 / v5.69.96 · local commit · รอ independent review)
  ...
  - 🔴 rev2 (independent review `STOP-PHASE-630-NOT-READY` · P1): rev1 เช็คแค่ array — แถว `{}` / `[]` / string ถูก map เป็นสินค้า "" qty 1 แล้วเขียนจริง. ตอนนี้ validate ทุกแถวก่อน map (plain object + ครบ 7 คอลัมน์ที่ mapper อ่าน · แถวค่า null ยังผ่าน) ไม่งั้น ITEM_FAIL (write 0) · +unit S11/S12 + MUT-21/22 + e2e 6 · RED vs rev1 · mutation 22/22 · lint 0 · unit 3501/3501 · e2e 171/171 · build 629 คงเดิม · ก่อน merge: owner นับ `delivery_invoices.status` NULL/ว่าง (read-only)
- Phase 628B Production Closeout docs(documents): **production smoke ใบเสนอราคา → ใบส่งสินค้า → ใบเสร็จ ผ่าน + cleanup** (docs only · build 628 / v5.69.95 ไม่ bump · local commit · รอ independent review)
  - PR #222 merged (`ec95993`) + deployed build 628 · owner-authorized, **Codex-operated** authenticated UI smoke 2026-09-24: INSERT/readback ของ `item_type` ผ่าน production app path ครบ 3 ตาราง (หัวข้อ + สินค้าคงชนิด/ลำดับ · ใบเสร็จคงสถานะ pending และในการ smoke ไม่ได้กดรับชำระหรือสั่งงานบัญชี/สต็อก — ไม่มี DB snapshot จึงไม่ได้ตรวจยืนยันตาราง JV หรือ stock โดยตรง) · cleanup RC → INV → QT ผ่าน (UI-observed ไม่ใช่ DB count)
  - closeout commit นี้ไม่แตะ runtime/SQL/โค้ด payment/accounting/stock · residual "INSERT ยังไม่พิสูจน์" = SUPERSEDED · รายงาน [`PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md`](PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md) · STOP `READY-FOR-INDEPENDENT-PHASE-628B-CLOSEOUT-REVIEW`
- Phase 628B feat(documents): **แถวหัวข้อ (`item_type = heading`) ใช้งานจริงใน ใบเสนอราคา → ใบส่งสินค้า → ใบเสร็จ** (build 628 / v5.69.95 · local commit · รอ independent review) *(SUPERSEDED 2026-09-24 เฉพาะสถานะ "local commit · รอ independent review" — Phase 628B merged (PR #222 · `ec95993`) + deployed build 628 · production smoke ผ่าน · ส่วนอื่นของบรรทัดนี้ยังเป็นบันทึกเดิม · ดู [`PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md`](PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md))*
...
- Phase 628A-CLOSEOUT docs(db): **บันทึก production apply ของ Phase 628A** (docs only · build 626 / v5.69.93 ไม่ bump · ไม่มี SQL/runtime)
  ...
  - หลักฐาน owner-measured → independent post-run reviewer: `READY-FOR-OWNER-PHASE-628B-AUTHORIZATION` (ไม่ใช่การอนุมัติ). ข้อความเดิม "SQL ยังไม่ apply" / "PostgreSQL parser/live execution: NOT RUN" = SUPERSEDED. Phase 628B ผ่าน prerequisite gate แต่ runtime ยังไม่เริ่ม. STOP `READY-FOR-INDEPENDENT-PHASE-628A-CLOSEOUT-REVIEW` *(SUPERSEDED 2026-09-24 เฉพาะสถานะ "Phase 628B ผ่าน prerequisite gate แต่ runtime ยังไม่เริ่ม" — Phase 628B merged (PR #222 · `ec95993`) + deployed build 628 · production smoke ผ่าน · ส่วนอื่นของบรรทัดนี้ยังเป็นบันทึกเดิม · ดู [`PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md`](PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md))*
- Phase 628A docs(documents): **owner SQL runbook** [`PHASE-628A-OWNER-SQL-RUNBOOK.md`](PHASE-628A-OWNER-SQL-RUNBOOK.md) (ปิด Should-fix จาก independent review · docs only · migration/test ไม่เปลี่ยน · build 626 / v5.69.93 ไม่ bump)
  - STEP 0 fresh pre-run count ต่อ table → STEP 1 รัน migration ทั้งไฟล์ครั้งเดียว → STEP 2 / 3 รัน POST-CHECK A (บรรทัด 102–125) / B (127–148) **แยกกัน** เพราะ Supabase SQL Editor อาจแสดงแค่ result set สุดท้าย → STEP 4 stop decision (ผ่าน = `PHASE-628A-SCHEMA-APPLIED-AWAITING-INDEPENDENT-POST-RUN-REVIEW` เท่านั้น)
  - acceptance ของ B เทียบ fresh count ต่อ table — 136 / 111 / 95 (รวม 342) เป็น metadata snapshot ไม่ใช่ค่าตายตัว. SQL ยังไม่ apply · PostgreSQL parser/live execution: NOT RUN · Phase 628B ยังไม่เริ่ม. STOP รอ independent runbook review *(SUPERSEDED 2026-09-23 เฉพาะสถานะ "SQL ยังไม่ apply / execution NOT RUN" — owner apply บน production แล้ว · ส่วนอื่นของบรรทัดนี้ยังเป็นบันทึกเดิม · ดูบล็อก Phase 628A-CLOSEOUT ด้านบน)* *(SUPERSEDED 2026-09-24 เฉพาะสถานะ "Phase 628B ยังไม่เริ่ม" — Phase 628B merged (PR #222 · `ec95993`) + deployed build 628 · production smoke ผ่าน · ส่วนอื่นของบรรทัดนี้ยังเป็นบันทึกเดิม · ดู [`PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md`](PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md))*
- Phase 628A feat(documents): **schema foundation `item_type` (`item|heading`) ของรายการเอกสาร 3 ตาราง — SQL owner-run ยังไม่ apply** (build 626 / v5.69.93 ไม่ bump · schema + tests + docs เท่านั้น) *(SUPERSEDED 2026-09-23 เฉพาะสถานะ "SQL ยังไม่ apply / execution NOT RUN" — owner apply บน production แล้ว · ส่วนอื่นของบรรทัดนี้ยังเป็นบันทึกเดิม · ดูบล็อก Phase 628A-CLOSEOUT ด้านบน)*
  - +`supabase-phase628a-document-item-type-schema.sql` (byte-exact จาก prompt §6): `quotation_items` / `delivery_invoice_items` / `receipt_items` ได้ `item_type text NOT NULL DEFAULT 'item'` + CHECK `item|heading` ชื่อ `<table>_item_type_check`; ข้อมูลเดิมทุกแถว → `item` (342 = metadata snapshot วันที่ owner ตรวจ ไม่ใช่ acceptance count — post-run เทียบ fresh count ต่อ table ตาม [`PHASE-628A-OWNER-SQL-RUNBOOK.md`](PHASE-628A-OWNER-SQL-RUNBOOK.md)) · ไม่ infer heading จากราคา 0 / `product_id=NULL` · ไม่สร้าง heading row · fail closed เมื่อเจอ column/CHECK ที่ไม่รู้ที่มา · มี post-check A/B
  - +tests/phase628a_document_item_type_schema.test.js (41 · block-scoped + positive control 23). Baseline RED 39/41 `missing artifact` · candidate 41/41 · disk mutation 24/24 KILLED (ERR_ASSERTION ล้วน · restore SHA ตรง). lint 0 · unit 3357/3357 · e2e 96/96 · diff --check / LF / no BOM PASS
  - **PostgreSQL parser/live execution: NOT RUN** — owner รันหลัง independent review; ยังไม่ live. ไม่แตะ runtime/UI/เงิน/สต็อก/RLS/markers. **Phase 628B (runtime/totals/conversion/rendering) ยังไม่เริ่ม**. STOP รอ independent review *(SUPERSEDED 2026-09-23 เฉพาะสถานะ "SQL ยังไม่ apply / execution NOT RUN" — owner apply บน production แล้ว · ส่วนอื่นของบรรทัดนี้ยังเป็นบันทึกเดิม · ดูบล็อก Phase 628A-CLOSEOUT ด้านบน)* *(SUPERSEDED 2026-09-24 เฉพาะสถานะ "Phase 628B (runtime/totals/conversion/rendering) ยังไม่เริ่ม" — Phase 628B merged (PR #222 · `ec95993`) + deployed build 628 · production smoke ผ่าน · ส่วนอื่นของบรรทัดนี้ยังเป็นบันทึกเดิม · ดู [`PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md`](PHASE-628B-PRODUCTION-SMOKE-CLOSEOUT.md))*
- Phase 626 fix(documents): **ใบส่งสินค้า → ใบเสร็จ ต้อง fail closed ก่อนสร้างเอกสาร** (build 626 / v5.69.93 · data integrity · rev2 = +duplicate row-shape, build/version คงเดิมเพราะยังไม่ push)
  - เดิม `convertToReceipt` เชื่อผล lookup โดยไม่พิสูจน์: duplicate-check ไม่เช็ค `response.ok`, ใช้ `.json().catch(() => [])` และ catch ตกไป `confirm` ⇒ เน็ตหลุด/HTTP 500/JSON พัง = "ไม่มีใบซ้ำ" → **ออกใบเสร็จซ้ำทับใบ active เดิมได้**; รายการก็มาจาก `_lineItems` ซึ่งเป็น preview cache ของใบที่เปิดล่าสุด ⇒ ได้รายการของ**ใบอื่น** หรือได้ **ใบเปล่าแต่มียอดเงินเต็ม** เมื่อโหลดล้มเงียบ
  - แก้: duplicate lookup ต้อง HTTP ok + JSON ผ่าน + เป็น array ครบสามด่าน ไม่งั้นหยุด (ไม่มี fallback ไป confirm) · โหลด snapshot รายการของ `inv.id` ใหม่ทุกครั้งเก็บใน local (เลิกใช้ `_lineItems` เป็น source) · โหลดสำเร็จแต่ 0 แถว = บล็อก ไม่มี override · header `ok:true` แต่ไม่มี `id` = หยุด write ที่เหลือ ไม่ rollback/retry และแจ้งว่า "อาจสร้างแล้ว แต่ยืนยันรหัสไม่ได้" · toast สุดท้ายสะท้อน partial state จริง (success ห้ามทับคำเตือน)
  - ยอดเงิน/payload header/ลำดับรายการ/PATCH ต้นทาง/accountant gate/1:1 block/inflight guard **คงเดิมทุกจุด**. ไม่มี SQL · ไม่แตะ stock/POS/accounting/CSP/SW strategy · `package.json` ไม่เปลี่ยน
  - 🔴 **rev2 (ผล independent review)**: rev1 ตรวจแค่ว่า response เป็น array แล้ว classify ด้วย `filter(d => d && …)` ⇒ payload `[null]` ถูกทิ้งเงียบ = "ไม่มีใบซ้ำ" แล้ว**สร้างใบเสร็จต่อ** (fail open) ส่วน `[{}]`/`[{status:null}]` แจ้ง "มีใบเสร็จ undefined". แก้: validate **ทุกแถว** ก่อน filter — ต้องเป็น non-null plain object + มี `status` เป็น string ที่ไม่ว่าง ไม่งั้นเข้า uncertainty path (write = 0, confirm = 0); cancelled-only / active duplicate ที่ valid คง behavior เดิม
  - +tests/di_to_receipt_failclosed.test.js (unit 43, รันโค้ดจริงใน `node:vm` + ordered request ledger) + tests/e2e/phase626_di_to_receipt_failclosed.spec.js (Chromium 42 — caller จริงทั้ง dropdown และปุ่มใน preview × 390×844/desktop). Baseline RED unit 32/43 · e2e 36/42 (rev1 RED: N4b + e2e 12/42); patched GREEN 43/43 · 42/42; mutation RED 12/12 (ERR_ASSERTION ล้วน). lint 0 errors · unit 3316/3316 · e2e 96/96 · diff --check/LF/no BOM/build markers PASS. ⚠️ gate นี้เป็น client-side ไม่ atomic (multi-device race ยังต้องพึ่ง DB constraint ที่ยังไม่มี) และไม่แก้ใบเก่าย้อนหลัง. STOP รอ independent review; ไม่ push/PR/merge/deploy
...
- `622` chore(documents): **ถอดตัวเลขวินิจฉัยออก — ปิดเคสเอกสารล้นหน้า** (build 622 / v5.69.90)
  ...
  - **ไม่มี SQL · ไม่แตะยอดเงิน/สต็อก/`functions/*`**
- `621` fix(documents): **🎯 ต้นตอจริงของ "4 แผ่น" — `zoom` ย่อแต่ภาพ ไม่ย่อกล่องที่ใช้แบ่งหน้า** (build 621 / v5.69.89)
  - วัดจาก **Chrome ของเจ้าของเอง** (เจ้าของอนุญาตให้เข้าดูหน้าจริง): `.doc-page` ที่ `zoom: 0.955` → `getBoundingClientRect()` = **296.0mm** (ภาพที่เห็น) แต่ `offsetHeight` = **309.8mm** (layout) และหน้า 2 เริ่มที่ `offsetTop` 309.8mm
  - ⇒ เบราว์เซอร์แบ่งหน้าจาก **layout box** ไม่ใช่ภาพ → ล้น A4 ไป **12.8mm** ทุกฉบับ ส่วนที่ล้นคือ padding ล่างเปล่า ๆ จึงได้ **"แผ่นเปล่าสนิท"** ต่อท้ายทุกใบ = 4 แผ่น (ตรงกับที่เจ้าของอธิบายเป๊ะ)
  - 🔴 ทำไมจำลองไม่ออก 5 รอบ: **Chromium ของ Playwright ยอมใช้ค่า `zoom` ตอนพิมพ์** แต่ Chrome จริงไม่ใช้ → ทุก e2e เขียวหมดทั้งที่ของจริงพัง · ตัววินิจฉัยบนแถบชื่อหน้าต่าง (620) รายงาน `296.0mm` ถูกต้อง เพราะมันอ่านจาก `getBoundingClientRect()` เหมือนกัน
  - แก้: ห่อทุก `.doc-page` ด้วย `.doc-fit` ที่ **layout = 209×296mm เป๊ะ** (`overflow:hidden`) แล้วย่อข้างในด้วย `transform: scale()` + `transform-origin: top left` — `.doc-fit` เป็นตัวที่ถูกแบ่งหน้า · เลิกใช้ `zoom` ถาวร · ห่อซ้ำไม่ได้ (fit ทำงาน 3 จังหวะ: ตอนแรก/ฟอนต์มา/`beforeprint`)
  - +guard: e2e วัด **`offsetHeight` ของ `.doc-fit`** ตรง ๆ (≤296.5mm) ไม่ใช่แค่นับจำนวนแผ่น · unit 18 ตัวจำลอง "transform ย่อแต่ภาพ layout เท่าเดิม" — mutation กลับไปใช้ `zoom` → **unit แดง 3 · e2e ยังเขียว** (จดไว้ในไฟล์เทสต์ว่าบั๊กชนิดนี้ Playwright จับไม่ได้ ต้องพึ่ง unit)
  - 🔎 ยังคงตัวเลขวินิจฉัย `[fit …]` บนแถบชื่อหน้าต่างไว้อีก 1 build — จะถอดออกเมื่อเจ้าของยืนยันว่าปิดเคส
  - **ไม่มี SQL · ไม่แตะยอดเงิน/สต็อก/`functions/*`**
- `620` fix(documents): **วัดใหม่ตอน `beforeprint` — ปุ่มพิมพ์บนแถบเครื่องมือยังได้ 4 แผ่นบนเครื่องเจ้าของ** (build 620 / v5.69.88)
  - อาการ (เจ้าของแยกให้): พิมพ์จาก**กล่องแชร์** = 2 หน้าถูก · พิมพ์จาก**ปุ่มบนแถบเครื่องมือ** = 4 หน้า — คนละเครื่องยนต์ (กล่องแชร์พิมพ์จากไฟล์ PDF ที่ `share_doc.js` สร้างไว้แล้ว ส่วนแถบเครื่องมือพิมพ์ HTML สดผ่าน `doc-utils.printDoc`)
  - 🔬 ตัดออกไปแล้วด้วยการวัด: ไม่ใช่จังหวะฟอนต์ (ความสูงไม่ขยับหลัง `fonts.ready`) · ไม่ใช่ระยะขอบ (619 ผ่านทั้งสองโหมด) · จำลองในเครื่องยังได้ 2 หน้าทุกเคส ⇒ ต่างกันที่สภาพแวดล้อมจริง
  - แก้เชิงหลักการ: `fitPrintedPages` ถูกเรียกใหม่ที่ **`beforeprint`** (จังหวะที่ Chrome สลับไปเลย์เอาต์กระดาษก่อนแบ่งหน้า) และที่ **`fonts.loadingdone`** — เดิมวัดครั้งเดียวบน**เลย์เอาต์หน้าจอ** ของ popup กว้าง 900px ซึ่งบนจอ scale 125-150% ไม่ใช่เลย์เอาต์เดียวกับตอนพิมพ์
  - 🔎 **ชั่วคราวเฉพาะ build นี้**: แถบชื่อหน้าต่างพิมพ์ต่อท้ายด้วย `[fit <zoom> · <mm>]` เพื่ออ่านค่าจริงจากเครื่องเจ้าของ (ถ้าไม่มีวงเล็บนี้ = ตัวย่อไม่ทำงานเลย) — จะถอดออกเมื่อปิดเคส
  - **ไม่มี SQL · ไม่แตะยอดเงิน/สต็อก/`functions/*`**
- `619` fix(documents): **ตั้ง "ระยะขอบ: ไม่มี" แล้วยังได้ 4 แผ่น — กล่องเอกสารเท่ากระดาษเป๊ะ** (build 619 / v5.69.87)
  - อาการ: 618 ได้ 2 แผ่นเมื่อระยะขอบ = "ค่าเริ่มต้น" แต่ถ้าผู้ใช้เลือก **"ไม่มี"** กลับได้ 4 แผ่น (หน้าเปล่าต่อท้ายทุกฉบับ)
  - เหตุ: 618 ตั้งกล่องเป็น `210×297mm` = **ขนาดกระดาษเป๊ะ** · วัดหน้าผาด้วย Chromium: กล่อง **297.0mm = 2 แผ่น แต่ 297.2mm = 4 แผ่น** — ห่างกันแค่ 0.2mm (297mm = 1122.52px ไม่ลงตัว) Chrome คนละรุ่น/ไดรเวอร์เครื่องพิมพ์ปัดเศษต่างนิดเดียวก็ตกผา · ตอนเลือก "ค่าเริ่มต้น" Chrome ย่อ fit-to-printable ให้เองจึงกลบอาการไว้
  - แก้: `PRINT_SAFE_MM = 1` — กล่องเป็น **209×296mm** ห่างขอบกระดาษ 1mm (5 เท่าของระยะหน้าผา) ทั้งตอนวัดและตอนเรนเดอร์
  - +guard: e2e พิมพ์ **ทั้งสองโหมดระยะขอบ** (`210×297 margin 0` และ `@page`) ต้องได้ 2 แผ่นทั้งคู่ · assert กล่อง ≤ 296.5×209.5mm (ห้ามแตะขอบ) และ ≥ 290×205mm (ห้ามหดครึ่งแผ่น) · mutation `PRINT_SAFE_MM=0` → e2e แดง 6/7 + unit แดง 2
  - **ไม่มี SQL · ไม่แตะยอดเงิน/สต็อก/`functions/*`**
- `618` fix(documents): **ย่อแล้วเอกสารหดไปครึ่งแผ่น — ต่อจาก 617** (build 618 / v5.69.86)
  - อาการ: 617 ได้ 2 แผ่นถูกแล้ว แต่เอกสารหดไปกองมุมซ้ายบน เหลือประมาณครึ่งแผ่น
  - เหตุ 2 ข้อ: (1) `zoom` ย่อ**ทั้งกว้างและสูง** แต่ไม่ได้ขยาย `width`/`min-height` ชดเชย · (2) วัดความสูงด้วย `width:100%` = ความกว้างของ**หน้าต่างพิมพ์** ซึ่งแคบกว่ากระดาษ (`window.open(...,"width=900")` + จอ scale 125-150%) → ข้อความตัดบรรทัดต่างจากตอนพิมพ์จริง = สูงผิด = ย่อเกินจำเป็นมาก
  - แก้: บังคับความกว้างตอนวัดเป็น `210mm / z` ตายตัว (ไม่ผูกกับหน้าต่าง) + คืน `min-height: 297mm / z` หลังเลือกตัวย่อ → ภาพที่ออกมา **เต็ม 210×297mm เสมอ** ลายเซ็นยังปักท้ายหน้า · เปลี่ยนเป็น binary search เพื่อได้ตัวย่อ**ใหญ่ที่สุดที่ยังพอดี** (ใบเสร็จ 17 รายการ = 81%, ใบส่งสินค้า = 90%)
  - +guard: e2e ตรวจ **ความกว้าง/ความสูงที่เรนเดอร์ = 210×297mm** และ **ตัวย่อต้องเท่ากันทุกขนาดหน้าต่าง** (520px vs 1280px) · unit 12 ตัวบน fake layout ที่จำลอง "กล่องกว้างขึ้น → เนื้อหาเตี้ยลง" · mutation 4 แบบ
  - **ไม่มี SQL · ไม่แตะยอดเงิน/สต็อก/`functions/*`**
- `617` fix(documents): **กดพิมพ์แล้วได้ 4 แผ่น — แก้ที่ `doc-utils.js` ซึ่งเป็นเส้นทางจริง** (build 617 / v5.69.85)
  - อาการ: ใบเสร็จ/ใบส่งสินค้าที่รายการเยอะ พิมพ์ออกมา 4 แผ่น (ควรได้ 2 = ต้นฉบับ+สำเนา)
  - 🔴 เหตุที่ 613–616 แก้ไม่หาย: `doc-override.js` ดัก `rcPrintBtn`/`diPrintBtn`/`qtPrintBtn` ตั้งแต่ **capturing phase** แล้ว `stopImmediatePropagation()` → ปุ่มพิมพ์ใช้ `doc-utils.PRINT_CSS` มาตลอด **ไม่เคยใช้** `doc-print.css` หรือ CSS ที่ฝังใน `receipts.js`/`delivery_invoices.js` ที่แก้ไป 4 รอบ
  - วัดของจริงด้วย Chromium: `.doc-page` สูง **349mm** (ใบเสร็จ) / **314mm** (ใบส่งสินค้า) ที่ 15 รายการ — เกิน A4 297mm → ล้นไปแผ่นถัดไปทุกฉบับ ไม่ใช่ "หน้าเปล่า"
  - แก้ที่ `doc-utils.js`: (1) `fitPrintedPages()` ย่อ `.doc-page` ด้วย `zoom` จนพอดีหน้า เพดานล่าง 0.55 — **กติกาเดียวกับหน้าแชร์** ทั้งสองทางจึงได้เอกสารเหมือนกัน · (2) `printWhenReady()` รอฟอนต์/โลโก้โหลดก่อนวัด (เดิม `setTimeout` 600ms แล้วพิมพ์เลย) · (3) `PRINT_CSS` เติม `page-break-inside: avoid` (ของเดิม**ไม่มีเลย** แถวตาราง/บล็อกรับชำระจึงขาดครึ่ง) + `thead` ซ้ำหัวตาราง + `width` ตายตัว 210mm → `max-width`
...
- `609` fix(documents): **เอกสารที่เครื่องพิมพ์ออกมา 1 หน้า ต้องไม่กลายเป็น 2 หน้าในไฟล์แชร์** (build 609 / v5.69.77)
  ...
  - แก้แบบเดียวกับที่เครื่องพิมพ์ทำ: `computeFitToPage()` — ยาวเกินหน้าไม่เกิน ~18% ให้ **ย่อทั้งใบให้พอดีหน้าเดียว** (คงสัดส่วน จัดกึ่งกลาง) แทนการแตกหน้า · เกินกว่านั้น = ยาวจริง แบ่งหน้าตามขอบแถวเหมือนเดิม
  - +guard 23 → 28 (พอดีหน้า → scale 1 · เกิน 1-10% ต้องย่อ · เกินมากต้องคืน null ไปแบ่งหน้า · `contentExtentPx` · ต้องพิจารณาย่อก่อนตัดสินใจแบ่งหน้า) · **ไม่มี SQL · ไม่แตะยอดเงิน/สต็อก**

- `608` fix(documents,build): **แชร์เอกสารที่พอดีหนึ่งหน้าแล้วกลายเป็นสองหน้า (regression 607) + กันเลข build ปลอมหลอก regex** (build 608 / v5.69.76)
  - 🔴 **regression จาก build 607:** หักขอบล่าง 8mm กับ **ทุกหน้า** รวมหน้าสุดท้าย → พื้นที่หน้าเดียวเหลือ 289mm แทน 297mm → ใบเสนอราคาที่เดิมพอดี 1 หน้า ถูกดันบล็อกลายเซ็นไปหน้า 2 (owner เจอจริง; หน้าพรีวิว/หน้าพิมพ์ยังเป็น 1 หน้าปกติ)
  - แก้: **หน้าสุดท้ายใช้ความสูงกระดาษเต็ม** — ขอบล่างมีไว้กันเนื้อหาชนขอบเฉพาะหน้าที่ถูกตัดกลางเท่านั้น · เพิ่มการ **ตัดพื้นที่ว่างท้ายเอกสารทิ้ง** ก่อนแบ่งหน้า (canvas สูงเท่ากรอบ A4 เสมอ ส่วนที่เนื้อหาไม่ถึงคือ padding) = ไม่มีหน้าว่างท้ายเล่มด้วย
  - +regression test 2 เคส (เอกสารพอดีหนึ่งหน้าต้องได้หน้าเดียว · เนื้อหาจบก่อนขอบ canvas ต้องไม่เกิดหน้าเพิ่ม) + แก้ test เดิมที่ยืนยัน invariant ผิด (ผ่านเพราะบังเอิญ)
  - `sw.js` คอมเมนต์ changelog ของ v599 ยกข้อความโค้ดเก่ามาอ้างในรูปแบบประกาศตัวแปร → เครื่องมือที่อ่านแบบไม่ anchor จับเลขในคอมเมนต์ก่อนค่าจริงที่บรรทัดล่าง (staging เคยรายงาน SW build = 599 ทั้งที่ไฟล์เป็น 607) — เขียนคอมเมนต์ใหม่ให้ไม่อยู่ในรูปแบบประกาศ
  - `tests/e2e/smoke.spec.js` เดิมอ่าน `swText.match(/cache-v(\d+)/)` **แบบไม่ anchor** = กับดักเดียวกันรออยู่ → เปลี่ยนเป็น anchored ที่บรรทัดประกาศจริง
  - +guard `build_marker_decoy` (6): `sw.js`/`index.html` ต้องมีเลข build **จุดเดียว** จริง ๆ · เลข 3 จุดตรงกัน · ตัวอ่านทุกตัวต้อง anchored (กันผ่อนทีหลัง)
  - **ไม่มี SQL · ไม่แตะ logic/cache strategy/ยอดเงิน**
...
- `606-b2c` feat(customer,security): **ลูกค้าเห็น "งานบริการของฉัน" ได้จริง + ยืนยันปิดงานแบบ flow-aware ผ่าน authenticated proxy** (build 605 / v5.69.73)
  ...
  - flow-aware: v1 `done`/`delivered` → `closed` · **v2 `delivered` เท่านั้น** (v2 `done` ถูกปฏิเสธแม้ยังไม่มี JV) · unknown flow = fail closed · **PATCH เฉพาะ `status` — note byte-exact ไม่ถูกแตะ**
  - 🔒 privacy fix: `sw.js` ให้ same-origin `/api/*` เป็น network-only (ไม่ cache.put / ไม่ fallback cache / offline = 503) — เดิม response ที่ผูกกับ user ถูก cache ตาม URL ทำให้เครื่องแชร์เห็นข้อมูลของอีกคนได้ (ครอบ `/api/v1/loyalty-balance` เดิมด้วย)
  - แท็บประวัติซื้อ = **honest unavailable state** (ไม่อ้างว่า "ยังไม่มีประวัติ") — residual เป็น Phase 606-b2d แยก
  - +guards customer_service_jobs_proxy (38) + customer_dashboard_service_flow (23) · mutation proof 32/32 · **ไม่มี SQL · ไม่ activate flow v2 · ไม่แตะ main.js/payment/JV/stock**
  - ⚠️ deviation: แตะ test เดิม 6 ไฟล์นอก scope (build pin 604 + guard ที่ผูกโค้ดที่ถูกถอด) — เหตุผลรายไฟล์ใน PR
  - 🔴 customer behavioral staging = NOT RUN · production customer confirm = NOT VERIFIED · 606-b3/607 NOT STARTED
- `606-B13a-closeout` docs(staging): **🎉 authenticated payment behavioral execution PASS (owner-run บน scratch เดิมของ B12 · source `17ab723` · final DB date 2026-08-02 · build 604 / v5.69.72 ไม่ bump)** — runtime ครบ lifecycle: recovery → S0.1–S0.7 → S0-RELOAD → PREFLIGHT → temp Auth admin → SEED → bootstrap → gates CAS → intent snapshot → **r1** (`inserted=true` · ledger+JV posted · 100/900) → **r2** (**duplicate-valid · `inserted=false`** · IDs เดิม · payment 1 แถว/JV 1 ใบ/lines 2/reversal 0 — ไม่มี write ซ้ำ) → verify_db → teardown (**deleted 8 rows exact-ID** · residual 0/0/0/0/0) → actor/local cleanup ครบ (Auth/profile 0/0 · clean login ถูกปฏิเสธ) → `execution_complete`
  - **certificates 2/2: `PAYMENT_BEHAVIOR_PASS` = 1 · `EXECUTION_COMPLETE` = 1** · evidence 6 (browser 3 + attestation 3 · failure 0) · retained evidence + sentinel คงบน scratch ห้ามลบ · B12 6/6 + 68/36 คงเดิม
  - behavioral results = **owner-attested จาก retained scratch evidence** (runs/results/evidence · คงไว้ให้ cross-team review · reviewer ไม่ได้ rerun SQL) · HTTP 409 ตอน duplicate JV attempt = owner-observed behavior ที่สอดคล้องกับ expected duplicate-valid retry contract · production untouched ตลอดทุกรอบ · docs-only ไม่รัน SQL เพิ่ม ไม่ bump build · 606-b2c/606-b3/607 ยังไม่เริ่ม ต้องมี prompt/audit/approval แยก
- `606-B13a.1.2` fix(staging): **normalize function-body EOL comparisons (package hotfix · build 604 / v5.69.72 ไม่ bump)** *(execution status ของ entry นี้ถูก superseded โดย closeout entry ด้านบน — เก็บ failure record เดิมไว้เป็นหลักฐาน)* — owner-run รอบสองบน scratch (DB date `2026-07-29` · package `e676f9c`): STEP 1–3 ผ่าน (sentinel `B13A-STAGING-2026-07-29` 1/1/true · `R0` 17/17) แต่ **STEP 4 `S0-ACL-RECOVERY` ล้ม SQLSTATE `P0001`** — raw `md5(v_prosrc)` เทียบกับ `c_md5` ที่ผูก canonical **LF** ขณะ prosrc บน scratch เป็น **CRLF** จาก SQL Editor paste (deterministic proof: LF md5 `8a185d25…` · LF→CRLF md5 `198fb98c…` ตรง actual เป๊ะ = line-ending transport ล้วน) · แก้: recovery + reuse `S0.4`–`S0.7` ใช้ EOL contract เดียวกัน — exact `replace(value, E'\r\n', E'\n')` ทั้งสองฝั่ง + reject lone CR + ห้าม trim/regexp folding · `c_md5` คงเดิม · guard G24/G60-G61 → normalized form + **G69** ใหม่ + mutation M1–M13
  - **ผลต่อ scratch:** error ก่อน `REVOKE` ทั้ง 4 · `DO` rollback ทั้ง statement · **no REVOKE committed** · ไม่มี business/payment/JV mutation · ACL surplus ยังอยู่ · `S0.1`–`S0.7` ไม่ได้รันในรอบนี้ · **certificates NOT ISSUED** · **ACL recovery ยัง NOT PASS**
  - **CRLF↔LF equivalence = ผ่อน byte-exact โดยตั้งใจเฉพาะ line-ending transport** — whitespace/tabs/comments/body semantics ยังต้อง exact · ความต่างอื่น STOP
  - ⚠️ guard/CI เขียวไม่ใช่ scratch runtime proof · 🚫 ห้าม rerun SHA `e676f9c` · หลัง merge ต้องเริ่ม execution prompt ใหม่ + refresh sentinel จาก `current_date` รอบใหม่
- `606-B13a.1.1` fix(staging): **cast `relkind` in recovery inventory (package hotfix · build 604 / v5.69.72 ไม่ bump)** — owner-run บน scratch เมื่อ DB date `2026-07-27`: STEP 1–3 ผ่าน (`R0` 17/17) แต่ **STEP 4 `S0-ACL-RECOVERY` ล้ม SQLSTATE `42725`** เพราะ unknown-object inventory ต่อสตริง `c.relname || ':' || c.relkind` โดยไม่ cast (`pg_class.relkind` = internal `"char"` → `text || "char"` = ambiguous operator) · แก้ surgical เป็น `c.relkind::text` · คง filter `c.relkind <> ALL (ARRAY['i','I','t'])` เดิม · ไม่แตะ tri-state / sentinel interlock / residue predicates / canonical body+MD5 / mutation set 4 REVOKE · guard ใหม่ **G68** + mutation proof M1/M2
  - **ผลต่อ scratch:** error เกิดก่อน `REVOKE` ทั้ง 4 คำสั่ง · `DO` rollback ทั้ง statement · **no REVOKE committed** · ไม่มี business/payment/JV/accounting mutation · ACL surplus ยังอยู่ interrupted state เดิม · S0.1–S0.4 ยังไม่ rerun · **S0.5–S0.7 NOT RUN** · **certificates NOT ISSUED**
  - ⚠️ **guard/CI เขียวไม่ใช่ scratch runtime ACL proof** — G68 เป็น source-regression proof ไม่ใช่ PostgreSQL runtime execution proof · authenticated payment behavioral verification ยังไม่ผ่าน
  - 🚫 ห้าม rerun package SHA `fefe477` · ห้ามแก้ SQL ด้วยมือใน SQL Editor · หลัง merge ต้องเริ่ม execution prompt ใหม่ + derive sentinel จาก `current_date` ของ session รอบใหม่ · ห้ามถือผล STEP 1–3 ของวันเดิมข้ามวัน/session
- `606-B13a.1` fix(staging): **close B13a fresh-create ACL gap + S0-ACL-RECOVERY (package hotfix · build 604 / v5.69.72 ไม่ bump)** — fresh-create S0.1–S0.7 ถอน default ACL ของ Supabase API role ให้ครบ + assert postcondition ด้วย `aclexplode()` ทันทีหลัง REVOKE/GRANT (ไม่ผ่าน = rollback ทั้ง DO statement) · เพิ่ม `S0-ACL-RECOVERY` (tri-state `NO-OP` / `PASS` / `STOP` · mutation เดียว = REVOKE surplus บน 4 objects เป๊ะ · precheck→REVOKE→post-check ใน statement เดียว) · residue absence proof ใช้ predicates เดียวกับ PREFLIGHT ทั้งสี่ (เห็น orphan JE `B13ATEST-JV1` ด้วย) · bootstrap identity ผูกด้วย canonical body MD5 ที่ guard cross-check สองทิศ · runbook เพิ่ม atomic sentinel refresh (B4.3) แบบ self-gated DO statement เดียว + B11 sentinel = current active row · guard ใหม่ G46–G67
  - **สถานะ scratch (2026-07-26):** `R0` 17/17 · **S0.1–S0.4 สำเร็จ** (3 ตาราง + bootstrap · 0 rows) · **S0.5–S0.7 NOT RUN** · PREFLIGHT/SEED/bootstrap/payment/JV NOT RUN · **certificates NOT ISSUED** · ไม่มี B13a SQL บน production
  - ⚠️ **guard/CI เขียวไม่ใช่ scratch ACL proof** — พิสูจน์แค่ว่า source ตรง contract · authenticated payment behavioral verification ยังไม่ผ่าน · 606-b2c/606-b3/607 ยังไม่ได้รับอนุญาต
  - deviation: G39 เดิม ban คำว่า `service_role` ทั้งไฟล์ (ขัดข้อกำหนดเฟสนี้) → เปลี่ยนเป็น context allowlist + เพิ่มข้อห้าม credential-context · G1–G38/G40–G45 ไม่ถูกแตะ
- `33356a4` test(staging,606-B13a): **authenticated cash-payment staging verification package (PR #202 merged · package-only · build 604 / v5.69.72 ไม่ bump)** *(บล็อกประวัติ ณ ตอน package merge — ข้อความ "execution = NOT RUN" ด้านล่างถูก **superseded** โดย partial execution 2026-07-26 · ดูรายการ 606-B13a.1 ด้านบน)* — 3 ไฟล์ใหม่: `staging-verify-b13a-auth-payment.sql` (sentinel-interlocked ทุก mutation unit รวม S0 wrappers · singleton run-state + typed evidence · sanctioned writers 3 ตัว · intent snapshot ครั้งเดียว + NULL-safe 8-field compare · one-time ID binding · fail-closed failure/abort/classify · exact-ID teardown · introspect-or-create-exact แบบ exact จริง: signature/prosrc/ACL allowlist+no-grant-option/owner-trust/constraints/defaults/policy) + `STAGING_B13A_RUNBOOK.md` (B1–B11) + `tests/staging_b13a_runbook_guard.test.js` (37 blocks / 45+ checks) · Codex audit 4 รอบ = ผ่าน (mutation proof 51/51 แดง) · **B13a execution = NOT RUN · PAYMENT_BEHAVIOR_PASS / EXECUTION_COMPLETE ยังไม่ออก — ห้ามถือว่า authenticated payment ผ่านจาก CI** · execution เป็น owner-controlled phase แยกบน scratch เดิมของ B12 · 606-b2c/606-b3/607 ยังไม่ได้รับอนุญาต

- docs(staging,606-B12-closeout): **Phase 606-B12 staging behavioral execution = PASS (owner-run 2026-07-22 Asia/Bangkok · target DB session 2026-07-21 UTC · build 604 / v5.69.72 ไม่ bump)** — B12a–e + TEARDOWN = **6/6 ok=true** จาก behavioral execution จริงบน scratch staging `bsk-staging-b12` (Path B: scratch + production database SQL/schema/data operations สำหรับ B12 เป็น read-only เท่านั้น — schema-only dump + data เฉพาะ `chart_of_accounts`/`account_mapping`; ไม่มี DDL/DML/trigger disable/`NOTIFY pgrst` บน production) · service_jobs triggers **6/6 'O'** + journal required **2/2** · residual B12 test rows jobs/JE/JL = **0/0/0** · `_staging_b12_results` + sentinel retained บน scratch (scratch ยังไม่ลบ — cleanup = คำสั่ง owner แยก) · **hard gate B12 ผ่านแล้ว — 606-b3 ยังไม่เริ่ม/ยังไม่ได้รับอนุญาต** (ต้อง prompt/audit/owner approval แยก) · known gap: authenticated payment RPC behavioral path ยังไม่ได้ทดสอบ · closeout นี้ docs-only ไม่รัน SQL เพิ่ม (staging B12 SQL รันแล้วภายใต้ owner control ก่อนหน้า)

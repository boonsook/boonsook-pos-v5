...
- Phase 628A feat(documents): **schema foundation `item_type` (`item|heading`) ของรายการเอกสาร 3 ตาราง — SQL owner-run ยังไม่ apply** (build 626 / v5.69.93 ไม่ bump · schema + tests + docs เท่านั้น)
  - +`supabase-phase628a-document-item-type-schema.sql` (byte-exact จาก prompt §6): `quotation_items` / `delivery_invoice_items` / `receipt_items` ได้ `item_type text NOT NULL DEFAULT 'item'` + CHECK `item|heading` ชื่อ `<table>_item_type_check`; ข้อมูลเดิม 342 แถว → `item` ทั้งหมด · ไม่ infer heading จากราคา 0 / `product_id=NULL` · ไม่สร้าง heading row · fail closed เมื่อเจอ column/CHECK ที่ไม่รู้ที่มา · มี post-check A/B

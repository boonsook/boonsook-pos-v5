...
- fix(products,build 507): หน้าสินค้า — **ช่องค้นหาบนมือถือพิมพ์ภาษาไทยแล้วตัวอักษรซ้ำ/เด้งเอง** (เช่น "หน้" → "หน้หน้า" ทั้งที่ไม่ได้พิมพ์) แก้แล้ว. สาเหตุ: ทุกครั้งที่พิมพ์ ระบบวาดหน้าใหม่ทั้งหน้า (สร้างช่องค้นหาใหม่ + re-focus) — ถ้าทำ "ตอนกำลังประกอบสระ/วรรณยุกต์" (Thai IME composition) ตัวอักษรจะซ้ำ. แก้: ข้ามการค้นหาระหว่างประกอบตัวอักษร (`e.isComposing`) แล้วค้นหาตอนประกอบเสร็จ (`compositionend`) — ช่องค้นหาไม่โดนสร้างใหม่กลางพิมพ์. surgical เฉพาะ logic ช่องค้นหา `products.js` ไม่แตะ render/binding/สต็อก. +guard products_search_ime. ⚠️ รอ owner smoke (มือถือ)
